...
static u32 Ai_SetMoveAccuracy(struct AiLogicData *aiData, u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (abilityAtk == ABILITY_NO_GUARD || abilityDef == ABILITY_NO_GUARD || GetMoveAccuracy(move) == 0) // Moves with accuracy 0 or no guard ability always hit.
        ...
        accuracy = GetTotalAccuracy(battlerAtk, battlerDef, move, abilityAtk, abilityDef);
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    bool32 isDoubleBattle = IsValidDoubleBattle(battlerAtk), hasValidSlot = FALSE;
    u32 i;
    u32 weather;
    ...
    if (IsPowderMove(move) && !IsAffectedByPowder(battlerDef, aiData->abilities[battlerDef], Ai_BattlerHasHoldEffect(battlerDef, HOLD_EFFECT_SAFETY_GOGGLES, aiData)))
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_BESTOW:
            ...
            for (i = 0; i < MAX_MON_ITEMS; i++)
            {
                if (CanBattlerGetOrLoseItem(battlerAtk, gBattleMons[battlerAtk].items[i]))
                    {
                        hasValidSlot = TRUE;
                        break;
                    }
            ...
            case EFFECT_SKILL_SWAP:
                if (aiData->abilities[battlerAtk] != aiData->abilities[BATTLE_PARTNER(battlerAtk)] && !attackerHasBadAbility)
                {
                    ...
                    if (aiData->abilities[battlerAtk] == ABILITY_COMPOUND_EYES
                        && HasMoveWithLowAccuracy(battlerAtkPartner, FOE(battlerAtkPartner), 90, TRUE, atkPartnerAbility, aiData->abilities[FOE(battlerAtkPartner)]))
                    {
                        ADJUST_SCORE(GOOD_EFFECT);
                    }
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    bool32 isDoubleBattle = IsValidDoubleBattle(battlerAtk), hasBerry = FALSE;
    u32 i;
    ...
    case EFFECT_LOCK_ON:
        if (HasMoveWithEffect(battlerAtk, EFFECT_OHKO))
            ADJUST_SCORE(GOOD_EFFECT);
        else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 85, TRUE, aiData->abilities[battlerAtk], aiData->abilities[battlerDef]))
            ADJUST_SCORE(GOOD_EFFECT);
        break;
    case EFFECT_DESTINY_BOND:
        if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
    ...
    case EFFECT_SANDSTORM:
        ...
        break;
    case EFFECT_HAIL:
        if (ShouldSetHail(battlerAtk, aiData->abilities[battlerAtk]))
        {
        ...
        break;
    case EFFECT_SNOWSCAPE:
        if (ShouldSetSnow(battlerAtk, aiData->abilities[battlerAtk]))
        {
            if ((HasMoveWithEffect(battlerAtk, EFFECT_AURORA_VEIL) || HasMoveWithEffect(BATTLE_PARTNER(battlerAtk), EFFECT_AURORA_VEIL))
              && ShouldSetScreen(battlerAtk, battlerDef, EFFECT_AURORA_VEIL))
                ADJUST_SCORE(GOOD_EFFECT);
    ...
    case EFFECT_FOLLOW_ME:
        if (isDoubleBattle
          ...
          && !IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef])
          && (!IsPowderMove(move) || IsAffectedByPowder(battlerDef, aiData->abilities[battlerDef], Ai_BattlerHasHoldEffect(battlerDef, HOLD_EFFECT_SAFETY_GOGGLES, aiData)))// Rage Powder doesn't affect powder immunities
          && IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
        {
    ...
    case EFFECT_RECYCLE:
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetUsedHeldItem(battlerAtk, i) != ITEM_NONE)
                {
                    ADJUST_SCORE(WEAK_EFFECT);
                    break;
                }
            if (IsRecycleEncouragedItem(GetUsedHeldItem(battlerAtk, i)))
                {
                    ADJUST_SCORE(WEAK_EFFECT);
                    break;
                }
            ...
            {
                u32 item = GetUsedHeldItem(battlerAtk, i);
    ...
    case EFFECT_GRAVITY:
        if (!(gFieldStatuses & STATUS_FIELD_GRAVITY))
        {
            if (HasSleepMoveWithLowAccuracy(battlerAtk, battlerDef)) // Has Gravity for a move like Hypnosis
                IncreaseSleepScore(battlerAtk, battlerDef, move, &score);
            if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, FALSE, aiData->abilities[battlerAtk], aiData->abilities[battlerDef]))
                ADJUST_SCORE(DECENT_EFFECT);
        }
        break;
    case EFFECT_ION_DELUGE:
                ...
                case MOVE_EFFECT_STEAL_ITEM:
                    {
                        ...
                        for (i = 0; i < MAX_MON_ITEMS; i++)
                        {
                            if (!CanBattlerGetOrLoseItem(battlerDef, aiData->items[battlerDef][i])
                        ...
                        && aiData->abilities[battlerDef] != ABILITY_STICKY_HOLD)
                        {
                            ...
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_CHOICE_SCARF, aiData))
                            {
                                ADJUST_SCORE(DECENT_EFFECT);
                            }
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_CHOICE_SPECS, aiData))
                            {
                                ADJUST_SCORE(DECENT_EFFECT);
                            }
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_BLACK_SLUDGE, aiData))
                            {
                                ADJUST_SCORE(DECENT_EFFECT);
                            }
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_BLACK_SLUDGE, aiData))
                            ...
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_TOXIC_ORB, aiData))
                            {
                                if (ShouldPoison(battlerAtk, battlerAtk))
                                    ADJUST_SCORE(DECENT_EFFECT);
                            }
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_FLAME_ORB, aiData))
                            {
                                if (ShouldBurn(battlerAtk, battlerAtk, aiData->abilities[battlerAtk]))
                                    ADJUST_SCORE(DECENT_EFFECT);
                            }
                            ...
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_IRON_BALL, aiData))
                            {
                                if (HasMoveWithEffect(battlerAtk, EFFECT_FLING))
                                    ADJUST_SCORE(DECENT_EFFECT);
                            }
                            if(Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_LAGGING_TAIL, aiData)
                             || Ai_BattlerHasItem(battlerDef, HOLD_EFFECT_STICKY_BARB, aiData))
                            {
                                break;
                            }
