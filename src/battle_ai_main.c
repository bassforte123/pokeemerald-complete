...
void SetBattlerAiData(u32 battler, struct AiLogicData *aiData)
{
    ...
    aiData->speedStats[battler] = GetBattlerTotalSpeedStat(battler, ability);
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IsPowderMove(move) && !IsAffectedByPowderMove(battlerDef, aiData->abilities[battlerDef], Ai_BattlerHasHoldEffect(battlerDef, HOLD_EFFECT_SAFETY_GOGGLES, aiData)))
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if (IsBattlerTerrainAffected(battlerDef, abilityDef, STATUS_FIELD_ELECTRIC_TERRAIN))
        {
            if (nonVolatileStatus == MOVE_EFFECT_SLEEP)
                RETURN_SCORE_MINUS(20);
        }

        if (IsBattlerTerrainAffected(battlerDef, abilityDef, STATUS_FIELD_MISTY_TERRAIN))
        {
            if (IsNonVolatileStatusMove(move) || IsConfusionMoveEffect(moveEffect))
                RETURN_SCORE_MINUS(20);
        }

        if (IsBattlerTerrainAffected(battlerAtk, abilityAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && atkPriority > 0)
        {
            RETURN_SCORE_MINUS(20);
        }
...
static s32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_ATTRACT:
        ...
        break;
    case EFFECT_SAFEGUARD:
        if (!IsBattlerTerrainAffected(battlerAtk, aiData->abilities[battlerAtk], STATUS_FIELD_MISTY_TERRAIN) || !AI_IsBattlerGrounded(battlerAtk))
    ...
    case EFFECT_FOLLOW_ME:
        if (hasPartner
          ...
          && !IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef])
          && (!IsPowderMove(move) || IsAffectedByPowderMove(battlerDef, aiData->abilities[battlerDef], Ai_BattlerHasHoldEffect(battlerDef, HOLD_EFFECT_SAFETY_GOGGLES, aiData))))
