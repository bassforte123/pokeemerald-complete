...
static bool32 HandleEndTurnLeechSeed(enum BattlerId battler)
{
     ...
     && !IsAbilityAndRecord(battler, ABILITY_MAGIC_GUARD))
    {
        ...
        if (BattlerHasTrait(battler, ABILITY_LIQUID_OOZE))
        {
            ...
            PushTraitStack(battler, ABILITY_LIQUID_OOZE);
...
static bool32 HandleEndTurnPoison(enum BattlerId battler)
{
    ...
    if (IsBattlerPresent(battler) && (gBattleMons[battler].status1 & STATUS1_POISON || isToxicPoison))
    {
        ...
        else if (BattlerHasTrait(battler, ABILITY_POISON_HEAL))
        {
            if (isToxicPoison && (gBattleMons[battler].status1 & STATUS1_TOXIC_COUNTER) != STATUS1_TOXIC_TURN(15)) // not 16 turns
                gBattleMons[battler].status1 += STATUS1_TOXIC_TURN(1);
            ...
            if (!IsBattlerAtMaxHp(battler) && !gBattleMons[battler].volatiles.healBlock)
            {
                ...
                PushTraitStack(battler, ABILITY_POISON_HEAL);
...
static bool32 HandleEndTurnBurn(enum BattlerId battler)
{
     ...
     && !IsAbilityAndRecord(battler, ABILITY_MAGIC_GUARD))
    {
        ...
        PushTraitStack(battler, ABILITY_HEATPROOF);
...
static bool32 HandleEndTurnWrap(enum BattlerId battler)
{
     ...
     && IsBattlerPresent(battler))
    {
        if (gBattleMons[battler].volatiles.wrapTurns != 0)
        {
            ...
            s32 bindDamage = 0;
            if (gBattleMons[battler].volatiles.wrappedBindingBand > 0)
                bindDamage = GetNonDynamaxMaxHP(battler) / (B_BINDING_DAMAGE >= GEN_6 ? 6 - (gBattleMons[battler].volatiles.wrappedBindingBand - 1)
                                                                                      : 8 - (gBattleMons[battler].volatiles.wrappedBindingBand - 1));
            else
...
static bool32 HandleEndTurnYawn(enum BattlerId battler)
{
    ...
    if (gBattleMons[battler].volatiles.yawn > 0)
    {
         ...
         && !IsLeafGuardProtected(battler))
        {
            ...
            if (IsElectricTerrainAffected(battler, gFieldStatuses))
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_TERRAINPREVENTS_ELECTRIC;
                BattleScriptCall(BattleScript_TerrainPrevents);
            }
            else if (IsMistyTerrainAffected(battler, gFieldStatuses))
            {
            ...
            else if ((gBattleScripting.battler = IsAbilityOnSide(battler, ABILITY_SWEET_VEIL)))
            {
                ...
                PushTraitStack(gBattleScripting.battler, ABILITY_SWEET_VEIL);
    ...
    case THIRD_EVENT_BLOCK_UPROAR:
        if (gBattleMons[battler].volatiles.uproarTurns)
        {
            for (gEffectBattler = 0; gEffectBattler < gBattlersCount; gEffectBattler++)
            {
                ...
                bool32 hasSoundproof = GetConfig(B_UPROAR_IGNORE_SOUNDPROOF) < GEN_5 && BattlerHasTrait(gEffectBattler, ABILITY_SOUNDPROOF);
...
static bool32 HandleEndTurnFormChange(enum BattlerId battler)
{
        ...
        || TryBattleFormChange(battler, FORM_CHANGE_BATTLE_HP_PERCENT_TURN_END))
    {
        ...
        {
            gBattleScripting.abilityPopupOverwrite = ABILITY_POWER_CONSTRUCT; // To prevent the new form's ability from pop up
            PushTraitStack(battler, ABILITY_POWER_CONSTRUCT);
            BattleScriptCall(BattleScript_PowerConstruct);
        }
        else if (SearchTraits(battlerTraits, ABILITY_HUNGER_SWITCH))
        {
        ...
        else if (SearchTraits(battlerTraits, ABILITY_ZEN_MODE))
        {
            ...
            gBattleScripting.abilityPopupOverwrite = ABILITY_ZEN_MODE; // To prevent the new form's ability from pop up
            PushTraitStack(battler, ABILITY_ZEN_MODE);
            BattleScriptCall(BattleScript_ZenMode);
        }
        else if (SearchTraits(battlerTraits, ABILITY_SCHOOLING))
        {  
            gBattleScripting.abilityPopupOverwrite = ABILITY_SCHOOLING; // To prevent the new form's ability from pop up
