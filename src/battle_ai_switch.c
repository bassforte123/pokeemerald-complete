...
bool32 IsSwitchinTSpikesAffected(enum BattlerId battler)
{
    ...
    bool32 ignoreItem = ((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || AI_BATTLER_HAS_TRAIT(battler, ABILITY_KLUTZ));
    if (gBattleMons[battler].status1 & STATUS1_ANY)
        return FALSE;
    if (IS_BATTLER_ANY_TYPE(battler, TYPE_POISON, TYPE_STEEL))
        return FALSE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_IMMUNITY) || AI_IsAbilityOnSide(battler, ABILITY_PASTEL_VEIL))
        return FALSE;
    if ((Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_HEAVY_DUTY_BOOTS, gAiLogicData) || Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_CURE_PSN, gAiLogicData) || Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_CURE_STATUS, gAiLogicData)) && !ignoreItem)
        return FALSE;
    ...
    if (IsFlowerVeilProtected(battler))
        return FALSE;
    if (IsSafeguardProtected(opposingBattler, battler))
        return FALSE;
    return TRUE;
}
...
static bool32 AI_DoesChoiceEffectBlockMove(enum BattlerId battler, enum Move move)
{
    ...
    if (gAiLogicData->lastUsedMove[battler] != MOVE_NONE && gAiLogicData->lastUsedMove[battler] != move
     ...
     || AI_BATTLER_HAS_TRAIT(battler, ABILITY_GORILLA_TACTICS)))
        return TRUE;
    return FALSE;
}
...
static bool32 ShouldSwitchIfHasBadOdds(struct SwitchAiContext *switchContext)
{
    ...
    if ((switchContext->battlerGetsOHKOd && !switchContext->canBattlerWin1v1) && (gBattleMons[switchContext->battler].hp >= gBattleMons[switchContext->battler].maxHP / 2 // And the current mon has at least 1/2 their HP, or 1/4 HP and Regenerator
            || (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_REGENERATOR) && gBattleMons[switchContext->battler].hp >= gBattleMons[switchContext->battler].maxHP / 4)))
    {
    ...
    if (switchContext->typeMatchup > UQ_4_12(2.0)) // If the player has favourable offensive matchup (2.0 is neutral, this must be worse)
    {
        ...
        && (gBattleMons[switchContext->battler].hp >= gBattleMons[switchContext->battler].maxHP / 2 // And the current mon has at least 1/2 their HP, or 1/4 HP and Regenerator
            || (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_REGENERATOR)
...
static bool32 ShouldSwitchIfTruant(struct SwitchAiContext *switchContext)
{
    ...
    if (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_TRUANT)
...
static bool32 ShouldSwitchIfWonderGuard(struct SwitchAiContext *switchContext)
{
    ...
    if (!AI_BATTLER_HAS_TRAIT(switchContext->opposingBattler, ABILITY_WONDER_GUARD))
        return FALSE;
...
static bool32 FindMonThatAbsorbsOpponentsMove(struct SwitchAiContext *switchContext)
{
    ...
    if (AreStatsRaised(switchContext->battler))
        return FALSE;
    if (HasMoldBreakerTypeAbility(switchContext->opposingBattler))
        return FALSE;
    if (switchContext->canBattlerWin1v1)
        return FALSE;
    ...
    {
        if (AI_BATTLER_HAS_TRAIT(switchContext->battler, absorbingTypeAbilities[absorbingAbilityIndex]))
            return FALSE;
    }
    ...
    for (u32 monIndex = 0; monIndex < switchContext->lastId; monIndex++)
    {
        ...
        party = GetBattlerParty(switchContext->battler);

        for (u32 absorbingAbilityIndex = 0; absorbingAbilityIndex < numAbsorbingAbilities; absorbingAbilityIndex++)
        {
            // Found a mon
            if (MonHasTrait(&party[monIndex], absorbingTypeAbilities[absorbingAbilityIndex]) && RandomPercentage(RNG_AI_SWITCH_ABSORBING, GetSwitchChance(SHOULD_SWITCH_ABSORBS_MOVE)))
...
static bool32 ShouldSwitchIfTrapperInParty(struct SwitchAiContext *switchContext)
{
    ...
    for (u32 monIndex = 0; monIndex < switchContext->lastId; monIndex++)
    {
        ...
        if (AI_CanSwitchinAbilityTrapOpponent(partyMonAbility, switchContext->opposingBattler, species) || (AI_CanSwitchinAbilityTrapOpponent(gAiLogicData->abilities[switchContext->opposingBattler], switchContext->opposingBattler, gBattleMons[switchContext->opposingBattler].species) && partyMonAbility == ABILITY_TRACE))
        {
...
static bool32 ShouldSwitchIfBadlyStatused(struct SwitchAiContext *switchContext)
{
    ...
    AI_STORE_BATTLER_TRAITS(switchContext->battler);
    //Perish Song
    ...
    if (gAiThinkingStruct->aiFlags[switchContext->battler] & AI_FLAG_SMART_SWITCHING)
    {
        ...
        if (gBattleMons[switchContext->battler].volatiles.yawn
            && CanBeSlept(switchContext->battler, switchContext->battler, BLOCKED_BY_SLEEP_CLAUSE)
            ...
            && RandomPercentage(RNG_AI_SWITCH_YAWN, GetSwitchChance(SHOULD_SWITCH_YAWN)))
        {
            ...
            if ((SearchTraits(AIBattlerTraits, ABILITY_NATURAL_CURE)
                ...
                || Ai_BattlerHasHoldEffect(switchContext->battler, HOLD_EFFECT_CURE_SLP, gAiLogicData) || Ai_BattlerHasHoldEffect(switchContext->battler, HOLD_EFFECT_CURE_STATUS, gAiLogicData)
                ...
                || (IsBattlerGrounded(switchContext->battler)
            ...
            enum Ability AIBattlerTraits[MAX_MON_TRAITS];
            AI_STORE_BATTLER_TRAITS(switchContext->opposingBattler);
        ...
        // Secondary Damage
        if (!AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_MAGIC_GUARD)
...
static bool32 GetHitEscapeTransformState(enum BattlerId battlerAtk, enum Move move)
{
    ...
    if (gBattleMons[battlerAtk].species != SPECIES_PALAFIN_ZERO
     || !AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_ZERO_TO_HERO))
        return FALSE;
    ...
    for (enum BattlerId battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        bool32 moldBreakerNegated = AI_MoldBreakerNegates(
            battlerAtk,
            battlerDef,
            move
        );
        ...
        {
            if ((moveType == TYPE_WATER && !moldBreakerNegated && AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_STORM_DRAIN))
             || (moveType == TYPE_ELECTRIC && !moldBreakerNegated && AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_VOLT_ABSORB)))
...
static bool32 CanIntimidateLowerOpponentAtk(enum BattlerId battler, enum BattlerId opposingBattler)
{
    ...
    if (Ai_BattlerHasHoldEffect(opposingBattler, HOLD_EFFECT_CLEAR_AMULET, gAiLogicData))
        return FALSE;
    ...
    if (SearchTraits(AIBattlerTraits, ABILITY_HYPER_CUTTER)
    || SearchTraits(AIBattlerTraits, ABILITY_CLEAR_BODY)
    || SearchTraits(AIBattlerTraits, ABILITY_FULL_METAL_BODY)
    || SearchTraits(AIBattlerTraits, ABILITY_WHITE_SMOKE))
        return FALSE;

    if (GetConfig(B_UPDATED_INTIMIDATE) >= GEN_8)
    {
        if (SearchTraits(AIBattlerTraits, ABILITY_INNER_FOCUS)
        || SearchTraits(AIBattlerTraits, ABILITY_SCRAPPY)
        || SearchTraits(AIBattlerTraits, ABILITY_OWN_TEMPO)
        || SearchTraits(AIBattlerTraits, ABILITY_OBLIVIOUS))
            return FALSE;
    }
...
static bool32 ShouldSwitchIfIntimidateBenefit(struct SwitchAiContext *switchContext)
{
    ...
    if (IsBattlerAlive(switchContext->opposingBattler))
    {
        ...
        if (canLowerAtk && (DoesIntimidateRaiseStats(switchContext->opposingBattler) || AI_BATTLER_HAS_TRAIT(switchContext->opposingBattler, ABILITY_MIRROR_ARMOR)))
            return FALSE;
    ...
    if (IsDoubleBattle() && IsBattlerAlive(opposingPartner))
    {
        ...
        if (canLowerAtk && (DoesIntimidateRaiseStats(opposingPartner) || AI_BATTLER_HAS_TRAIT(opposingPartner, ABILITY_MIRROR_ARMOR)))
            return FALSE;
...
static bool32 ShouldSwitchIfAbilityBenefit(struct SwitchAiContext *switchContext)
{
    ...
    if (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_NATURAL_CURE))
    {
    ...
    if (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_REGENERATOR))
    {
    ...
    if (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_INTIMIDATE))
    {
    ...
    if (AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_ZERO_TO_HERO))
    {
...
static bool32 ShouldSwitchIfBadChoiceLock(struct SwitchAiContext *switchContext)
{
    ...
    {
        enum BattlerId opposingPartner = BATTLE_PARTNER(switchContext->opposingBattler);
        if (BattlerHasHoldEffectChoice(ctx.battlerAtk) && IsBattlerItemEnabled(switchContext->battler))
        {
    ...
    else if (BattlerHasHoldEffectChoice(ctx.battlerAtk) && IsBattlerItemEnabled(switchContext->battler))
    {
...
void GetShouldSwitchMoveData(struct SwitchAiContext *switchContext)
{
    ...
    for (u32 moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
    {
        ...
        if (playerMove != MOVE_NONE && !IsBattleMoveStatus(playerMove) && GetMoveEffect(playerMove) != EFFECT_FOCUS_PUNCH && gBattleMons[switchContext->opposingBattler].pp[moveIndex] > 0)
        {
            ...
            if (GetBattleMovePriority(switchContext->opposingBattler, playerMove) > 0 && hitsToKOAI < minHitsToKOAIPriority && !AI_DoesChoiceEffectBlockMove(switchContext->opposingBattler, playerMove))
            {
    ...
    for (u32 moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
    {
        ...
        if (aiMoveEffect == EFFECT_REVIVAL_BLESSING || IsSwitchOutEffect(aiMoveEffect))
        {
            ...
            if (gBattleMons[switchContext->battler].species == SPECIES_PALAFIN_ZERO
             && AI_BATTLER_HAS_TRAIT(switchContext->battler, ABILITY_ZERO_TO_HERO)
...
static u32 GetSwitchinSingleUseItemHealing(enum BattlerId battler, enum BattlerId opposingBattler, s32 currentHP)
{
    ...
    if ((currentHP <= 0)
     || AI_BATTLER_HAS_TRAIT(battler, ABILITY_KLUTZ)
     || (AI_BATTLER_HAS_TRAIT(opposingBattler, ABILITY_UNNERVE) && BattlerHasBerry(battler)))
        return itemHeal;

    for (u32 i = 0; i < MAX_MON_ITEMS; i++)
    {
        aiItem = gAiLogicData->items[battler][i];
        ...
        if (itemHeal > 0)
            break; // Stop looking for items if one can already heal (Multi)
    }
...
static u32 GetSwitchinHazardsDamage(enum BattlerId battler)
{
    ...
    bool32 hasHeavyDutyBoots = Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_HEAVY_DUTY_BOOTS, gAiLogicData);

    // Check ways mon might avoid all hazards
    if (!AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGIC_GUARD) || (hasHeavyDutyBoots &&
        !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || AI_BATTLER_HAS_TRAIT(battler, ABILITY_KLUTZ))))
    {
        // Stealth Rock
        if (IsHazardOnSide(side, HAZARDS_STEALTH_ROCK) && !hasHeavyDutyBoots)
            hazardDamage += GetStealthHazardDamage(TYPE_SIDE_HAZARD_POINTED_STONES, battler);
        // G-Max Steelsurge
        if (IsHazardOnSide(side, HAZARDS_STEELSURGE) && !hasHeavyDutyBoots)
        ...
        if (IsHazardOnSide(side, HAZARDS_TOXIC_SPIKES) && (!IS_BATTLER_ANY_TYPE(battler, TYPE_POISON, TYPE_STEEL)
            && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_IMMUNITY) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_POISON_HEAL) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_COMATOSE)
            && status == 0
            && !(gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD)
            && !IsAbilityOnSide(battler, ABILITY_PASTEL_VEIL)
            && !IsMistyTerrainAffected(battler, gFieldStatuses)
            && !IsAbilityStatusProtected(battler)
            && !Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_CURE_PSN, gAiLogicData) && !Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_CURE_STATUS, gAiLogicData)
...
static s32 GetSwitchinWeatherImpact(enum BattlerId battler)
{
    ...
    // Damage
    if (!Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_SAFETY_GOGGLES, gAiLogicData) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGIC_GUARD) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_OVERCOAT))
    {
        if ((weather  & B_WEATHER_HAIL)
         && !IS_BATTLER_OF_TYPE(battler, TYPE_ICE)
         && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_SNOW_CLOAK) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_ICE_BODY))
        {
            ...
            && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_SAND_VEIL) && AI_BATTLER_HAS_TRAIT(battler, ABILITY_SAND_RUSH) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_SAND_FORCE))
        {
            weatherImpact = maxHP / 16;
            if (weatherImpact == 0)
                weatherImpact = 1;
    ...
    if ((weather  & B_WEATHER_SUN) && !Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_UTILITY_UMBRELLA, gAiLogicData)
     && (AI_BATTLER_HAS_TRAIT(battler, ABILITY_SOLAR_POWER) || AI_BATTLER_HAS_TRAIT(battler, ABILITY_DRY_SKIN)))
    {
    ...
    // Healing
    if (weather  & B_WEATHER_RAIN && !Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_UTILITY_UMBRELLA, gAiLogicData))
    {
        ...
        else if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_RAIN_DISH))
        {
            weatherImpact = -(maxHP / 16);
            if (weatherImpact == 0)
                weatherImpact = -1;
    ...
    if ((weather & (B_WEATHER_HAIL | B_WEATHER_SNOW)) && AI_BATTLER_HAS_TRAIT(battler, ABILITY_ICE_BODY))
    {
        weatherImpact = -(maxHP / 16);
        if (weatherImpact == 0)
            weatherImpact = -1;
...
static u32 GetSwitchinRecurringHealing(enum BattlerId battler)
{
    ...
    {
        if (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_BLACK_SLUDGE, gAiLogicData) && IS_BATTLER_OF_TYPE(battler, TYPE_POISON))
        {
            recurringHealing = maxHP / 16;
            if (recurringHealing == 0)
                recurringHealing = 1;
        }
        else if (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_LEFTOVERS, gAiLogicData))
        {
            recurringHealing = maxHP / 16;
            if (recurringHealing == 0)
                recurringHealing = 1;
    ...
    // Abilities
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_POISON_HEAL) && (gBattleMons[battler].status1 & STATUS1_POISON))
    {
        u32 healing = maxHP / 8;
        if (healing == 0)
            healing = 1;
...
static u32 GetSwitchinRecurringDamage(enum BattlerId battler)
{
    ...
    // Items
    if (!AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGIC_GUARD) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_KLUTZ))
    {
        if (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_BLACK_SLUDGE, gAiLogicData) && !IS_BATTLER_OF_TYPE(battler, TYPE_POISON))
        {
        ...
        else if (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_LIFE_ORB, gAiLogicData) && !AI_BATTLER_HAS_TRAIT(battler,  ABILITY_SHEER_FORCE))
        {
            passiveDamage = maxHP / 10;
            if (passiveDamage == 0)
                passiveDamage = 1;
        }
        else if (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_STICKY_BARB, gAiLogicData))
        {
            passiveDamage = maxHP / 8;
            if (passiveDamage == 0)
                passiveDamage = 1;
...
static u32 GetSwitchinStatusDamage(enum BattlerId battler)
{
    ...
    if ((status != 0) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGIC_GUARD))
    {
        if (status & STATUS1_BURN)
        {
            ...
            if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_HEATPROOF))
        ...
        else if ((status & STATUS1_POISON) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_POISON_HEAL))
        {
            statusDamage = maxHP / 8;
            if (statusDamage == 0)
                statusDamage = 1;
        }
        else if ((status & STATUS1_TOXIC_POISON) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_POISON_HEAL))
        {
...
static u32 GetSwitchinHitsToKO(s32 damageTaken, enum BattlerId battler, const struct IncomingHealInfo *healInfo, u32 originalHp)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_FOCUS_SASH, gAiLogicData) || (!opponentCanBreakMold && GetConfig(B_STURDY) >= GEN_5 && AI_BATTLER_HAS_TRAIT(battler, ABILITY_STURDY))) && hitsToKO < 1)
            currentHP = 1;
...
static bool32 AI_CanSwitchinAbilityTrapOpponent(enum Ability ability, enum BattlerId opposingBattler, u32 species)
{
    ...
    {
        if (B_SHADOW_TAG_ESCAPE >= GEN_4 && AI_BATTLER_HAS_TRAIT(opposingBattler, ABILITY_SHADOW_TAG))
            return FALSE;
        else
            return TRUE;
    }
    else if ((ability == ABILITY_ARENA_TRAP || SpeciesHasInnate(species, ABILITY_ARENA_TRAP)) && IsBattlerGrounded(opposingBattler))
        return TRUE;
    else if ((ability == ABILITY_MAGNET_PULL || SpeciesHasInnate(species, ABILITY_MAGNET_PULL)) && IS_BATTLER_OF_TYPE(opposingBattler, TYPE_STEEL))
        return TRUE;
    else
        return FALSE;
}
...
static u32 GetBestMonIntegrated(struct Pokemon *party, int lastId, enum BattlerId battler, enum BattlerId opposingBattler, enum BattlerId battlerIn1, enum BattlerId battlerIn2, enum SwitchType switchType)
{
    ...
    for (u32 monIndex = 0; monIndex < lastId; monIndex++)
    {
        ...
        if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_TRUANT) && IsTruantMonVulnerable(battler, opposingBattler))
            continue;
...
{
    enum Item aiItem;
    bool32 isStickyWebsAffected = (IsHazardOnSide(GetBattlerSide(battler), HAZARDS_STICKY_WEB) && IsBattlerAffectedByHazards(battler, FALSE) && IsBattlerGrounded(battler));
    ...
    if (SearchTraits(AIBattlerTraits, ABILITY_INTREPID_SWORD))
        gBattleMons[battler].statStages[STAT_ATK] += 1;
    if (SearchTraits(AIBattlerTraits, ABILITY_DAUNTLESS_SHIELD))
        gBattleMons[battler].statStages[STAT_DEF] += 1;
    if (SearchTraits(AIBattlerTraits, ABILITY_DOWNLOAD))
        gBattleMons[battler].statStages[GetDownloadStat(battler)] += 1;
    if (SearchTraits(AIBattlerTraits, ABILITY_INTIMIDATE))
        if (CanLowerStat(battler, opposingBattler, gAiLogicData, STAT_ATK))
        {
            if (AI_BATTLER_HAS_TRAIT(opposingBattler, ABILITY_CONTRARY))
            {
                gBattleMons[opposingBattler].statStages[STAT_ATK] += 1;
            }
            opponentStatDrop = TRUE;
            gBattleMons[opposingBattler].statStages[STAT_ATK] -= 1;
            if (AI_BATTLER_HAS_TRAIT(opposingBattler, ABILITY_DEFIANT))
                gBattleMons[opposingBattler].statStages[STAT_ATK] += 2;
            if (AI_BATTLER_HAS_TRAIT(opposingBattler, ABILITY_COMPETITIVE))
                gBattleMons[opposingBattler].statStages[STAT_SPATK] += 2;
        }
    if (SearchTraits(AIBattlerTraits, ABILITY_SUPERSWEET_SYRUP))
    ...
    if (SearchTraits(AIBattlerTraits, ABILITY_WIND_RIDER))
        if (gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_TAILWIND)
            gBattleMons[battler].statStages[STAT_ATK] += 1;
    if (SearchTraits(AIBattlerTraits, ABILITY_DEFIANT))
        if (isStickyWebsAffected)
            gBattleMons[battler].statStages[STAT_ATK] += 2;
    if (SearchTraits(AIBattlerTraits, ABILITY_COMPETITIVE))
        if (isStickyWebsAffected)
            gBattleMons[battler].statStages[STAT_SPATK] += 2;
    if (SearchTraits(AIBattlerTraits, ABILITY_CONTRARY))
        if (isStickyWebsAffected)
            gBattleMons[battler].statStages[STAT_SPEED] += 1;


    // Loop since only one consumable can be used at a time (Multi)
    for (u32 i = 0; i < MAX_MON_ITEMS; i++)
    {
        aiItem = gBattleMons[battler].items[i];
        ...
        if (aiItem != ITEM_NONE)
        {
            ...
            switch(GetItemHoldEffect(aiItem))
            {
                ...
                case HOLD_EFFECT_ATTACK_UP:
                    if (HasEnoughHpToEatBerry(battler, GetItemHoldEffectParam(aiItem), aiItem))
                    {
                        gBattleMons[battler].statStages[STAT_ATK] += 1;
                        itemUsed = TRUE;
                    }
        ...
        // Hazard stat changes
        if (isStickyWebsAffected && !BattlerHasHoldItemEffect(battler, HOLD_EFFECT_WHITE_HERB, TRUE))
            gBattleMons[battler].statStages[STAT_SPEED] -= 1;
}
...
static void SetBattlerVolatilesForSwitchin(enum BattlerId battler, u32 weather, u32 fieldStatus)
{
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_VESSEL_OF_RUIN))
        gBattleMons[battler].volatiles.vesselOfRuin = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_SWORD_OF_RUIN))
        gBattleMons[battler].volatiles.swordOfRuin = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_TABLETS_OF_RUIN))
        gBattleMons[battler].volatiles.tabletsOfRuin = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_BEADS_OF_RUIN))
        gBattleMons[battler].volatiles.beadsOfRuin = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_QUARK_DRIVE))
        if ((fieldStatus & STATUS_FIELD_ELECTRIC_TERRAIN) || Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_BOOSTER_ENERGY, gAiLogicData))
            gBattleMons[battler].volatiles.boosterEnergyActivated = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_PROTOSYNTHESIS))
        if ((weather & B_WEATHER_SUN) || Ai_BattlerHasHoldEffect(battler, HOLD_EFFECT_BOOSTER_ENERGY, gAiLogicData))
            gBattleMons[battler].volatiles.boosterEnergyActivated = TRUE;
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_WIND_POWER))
        if (gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_TAILWIND)
