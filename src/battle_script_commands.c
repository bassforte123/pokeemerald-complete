...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef)
{
    ...
    if (holdEffectCritStage > 0) //This may break some hold effects with multiple items but I doubt we plan to use Gen 1 formulas for anything
            ...
            if (stats & 1)
            {
                if (flags & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[battler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && !BattlerHasHeldItemEffect(battler, HOLD_EFFECT_CLEAR_AMULET, TRUE)
...
static bool32 TryKnockOffBattleScript(u32 battlerDef)
{
    ...
    for (i = 0; i < MAX_MON_ITEMS; i++)
    {
        if ((gBattleMons[battlerDef].items[i] != ITEM_NONE) && CanBattlerGetOrLoseItem(battlerDef, gBattleMons[battlerDef].items[i]))
        {
            if (targetableSlots[0] != MAX_MON_ITEMS)
                index++;
            targetableSlots[index] = i;
...
static inline bool32 IsProtectEffectAffected(u32 battler, u32 move)
{
    ...
    if (IsProtectivePadsProtected(battler))
        return TRUE;
...
static inline bool32 CanEjectPackTrigger(u32 battlerAtk, u32 battlerDef, enum BattleMoveEffects moveEffect)
{
    if (gProtectStructs[battlerDef].statFell
     && GetBattlerHeldItemWithEffect(battlerDef, HOLD_EFFECT_EJECT_PACK, TRUE)
     && IsBattlerAlive(battlerDef)
     && CountUsablePartyMons(battlerDef) > 0
...
static bool32 HandleMoveEndAbilityBlock(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case ABILITY_MAGICIAN:
        ...
        if (targetableSlots[0] != MAX_MON_ITEMS)
        {
            ...
            //&& gBattleMons[battlerAtk].item == ITEM_NONE // In loop
            //&& gBattleMons[battlerDef].item != ITEM_NONE // In loop
            && IsBattlerAlive(battlerAtk)
            && IsBattlerTurnDamaged(battlerDef)
            //&& CanStealItem(battlerAtk, battlerDef, gBattleMons[battlerDef].item) //In loop
    ...
    do
    {
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
            {
                ...
                if (gHitMarker & HITMARKER_OBEYS
                 ...
                 && (BATTLER_IS_HOLDING_CHOICE_ITEM_WITH_NEGATION(gBattlerAttacker) || GetBattlerAbility(gBattlerAttacker) == ABILITY_GORILLA_TACTICS))
                {
        ...
        case MOVEEND_EJECT_BUTTON:
            {
                ...
                for (i = 0; i < gBattlersCount; i++)
                {
                    ...
                    gBattleScripting.battler = battler;
                    gLastUsedItem = BattlerHasHeldItemEffect(battler, HOLD_EFFECT_EJECT_BUTTON, TRUE);
                    effect = TRUE;
        ...
        case MOVEEND_WHITE_HERB:
            for (i = 0; i < gBattlersCount; i++)
            {
                ...
                if (GetBattlerHeldItemWithEffect(i, HOLD_EFFECT_WHITE_HERB, TRUE)
        ...
        case MOVEEND_PICKPOCKET:
            u8 slot, i, j, targetableSlots[MAX_MON_ITEMS];
            u8 index;
...
static UNUSED void Cmd_removeitem(void)
{
    ...
    gBattleMons[battler].item = gBattleMons[battler].items[0]= ITEM_NONE;
...
static void Cmd_various(void)
{
    ...
    struct Pokemon *mon;
    s32 i;
    u8 data[10], index, slot, targetableSlots[MAX_MON_ITEMS];
    u32 battler, bits;
    ...
    case VARIOUS_SET_SIMPLE_BEAM:
    {
        ...
        else if (BattlerHasHeldItemEffect(gBattlerTarget, HOLD_EFFECT_ABILITY_SHIELD, TRUE))
        {
    ...
    case VARIOUS_CONSUME_BERRY:
    {
        ...
        if (cmd->fromBattler && gLastItemSlot != MAX_MON_ITEMS 
            && GetPocketByItemId(gBattleMons[battler].items[gLastItemSlot]) == POCKET_BERRIES)
            {
                gLastUsedItem = gBattleMons[battler].items[gLastItemSlot];
            }
        else
        {
            if (GetPocketByItemId(gBattleMons[battler].items[gLastItemSlot]) != POCKET_BERRIES 
    ...
    {
        VARIOUS_ARGS();
        gBattleMons[battler].items[gLastItemSlot] = gLastUsedItem;
        break;
    }
    case VARIOUS_LAST_USED_ITEM_TO_BATTLER_BERRY: //For Stuff Cheeks
    {
        VARIOUS_ARGS();
        index = 0;
        targetableSlots[0] = MAX_MON_ITEMS; // Invalid value for first slot if no valid slots found
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if(GetPocketByItemId(gBattleMons[battler].items[i]) == POCKET_BERRIES)
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
...
static void Cmd_tryrecycleitem(void)
{
    ...
    else if (cmd->type == RECYCLE_ITEM_HARVEST)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetPocketByItemId(GetUsedHeldItem(gBattlerAttacker, i)) == POCKET_BERRIES)
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
        ...
        if (B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9)
        {
            ...
            for (int i = 0; i < MAX_MON_ITEMS; i++)
            {
                ...
                if (lostItem != ITEM_NONE && GetItemPocket(lostItem) != POCKET_BERRIES)
                    SetMonData(GetBattlerMon(GetCatchingBattler()), MON_DATA_HELD_ITEM + i, &lostItem);  // Restore non-berry items
            }
        }
                    ...
                    switch (evolutionCondition)
                    {
                        ...
                        case IF_DEFEAT_X_WITH_ITEMS:
                            for (k = 0; k < MAX_MON_ITEMS; k++)
                            {
                                ...
                                && GetMonData(GetBattlerMon(gBattlerTarget), MON_DATA_HELD_ITEM + k) == evolutions[i].params[j].arg2)
                                    SetMonData(GetBattlerMon(gBattlerAttacker), MON_DATA_EVOLUTION_TRACKER, &val);
                                break;
                            }
                            break;
...
void BS_TeatimeInvul(void)
{
    ...
    u32 battler = GetBattlerForBattleScript(cmd->battler);
    if (BattlerHasBerry(battler) && !(gStatuses3[gBattlerTarget] & (STATUS3_SEMI_INVULNERABLE)))
        gBattlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = cmd->jumpInstr;
}
