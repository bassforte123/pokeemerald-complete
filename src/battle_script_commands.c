...
bool32 ProteanTryChangeType(enum BattlerId battler, enum Move move, enum Type moveType)
{
     ...
     && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_FOE_ENDURED))
    {
        ...
        else if (gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_FOE_HUNG_ON)
        {
            if (IsBattlerAtMaxHp(gBattlerTarget))
                gLastUsedItem = GetBattlerHoldItemWithEffect(gBattlerTarget, HOLD_EFFECT_FOCUS_SASH, TRUE);
            if (gLastUsedItem == ITEM_NONE)
                gLastUsedItem = GetBattlerHoldItemWithEffect(gBattlerTarget, HOLD_EFFECT_FOCUS_BAND, TRUE);
...
static inline bool32 DoesBattlerNegateDamage(enum BattlerId battler)
{
    ...
    if (gBattleMons[battler].volatiles.transformed)
        return FALSE;
    if (BattlerHasTrait(battler, ABILITY_DISGUISE) && IsMimikyuDisguised(battler))
        return TRUE;
...
static inline bool32 TryActivateWeaknessBerry(enum BattlerId battlerDef)
{
    ...
    if (gSpecialStatuses[battlerDef].berryReducedType != 0)
    {
        for (int i = 0; i < MAX_MON_ITEMS; i++)
        {
            ...
            {
                itemDef = GetSlotHoldItem(battlerDef, i, TRUE);
                break;
            }
        }
...
static inline bool32 TrySetReflect(enum BattlerId battler)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_REFLECT))
    {
        ...
        {
            if (GetSlotHoldItemEffect(battler, i, TRUE) == HOLD_EFFECT_LIGHT_CLAY
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    u32 index, slot, targetableSlots[MAX_MON_ITEMS];
    ...
    if (!primary && !affectsUser && IsMoveEffectBlockedByTarget())
        moveEffect = MOVE_EFFECT_NONE;
          ...
          && IsSheerForceAffected(gCurrentMove, battlerAtk)
    ...
    case MOVE_EFFECT_TOXIC:
    case MOVE_EFFECT_FROSTBITE:
        if (IsSafeguardProtected(battlerAtk, effectBattler) && !primary)
        {
            gBattlescriptCurrInstr = battleScript;
        }
    ...
    case MOVE_EFFECT_FLAME_BURST:
        if (IsBattlerAlive(BATTLE_PARTNER(effectBattler))
         ...
         && !BattlerHasTrait(BATTLE_PARTNER(effectBattler), ABILITY_MAGIC_GUARD))
        {
    ...
    case MOVE_EFFECT_INCINERATE:
        ...
        {
            if (((gItemsInfo[gBattleMons[effectBattler].items[i]].pocket == POCKET_BERRIES) //Gather all valid item slots
             || (B_INCINERATE_GEMS >= GEN_6 && BattlerHasHoldItemEffect(effectBattler, HOLD_EFFECT_GEMS, FALSE)))
             && !BattlerHasTrait(effectBattler, ABILITY_STICKY_HOLD))
        {
        ...
        if (targetableSlots[0] != MAX_MON_ITEMS)
        {
            ...
            gLastUsedItem = gBattleMons[effectBattler].items[slot];
            gBattleMons[effectBattler].items[slot] = ITEM_NONE;
            CheckSetUnburden(effectBattler);

            BtlController_EmitSetMonData(effectBattler, B_COMM_TO_CONTROLLER, REQUEST_HELDITEM_BATTLE + slot, 0, sizeof(gBattleMons[effectBattler].items[slot]), &gBattleMons[effectBattler].items[slot]);
            MarkBattlerForControllerExec(effectBattler);
            BattleScriptPush(battleScript);
    ...
    case MOVE_EFFECT_BUG_BITE:
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetItemPocket(gBattleMons[effectBattler].items[i]) == POCKET_BERRIES //Gather all berry item slots
            && GetItemHoldEffect(gBattleMons[effectBattler].items[i]) != HOLD_EFFECT_JABOCA_BERRY // jaboca berry triggers instead of being stolen
            && !BattlerHasTrait(effectBattler, ABILITY_STICKY_HOLD))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
    ...
    case MOVE_EFFECT_FLING:
        if (CanFling(battlerAtk) || gBattleStruct->flungItem == FLUNG_ITEM_REMOVED)
        {
            ...
            case HOLD_EFFECT_WHITE_HERB:
                ItemBattleEffects(effectBattler, 0, IsOnFlingActivation);
                break;
            default:
                break;
            }
    ...
    case MOVE_EFFECT_HAIL:
    {
        ...
        if (TryChangeBattleWeather(battlerAtk, weather, FALSE))
        {
    ...
    case MOVE_EFFECT_AURORA_VEIL:
        if (!(gSideStatuses[GetBattlerSide(effectBattler)] & SIDE_STATUS_AURORA_VEIL))
        {
            ...
            for (i = 0; i < MAX_MON_ITEMS; i++)
            {
                if (GetSlotHoldItemEffect(effectBattler, i, TRUE) == HOLD_EFFECT_LIGHT_CLAY
                 && (gSideTimers[GetBattlerSide(effectBattler)].auroraVeilTimer == 5 || GetConfig(B_ALLOW_HELD_DUPES)))
                    gSideTimers[GetBattlerSide(effectBattler)].auroraVeilTimer += 3;
            }
    ...
    case MOVE_EFFECT_YAWN_FOE:
        if (gBattleMons[effectBattler].volatiles.yawn == 0
         && CanBeSlept(battlerAtk, effectBattler, BLOCKED_BY_SLEEP_CLAUSE)
...
static void Cmd_setpreattackadditionaleffect(void)
{
    ...
    while (gEffectBattler != MAX_BATTLERS_COUNT)
    {
        ...
        if (numAdditionalEffects > gBattleStruct->additionalEffectsCounter)
        {
            ...
            u32 percentChance = CalcSecondaryEffectChance(gBattlerAttacker, additionalEffect);
...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                ...
                if (MonHasItemHoldEffect(&gParties[B_TRAINER_PLAYER][i], HOLD_EFFECT_EXP_SHARE) || IsGen6ExpShareEnabled())
                {
                    expShareBits |= 1u << i;
                    viaExpShare++;
                }
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if ((!MonHasItemHoldEffect(&gParties[B_TRAINER_PLAYER][*expMonId], HOLD_EFFECT_EXP_SHARE) && !wasSentOut && !IsGen6ExpShareEnabled())
            ...
            else
            {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    if ((MonHasItemHoldEffect(&gParties[B_TRAINER_PLAYER][*expMonId], HOLD_EFFECT_EXP_SHARE) || IsGen6ExpShareEnabled())
                    ...
                    if (wasSentOut || MonHasItemHoldEffect(&gParties[B_TRAINER_PLAYER][*expMonId], HOLD_EFFECT_EXP_SHARE))
                    {
...
static void Cmd_tryselfconfusiondmgformchange(void)
{
    ...
    if (TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_HIT_BY_CONFUSION_SELF_DMG))
    {
        ...
        if (IsAbilityAndRecord(gBattlerAttacker, ABILITY_DISGUISE))
        {
...
static inline bool32 IsProtectivePadsProtected(enum BattlerId battler)
{
    if (!BattlerHasHoldItemEffect(battler, HOLD_EFFECT_PROTECTIVE_PADS, TRUE))
        return FALSE;
...
static void Cmd_jumpifcantswitch(void)
{
    ...
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (!cmd->ignoreEscapePrevention && !CanBattlerEscape(battler) && !BattlerHasHoldItemEffect(battler, HOLD_EFFECT_SHED_SHELL, TRUE))
    {
        gBattlescriptCurrInstr = cmd->jumpInstr;
    }
    else
...
static void Cmd_returntoball(void)
{
    ...
    if (!cmd->changingForm)
        TryBattleFormChange(battler, FORM_CHANGE_BATTLE_SWITCH_OUT);
...
static void Cmd_removeitemwitheffect(void)
{
    ...
    {
        itemId = GetBattlerHoldItemWithEffect(battler, holdEffect, TRUE);
    }

    slot = GetHoldItemSlot(battler, itemId, TRUE);
...
static void Cmd_setfieldweather(void)
{
    ...
    if (TryChangeBattleWeather(gBattlerAttacker, GetMoveWeatherType(gCurrentMove), FALSE))
    {
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
...
static void Cmd_trysetrest(void)
{
    ...
    if (IsElectricTerrainAffected(gBattlerTarget, gFieldStatuses))
    {
        gBattlescriptCurrInstr = BattleScript_ElectricTerrainPrevents;
    }
    else if (IsMistyTerrainAffected(gBattlerTarget, gFieldStatuses))
    {
...
{
    enum BattlerId i;
    bool32 hasSoundproof = (GetConfig(B_UPROAR_IGNORE_SOUNDPROOF) < GEN_5 && BattlerHasTrait(battler, ABILITY_SOUNDPROOF));
...
static void Cmd_tryactivateitem(void)
{
    ...
    case ACTIVATION_ON_PICK_UP:
        ItemBattleEffects(battler, 0, IsForceTriggerItemActivation);
        break;
    case ACTIVATION_ON_HARVEST:
        ItemBattleEffects(battler, 0, IsOnBerryActivation);
        break;
    case ACTIVATION_ON_HP_THRESHOLD:
        ItemBattleEffects(battler, 0, IsOnHpThresholdActivation);
        break;
    case ACTIVATION_ON_STATUS_CHANGE:
        ItemBattleEffects(battler, 0, IsOnStatusChangeActivation);
        break;
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        else if (GetConfig(B_TIME_OF_DAY_HEALING_MOVES) != GEN_2)
        {
            if (attackerWeather & B_WEATHER_SUN)
                recoverAmount = 20 * GetNonDynamaxMaxHP(gBattlerAttacker) / 30;
            else if (!(healingWeather & B_WEATHER_ANY) || BattlerHasHoldItemEffect(gBattlerAttacker, HOLD_EFFECT_UTILITY_UMBRELLA, TRUE))
                recoverAmount = GetNonDynamaxMaxHP(gBattlerAttacker) / 2;
...
static void Cmd_settaunt(void)
{
    ...
    if (GetConfig(B_OBLIVIOUS_TAUNT) >= GEN_6 && BattlerHasTrait(gBattlerTarget, ABILITY_OBLIVIOUS))
    {
        gBattlescriptCurrInstr = BattleScript_NotAffectedAbilityPopUp;
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_DOESNT_AFFECT_FOE;
        gBattlerAbility = gBattlerTarget;
    ...
    else
    {
        ...
        for (int i = 0; i < MAX_MON_ITEMS; i++)
        {
            ...
            // or if either of them is an enigma berry, a mail, or other invalid item
                 ...
                 || !CanBattlerGetOrLoseItem(gBattlerAttacker, gBattlerTarget, gBattleMons[gBattlerAttacker].items[i])
                 || !CanBattlerGetOrLoseItem(gBattlerAttacker, gBattlerTarget, gBattleMons[gBattlerTarget].items[i])
                 || !CanBattlerGetOrLoseItem(gBattlerTarget, gBattlerAttacker, gBattleMons[gBattlerTarget].items[i])
                 || !CanBattlerGetOrLoseItem(gBattlerTarget, gBattlerAttacker, gBattleMons[gBattlerAttacker].items[i]))
            {
                continue;
            }
        ...
        {
            u32 slot;
            u16 oldItemAtk = 0, oldItemDef = 0;

            slot = gLastItemSlot = GetSlot(targetableSlots, index);

            oldItemAtk = gBattleMons[gBattlerAttacker].items[slot];
            oldItemDef = gBattleMons[gBattlerTarget].items[slot];

            gBattleMons[gBattlerAttacker].items[slot] = oldItemDef;
            ...
            BtlController_EmitSetMonData(gBattlerAttacker, B_COMM_TO_CONTROLLER, REQUEST_HELDITEM_BATTLE + slot, 0, sizeof(gBattleMons[gBattlerAttacker].items[slot]), &gBattleMons[gBattlerAttacker].items[slot]); // set attacker item
            MarkBattlerForControllerExec(gBattlerAttacker);
            ...
            if (!BattlerHasTrait(gBattlerAttacker, ABILITY_GORILLA_TACTICS)
             && (!BattlerHasHoldEffectChoice(gBattlerTarget)
            ...
            if (!(IsBattlerAlly(gBattlerAttacker, gBattlerTarget) && IsPartnerMonFromSameTrainer(gBattlerAttacker)))
            {
                ...
                if (IsOnPlayerSide(gBattlerTarget))
                    TrySaveExchangedItem(gBattlerTarget, oldItemDef, slot);
            }
...
static bool32 CanAbilityShieldActivateForBattler(enum BattlerId battler)
{
    ...
    gBattlerAbility = battler;
    gLastUsedItem = GetBattlerHoldItemWithEffect(battler, HOLD_EFFECT_ABILITY_SHIELD, FALSE);
    return TRUE;
}
...
static void Cmd_setgastroacid(void)
{
    ...
    else if (BattlerHasHoldItemEffectIgnoreAbility(gBattlerTarget, HOLD_EFFECT_ABILITY_SHIELD, TRUE))
    {
...
static void Cmd_setyawn(void)
{
    ...
    else if (IsElectricTerrainAffected(gBattlerTarget, gFieldStatuses))
    {
...
static void Cmd_pickup(void)
{
    ...
    enum Item storeItem = ITEM_NONE, heldItem = ITEM_NONE;
    u8 lvlDivBy10;
    u8 slot = MAX_MON_ITEMS;
    ...
    if (!InBattlePike()) // No items in Battle Pike.
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if (MonHasTrait(&gParties[B_TRAINER_PLAYER][i], ABILITY_PICKUP)
                ...
                else
                {
                    ...
                    for (j = 0; j < ARRAY_COUNT(sPickupTable); j++)
                    {
                        ...
                        if (rand > percentTotal)
                        {
                            ...
                            slot = GetMonNextEmptySlot(&gParties[B_TRAINER_PLAYER][i], storeItem);
                            if (slot != MAX_MON_ITEMS)
                            {
                                heldItem = storeItem; // Gives the rarest chosen item that fits slot categorization
                            }
                        }
                    }
                    if (slot != MAX_MON_ITEMS)
                ...
                && (Random() % 1) == 0)
            {
                ...
                for (k = 0; k < MAX_MON_ITEMS; k++)
                {
                    if (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_HELD_ITEM + k) == ITEM_ORAN_BERRY)
                    {
                        ...
                        SetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_HELD_ITEM + k, &heldItem);
                        break;
                    }
                }
            }
...
bool32 IsSubstituteProtected(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move)
{
...
{
    return IsSubstituteProtected(battlerAtk, battlerDef, move);
}
...
bool32 DoesIceFaceBlockMove(enum BattlerId battler, enum Move move)
{
    if (gBattleMons[battler].species != SPECIES_EISCUE_ICE
     ...
     || !IsAbilityAndRecord(battler, ABILITY_ICE_FACE))
        return FALSE;
    else
        PushTraitStack(battler, ABILITY_DISGUISE);
        return TRUE;
...
static void ComputeBallData(u32 wildMonBattler, u32 playerBattler, struct BallData *ball)
{
    ...
    case BALL_HEAVY:
        ...
        break;
    case BALL_DREAM:
        if (B_DREAM_BALL_MODIFIER >= GEN_8 && (battleMon->status1 & STATUS1_SLEEP || (BattlerHasTrait(wildMonBattler, ABILITY_COMATOSE))))
            ball->multiplier = 400;
        break;
    case BALL_SAFARI:
...
{
    return GetConfig(B_HIT_THAW) >= GEN_6 && !IsSheerForceAffected(move, battlerAtk) && MoveThawsUser(move);
}
...
void BS_JumpIfCantLoseItem(void)
{
    ...
    for (i = 0; i < MAX_MON_ITEMS; i++)
    {
        item = gBattleMons[gBattlerTarget].items[i];
...
void BS_TrySymbiosis(void)
{
    ...
    if (TryTriggerSymbiosis(battler, partner))
    {
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            ...
            && CanBattlerGetOrLoseItem(battler, partner, gBattleMons[BATTLE_PARTNER(battler)].items[i])
            && CanBattlerGetOrLoseItem(partner, battler, gBattleMons[BATTLE_PARTNER(battler)].items[i]))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    for (i = 0; i < MAX_MON_ITEMS; i++)
    {
        if (GetItemHoldEffect(GetMonData(&gParties[B_TRAINER_PLAYER][expGetterMonId], MON_DATA_HELD_ITEM + i)) == HOLD_EFFECT_LUCKY_EGG
...
void BS_JumpIfTerrainAffected(void)
{
    ...
    if (IsBattlerTerrainAffected(battler, gFieldStatuses, cmd->flags))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
void BS_TryBoosterEnergy(void)
{
    ...
    for (u32 orderNum = 0; orderNum < gBattlersCount; orderNum++)
    {
        ...
        if (!BattlerHasHoldItemEffect(battler, HOLD_EFFECT_BOOSTER_ENERGY, TRUE))
            continue;
...
void BS_JumpIfAbilityCantBeReactivated(void)
{
    NATIVE_ARGS(u8 battler, const u8 *jumpInstr);
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    enum Ability ability = gBattleMons[battler].ability; //Specific to Main Ability negation (Multi)
...
void BS_TryActivateAbilityShield(void)
{
    ...
    {
        gLastUsedItem = GetBattlerHoldItemWithEffect(battler, HOLD_EFFECT_ABILITY_SHIELD, TRUE);
...
void BS_JumpIfHoldEffect(void)
{
    ...
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (BattlerHasHoldItemEffect(battler, cmd->holdEffect, TRUE) == cmd->equal)
    {
        if (cmd->equal)
            gLastUsedItem = GetBattlerHoldItemWithEffect(battler, cmd->holdEffect, TRUE); // For B_LAST_USED_ITEM
        gBattlescriptCurrInstr = cmd->jumpInstr;
    }
    else
    {
...
void BS_SetLastUsedItem(void)
{
    ...
    {
        slot = GetHoldItemSlot(battler, gLastUsedItem, TRUE);

        if (slot != MAX_MON_ITEMS) // If no effect specified, use lastuseditem if battler has it
            gLastUsedItem = GetSlotHoldItem(battler, slot, TRUE);
        else // If no effect specified and battler does not have lastuseditem, select an item from the battler
        {
            ...
            if (targetableSlots[0] != MAX_MON_ITEMS)
            {
                ...
                gLastUsedItem = GetSlotHoldItem(battler, slot, TRUE); // For B_LAST_USED_ITEM
    ...
    {
        slot = GetBattlerHoldItemSlotWithEffect(battler, holdEffect, TRUE);
...
{
    NATIVE_ARGS(u8 battler, u8 opposingBattler, u8 type);
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    enum BattlerId opposingBattler = GetBattlerForBattleScript(cmd->opposingBattler);
    u8 type = cmd->type;
    u32 i;
    u32 targetableSlots[MAX_MON_ITEMS];
    u32 index = 0;
    ...
    if (type == LAST_ITEM_FLING)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (gBattleMons[battler].items[i] != ITEM_NONE && GetItemPocket(gBattleMons[battler].items[i]) != POCKET_BERRIES && CanBattlerGetOrLoseItem(battler, opposingBattler, gBattleMons[battler].items[i]))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
    ...
    if (targetableSlots[0] == MAX_MON_ITEMS || type == LAST_ITEM_CORROSIVE)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (gBattleMons[battler].items[i] != ITEM_NONE && CanBattlerGetOrLoseItem(battler, opposingBattler, gBattleMons[battler].items[i]))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
    ...
    while (gBattleScripting.battler < gBattlersCount)
    {
        ...
        {       
            for (int i = 0; i < MAX_MON_ITEMS; i++)
            {
                ...
                {
                    friskcount++;
                    friskedItem = gBattleMons[battler].items[i];
                    if (friskcount == 1)
                    {
            ...
            if (friskcount > 0)
            {
                ...
                RecordItemEffectBattle(battler, GetItemHoldEffect(gLastUsedItem)); //GetItemHoldEffect ignores negation so negation doesn't need to be added
...
void BS_SwitchinAbilities(void)
{
    ...
    AbilityBattleEffects(ABILITYEFFECT_DEPENDS_ON_ALLY, battler, MOVE_NONE, TRUE);
    AbilityBattleEffects(ABILITYEFFECT_ON_WEATHER, battler, MOVE_NONE, TRUE);
    AbilityBattleEffects(ABILITYEFFECT_ON_TERRAIN, battler, MOVE_NONE, TRUE);
...
void BS_TryActivateReceiver(void)
{
    ...
    if (IsBattlerAlive(receiverBattler)
     && (receiverAbility == ABILITY_RECEIVER || receiverAbility == ABILITY_POWER_OF_ALCHEMY) // Should not be Innates (Multi)
     && !BattlerHasHoldItemEffectIgnoreAbility(receiverBattler, HOLD_EFFECT_ABILITY_SHIELD, TRUE)
...
void BS_TryActivateSoulheart(void)
{
    ...
    while (gBattleStruct->soulheartBattlerId < gBattlersCount)
    {
        ...
        if (BattlerHasTrait(gBattlerAbility, ABILITY_SOUL_HEART)
            && IsBattlerAlive(gBattlerAbility)
            && !NoAliveMonsForEitherParty()
            && CompareStat(gBattlerAbility, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN))
        {
            SetStatChange(gBattlerAbility, STAT_SPATK, 1);
            PushTraitStack(gBattlerAbility, ABILITY_SOUL_HEART);
...
void BS_TryEntrainment(void)
{
    ...
    else
    {
        ...
        {
            RemoveAbilityFlags(gBattlerTarget);
            gBattleMons[gBattlerTarget].ability = gBattleMons[gBattlerTarget].volatiles.overwrittenAbility = gDisplayAbility = gBattleMons[gBattlerAttacker].ability;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
    }
}
...
void BS_TryPsychoShift(void)
{
    ...
    if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_POISON)
     && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_TOXIC, CHECK_TRIGGER))
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = 0;
          ...
          && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_TOXIC, CHECK_TRIGGER))
    {
        ...
        synchronizeEffect = MOVE_EFFECT_TOXIC;
          ...
          && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_BURN, CHECK_TRIGGER))
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = 2;
        synchronizeEffect = MOVE_EFFECT_BURN;
    }
    else if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_PARALYSIS)
          && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_PARALYSIS, CHECK_TRIGGER))
    {
        ...
        synchronizeEffect = MOVE_EFFECT_PARALYSIS;
          ...
          && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_SLEEP, CHECK_TRIGGER))
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = 4;
    }
    else if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_FROSTBITE)
          && CanSetNonVolatileStatus(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_FREEZE_OR_FROSTBITE, CHECK_TRIGGER))
    {
...
void BS_TryBestow(void)
{
    ...
    u32 slot = gLastItemSlot = GetSlot(targetableSlots, index);    
...
void BS_SetAuroraVeil(void)
{
    ...
    else
    {
        ...
        for (u32 i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetSlotHoldItemEffect(gBattlerAttacker, i, TRUE) == HOLD_EFFECT_LIGHT_CLAY
...
void BS_TryRoomService(void)
{
    NATIVE_ARGS(u8 battler, const u8 *failInstr);
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (BattlerHasHoldItemEffect(battler, HOLD_EFFECT_ROOM_SERVICE, TRUE) && ItemBattleEffects(battler, 0, IsOnEffectActivation))
        return;
...
void BS_TryTerrainSeed(void)
{
    NATIVE_ARGS(u8 battler, const u8 *failInstr);
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (BattlerHasHoldItemEffect(battler, HOLD_EFFECT_TERRAIN_SEED, TRUE) && ItemBattleEffects(battler, 0, IsOnEffectActivation))
        return;
...
void BS_JumpIfAbilityPreventsRest(void)
{
    NATIVE_ARGS(u8 battler, const u8 *jumpInstr);
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (GetConfig(B_LEAF_GUARD_PREVENTS_REST) >= GEN_5 && IsLeafGuardProtected(battler))
        gBattlescriptCurrInstr = cmd->jumpInstr;
...
{
    NATIVE_ARGS();
    u8 itemslot = GetHoldItemSlot(gBattlerTarget, gLastUsedItem, TRUE);
...
void BS_TryActivateAbilityWithAbilityShield(void)
{
    ...
    {
        bool32 attackerHasAbilityShield = BattlerHasHoldItemEffectIgnoreAbility(gBattlerAttacker, HOLD_EFFECT_ABILITY_SHIELD, TRUE);
        bool32 targetHasAbilityShield = BattlerHasHoldItemEffectIgnoreAbility(gBattlerTarget, HOLD_EFFECT_ABILITY_SHIELD, TRUE);
...
void BS_TryDefiantRattled(void)
{
    ...
    bool32 defiantCompetitive = FALSE;
    enum BattlerId battler = gBattleScripting.battler;
    enum Ability battlerTraits[MAX_MON_TRAITS];
...
void BS_TryAdrenalineOrb(void)
{
    ...
    if (gBattleStruct->intimidateActivated
     ...
     && BattlerHasHoldItemEffect(battler, HOLD_EFFECT_ADRENALINE_ORB, TRUE))
    {
