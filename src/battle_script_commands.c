...
static void Cmd_attackcanceler(void)
{
    ...
    {
        if (!CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), gCurrentMove))
    ...
    else if (gProtectStructs[gBattlerTarget].beakBlastCharge && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), gCurrentMove))
    {
        gProtectStructs[gBattlerAttacker].touchedProtectLike = TRUE;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        && (abilityAtk == ABILITY_SKILL_LINK || BattlerHasHeldItemEffect(gBattlerAttacker, HOLD_EFFECT_LOADED_DICE, TRUE)
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef)
{
    s32 critChance = 0;
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef)
{
    s32 critChance = 0;
...
static void Cmd_critcalc(void)
{
    ...
    for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (GetGenConfig(GEN_CONFIG_CRIT_CHANCE) == GEN_1)
            gBattleStruct->critChance[battlerDef] = CalcCritChanceStageGen1(gBattlerAttacker, battlerDef, gCurrentMove, TRUE, abilityAtk, abilityDef);
        else
            gBattleStruct->critChance[battlerDef] = CalcCritChanceStage(gBattlerAttacker, battlerDef, gCurrentMove, TRUE, abilityAtk, abilityDef);
...
{
    gLastUsedItem = gBattleMons[battlerItem].items[slot];
    gBattleMons[battlerItem].items[slot] = ITEM_NONE;
    ...
    BtlController_EmitSetMonData(battlerItem, B_COMM_TO_CONTROLLER, REQUEST_HELDITEM_BATTLE + slot, 0, sizeof(gBattleMons[gBattlerTarget].items[slot]), &gBattleMons[battlerItem].items[slot]);  // remove target item
    MarkBattlerForControllerExec(battlerItem);

    gBattleStruct->choicedMove[battlerItem] = 0;

    TrySaveExchangedItem(battlerItem, gLastUsedItem, slot);
}
...
void SetMoveEffect(u32 battler, u32 effectBattler, bool32 primary, bool32 certain)
{
    ...
    u8 i, index, slot, targetableSlots[MAX_MON_ITEMS];
...
{
    if (BattlerHasHeldItemEffect(battler, HOLD_EFFECT_SAFETY_GOGGLES, TRUE))
        return FALSE;
...
static bool32 HandleMoveEndAbilityBlock(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case ABILITY_MAGICIAN:
        ...
        if (targetableSlots[0] != MAX_MON_ITEMS)
        {
            ...
            {
                StealTargetItem(battlerAtk, battlerDef, slot);
...
static bool32 HandleMoveEndMoveBlock(u32 moveEffect)
{
    ...
    case EFFECT_KNOCK_OFF:
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if ((gBattleMons[gBattlerTarget].items[i] != ITEM_NONE) && CanBattlerGetOrLoseItem(gBattlerTarget, gBattleMons[gBattlerTarget].items[i]))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
        ...
        DebugPrintf("Knock Off: %d", gBattleMons[gBattlerTarget].items[0]);
        DebugPrintf("Knock Off: %d", gBattleMons[gBattlerTarget].items[1]);

        slot = gLastItemSlot = GetSlot(targetableSlots, index);    

        if (slot != MAX_MON_ITEMS && !NoAliveMonsForEitherParty())
        {
            if (GetBattlerAbility(gBattlerTarget) == ABILITY_STICKY_HOLD && IsBattlerAlive(gBattlerTarget))
            {
                gBattlerAbility = gBattlerTarget;
            ...
            else
            {
                ...
                DebugPrintf("Knocked off item %d from %d\n", gLastUsedItem, gBattlerTarget);
                BattleScriptCall(BattleScript_KnockedOff);
            }
        }
        DebugPrintf("Knock Off2: %d", gBattleMons[gBattlerTarget].items[0]);
        DebugPrintf("Knock Off2: %d", gBattleMons[gBattlerTarget].items[1]);
        break;
    ...
    do
    {
        ...
        case MOVEEND_WHITE_HERB:
            for (i = 0; i < gBattlersCount; i++)
            {
                ...
                if (BattlerHasHeldItemEffect(i, HOLD_EFFECT_WHITE_HERB, TRUE)
            ...
            case EFFECT_NATURAL_GIFT:
                ...
                if (!(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE))
                {
                    ...
                    if (item != ITEM_NONE)
                    {
                        ...
                        gBattleStruct->usedHeldItems[gBattlerPartyIndexes[gBattlerAttacker]][GetBattlerSide(gBattlerAttacker)][i] = item;
...
static UNUSED void Cmd_removeitem(void)
{
    CMD_ARGS(u8 battler);
    ...
    battler = GetBattlerForBattleScript(cmd->battler);
    itemId = gBattleMons[battler].items[0];
    ...
    if (!BattlerHasHeldItemEffect(battler, HOLD_EFFECT_AIR_BALLOON, TRUE)
     && GetMoveEffect(gCurrentMove) != EFFECT_CORROSIVE_GAS)
        gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)][0] = itemId; // Remember if switched out

    gBattleMons[battler].items[0]= ITEM_NONE;
    gBattleStruct->battlerState[battler].canPickupItem = TRUE;
    CheckSetUnburden(battler);

    BtlController_EmitSetMonData(battler, B_COMM_TO_CONTROLLER, REQUEST_HELDITEM_BATTLE, 0, sizeof(gBattleMons[battler].items[0]), &gBattleMons[battler].items[0]);
    MarkBattlerForControllerExec(battler);
...
static u32 ChangeStatBuffs(u32 battler, s8 statValue, u32 statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((BattlerHasHeldItemEffect(battler, HOLD_EFFECT_CLEAR_AMULET, TRUE) || CanAbilityPreventStatLoss(battlerAbility))
...
static void Cmd_tryrecycleitem(void)
{
    ...
    if (cmd->type == RECYCLE_ITEM_RECYCLE)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetUsedHeldItem(gBattlerAttacker, i) != ITEM_NONE)
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
    ...
    else if (cmd->type == RECYCLE_ITEM_PICKUP)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetUsedHeldItem(gBattlerTarget, i) != ITEM_NONE)
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
    ...
    else if (cmd->type == RECYCLE_ITEM_HARVEST)
    {
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (GetItemPocket(GetUsedHeldItem(gBattlerAttacker, i)) == POCKET_BERRIES)
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
    ...
    if (slot != MAX_MON_ITEMS)
    {
        if (gCurrentMove == MOVE_NONE && GetBattlerAbility(gBattlerAttacker) == ABILITY_PICKUP)
            ...
            usedHeldItem = &gBattleStruct->usedHeldItems[gBattlerPartyIndexes[gBattlerAttacker]][GetBattlerSide(gBattlerAttacker)][slot];
...
void BS_SetTerrain(void)
{
    ...
    gFieldStatuses &= ~STATUS_FIELD_TERRAIN_ANY;
    gFieldStatuses |= statusFlag;
    gFieldTimers.terrainTimer = gBattleTurnCounter + BattlerHasHeldItemEffect(gBattlerAttacker, HOLD_EFFECT_TERRAIN_EXTENDER, TRUE) ? 8 : 5;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
void BS_TryIntimidateEjectPack(void)
{
    ...
    if (affectedBattler != 0xFF)
    {
        ...
        gProtectStructs[partnerBattler].tryEjectPack = FALSE;
        gAiLogicData->ejectPackSwitch = TRUE;
...
void BS_TryRecycleBerry(void)
{
    ...
    for (i = 0; i < MAX_MON_ITEMS; i++)
    {
        usedHeldItem = &gBattleStruct->usedHeldItems[gBattlerPartyIndexes[gBattlerTarget]][GetBattlerSide(gBattlerTarget)][i];
...
void BS_TryBoosterEnergy(void)
{
    ...
    for (u32 orderNum = 0; orderNum < gBattlersCount; orderNum++)
    {
        ...
        if (!BattlerHasHeldItemEffect(battlerByTurnOrder, HOLD_EFFECT_BOOSTER_ENERGY, TRUE))
            continue;
    ...
    while (gBattleStruct->friskedBattler < gBattlersCount)
    {
            ...
            && IsBattlerAlive(gBattlerTarget))
        {
            ...
            if (friskcount > 0)
            {
                ...
                RecordItemEffectBattle(gBattlerTarget, GetItemHoldEffect(gLastUsedItem));
                BattleScriptPushCursor();
                ...
                else if (friskcount == 2)
                {
                    if (gBattleStruct->friskedAbility)
                    {
                        gBattlescriptCurrInstr = BattleScript_FriskMsg2;
                    }
                    else
                    {
                        gBattleStruct->friskedAbility = TRUE;
                        gBattlescriptCurrInstr = BattleScript_FriskMsgWithPopup2;
                    }
                }
                else if (friskcount == 3) //For MAX_MON_ITEMS = 3
                {
                    if (gBattleStruct->friskedAbility)
                    {
                        gBattlescriptCurrInstr = BattleScript_FriskMsg3;
                    }
                    else
                    {
                        gBattleStruct->friskedAbility = TRUE;
                        gBattlescriptCurrInstr = BattleScript_FriskMsgWithPopup3;
                    }
                }
                return;
            }
...
{
    NATIVE_ARGS(u8 battler, const u8 *jumpInstr);
    if ((BattlerHasBerry(cmd->battler)))
        gBattlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = cmd->jumpInstr;
}
...
void BS_TryActivateReceiver(void)
{
    ...
    if (IsBattlerAlive(gBattlerAbility)
        ...
        && !BattlerHasHeldItemEffect(battler, HOLD_EFFECT_ABILITY_SHIELD, TRUE)
...
void BS_TryTerrainSeed(void)
{
    NATIVE_ARGS(u8 battler, const u8 *failInstr);
    u32 battler = GetBattlerForBattleScript(cmd->battler);
    if(BattlerHasHeldItemEffect(battler, HOLD_EFFECT_SEEDS, TRUE))
    {
        enum ItemEffect effect = ITEM_NO_EFFECT;
        u16 item = GetBattlerHeldItemWithEffect(battler, HOLD_EFFECT_SEEDS, TRUE);
        if(GetBattlerItemHoldEffectParam(battler, item) == HOLD_EFFECT_PARAM_ELECTRIC_TERRAIN)
            effect = TryHandleSeed(battler, STATUS_FIELD_ELECTRIC_TERRAIN, STAT_DEF, item, ITEMEFFECT_NONE);
        if(GetBattlerItemHoldEffectParam(battler, item) == HOLD_EFFECT_PARAM_GRASSY_TERRAIN)
            effect = TryHandleSeed(battler, STATUS_FIELD_GRASSY_TERRAIN, STAT_DEF, item, ITEMEFFECT_NONE);
        if(GetBattlerItemHoldEffectParam(battler, item) == HOLD_EFFECT_PARAM_MISTY_TERRAIN)
            effect = TryHandleSeed(battler, STATUS_FIELD_MISTY_TERRAIN, STAT_SPDEF, item, ITEMEFFECT_NONE);
        if(GetBattlerItemHoldEffectParam(battler, item) == HOLD_EFFECT_PARAM_PSYCHIC_TERRAIN)
...
void BS_ConsumeBerry(void)
{
    ...
    if (cmd->fromBattler && gLastItemSlot != MAX_MON_ITEMS 
    ...
    {
        if (GetItemPocket(gBattleMons[battler].items[gLastItemSlot]) != POCKET_BERRIES 
            && GetHeldItemSlot(battler, gLastUsedItem, TRUE) != MAX_MON_ITEMS)
            gLastUsedItem = gBattleMons[battler].items[GetBattlerBerry(battler)];
    }
