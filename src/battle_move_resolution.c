...
{
    if (TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_BEFORE_MOVE)
        || TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_BEFORE_MOVE_CATEGORY))
    {
...
static enum CancelerResult CancelerAsleepOrFrozen(struct BattleCalcValues *cv)
{
    ...
    if (gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP)
    {
        ...
        {
            u32 toSub;
            if (IsAbilityAndRecord(cv->battlerAtk, ABILITY_EARLY_BIRD))
                toSub = 2;
            else
                toSub = 1;
...
{
    if (BattlerHasTrait(cv->battlerAtk, ABILITY_TRUANT) && gBattleMons[cv->battlerAtk].volatiles.truantCounter)
    {
        CancelMultiTurnMoves(cv->battlerAtk);
...
static enum CancelerResult CancelerFlinch(struct BattleCalcValues *cv)
{
    ...
    {
        CancelMultiTurnMoves(cv->battlerAtk);
        if (BattlerHasTrait(cv->battlerAtk, ABILITY_STEADFAST))
        {
            SetStatChange(cv->battlerAtk, STAT_SPEED, 1);
            gBattlerAbility = cv->battlerAtk;
            PushTraitStack(cv->battlerAtk, ABILITY_STEADFAST);
...
static enum CancelerResult CancelerParalyzed(struct BattleCalcValues *cv)
{
    if (gBattleMons[cv->battlerAtk].status1 & STATUS1_PARALYSIS
        && !(B_MAGIC_GUARD == GEN_4 && IsAbilityAndRecord(cv->battlerAtk, ABILITY_MAGIC_GUARD))
...
static enum CancelerResult CancelerChoiceLock(struct BattleCalcValues *cv)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (BattlerHasHoldEffectChoice(cv->battlerAtk) || BattlerHasTrait(cv->battlerAtk, ABILITY_GORILLA_TACTICS)))
...
bool32 IsAffectedByFollowMe(enum BattlerId battlerAtk, enum BattleSide defSide, enum Move move)
{
    ...
    if (gSideTimers[defSide].followmePowder && !IsAffectedByPowderMove(battlerAtk))
        return FALSE;
...
static enum CancelerResult CancelerPPDeduction(struct BattleCalcValues *cv)
{
    ...
    {
        if (cv->battlerAtk != cv->battlerDef && BattlerHasTrait(cv->battlerDef, ABILITY_PRESSURE))
             ppToDeduct++;
    }
...
static enum CancelerResult CancelerMoveFailure(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_REST:
        ...
        if (BattlerHasTrait(cv->battlerAtk, ABILITY_INSOMNIA))
            ability = ABILITY_INSOMNIA;
        else if (BattlerHasTrait(cv->battlerAtk, ABILITY_VITAL_SPIRIT))
            ability = ABILITY_VITAL_SPIRIT;
        else if (BattlerHasTrait(cv->battlerAtk, ABILITY_PURIFYING_SALT))
            ability = ABILITY_PURIFYING_SALT;
        else if (BattlerHasTrait(cv->battlerAtk, ABILITY_COMATOSE))
        ...
        PushTraitStack(cv->battlerAtk, ability);
        break;
    case EFFECT_SNORE:
        if (!(gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP)
         && !BattlerHasTrait(cv->battlerAtk, ABILITY_COMATOSE))
            battleScript = BattleScript_ButItFailed;
        break;
    ...
    case EFFECT_STUFF_CHEEKS:
        if (!BattlerHasBerry(cv->battlerAtk))
            battleScript = BattleScript_ButItFailed;
        break;
    case EFFECT_SWALLOW:
    case EFFECT_SPIT_UP:
    ...
    case EFFECT_NATURAL_GIFT:
        if (!BattlerHasBerry(cv->battlerAtk)
         ...
         || BattlerHasTrait(cv->battlerAtk, ABILITY_KLUTZ)
...
static enum CancelerResult CancelerMoveEffectFailureTarget(struct BattleCalcValues *cv)
{
    ...
    while (gBattleStruct->eventState.atkCancelerBattler < gBattlersCount)
    {
        ...
        {
            s32 prio = GetChosenMovePriority(battlerDef);
        ...
        case EFFECT_STAT_CHANGE_MAGNETIC:
            if (!BattlerHasTrait(battlerDef, ABILITY_PLUS) && !BattlerHasTrait(battlerDef, ABILITY_MINUS))
            {
                battleScript = BattleScript_ButItFailed;
            }
            else
...
static enum CancelerResult CancelerPowderStatus(struct BattleCalcValues *cv)
{
    ...
    {
        if (!IsAbilityAndRecord(cv->battlerAtk, ABILITY_MAGIC_GUARD))
...
{
    if (BattlerHasTrait(battler, ABILITY_DAZZLING))
    {
        gLastUsedAbility = ABILITY_DAZZLING;
        return TRUE;
    }
    if (BattlerHasTrait(battler, ABILITY_QUEENLY_MAJESTY))
    {
        gLastUsedAbility = ABILITY_QUEENLY_MAJESTY;
        return TRUE;
    }
    if (BattlerHasTrait(battler, ABILITY_ARMOR_TAIL))
    {
...
{
    bool32 effect = FALSE;
    s32 priority = GetChosenMovePriority(cv->battlerAtk);
    enum MoveTarget moveTarget = GetBattlerMoveTargetType(cv->battlerAtk, cv->move);
...
{
    enum Type moveType = GetBattleMoveType(cv->move);
    if (ProteanTryChangeType(cv->battlerAtk, cv->move, moveType))
    {
...
static bool32 CanTwoTurnMoveFireThisTurn(struct BattleCalcValues *cv)
{
    ...
    u32 attackerWeather = GetAttackerWeather(cv->battlerAtk, weather);
...
static enum CancelerResult CancelerTargetFailure(struct BattleCalcValues *cv)
{
    ...
    enum MoveTarget moveTarget = GetBattlerMoveTargetType(cv->battlerAtk, cv->move);
    s32 movePriority = GetChosenMovePriority(cv->battlerAtk);
    ...
    while (gBattleStruct->eventState.atkCancelerBattler < MAX_BATTLERS_COUNT)
    {
        ...
        else
        {
            ...
            if (ctx.abilityBlocked)
            {
                ...
                RecordAbilityBattle(cv->battlerDef, GetBattlerAbilityIgnoreMoldBreaker(cv->battlerDef)); //
...
static enum CancelerResult CancelerAccuracyCheck(struct BattleCalcValues *cv)
{
    ...
    if (ShouldSkipFRLGAccuracyCheck()
     || ShouldSkipAccuracyCalcPastFirstHit(cv->battlerAtk, cv->moveEffect)
     || IsMaxMove(cv->move)
     || IsZMove(cv->move))
        return CANCELER_RESULT_SUCCESS;
...
{
    if (!BattlerHasTrait(cv->battlerAtk, ABILITY_PARENTAL_BOND)
...
static enum CancelerResult CancelerMultihitMoves(struct BattleCalcValues *cv)
{
    ...
    else if (IsMultiHitMove(cv->move))
    {
        ...
        else if (BattlerHasTrait(cv->battlerAtk, ABILITY_SKILL_LINK))
        {
            gMultiHitCounter = 5;
        }
        else
...
static enum MoveEndResult MoveEndProtectLikeEffect(struct BattleCalcValues *cv)
{
    ...
    if (gProtectStructs[cv->battlerAtk].chargingTurn
     ...
     || CanBattlerAvoidContactEffects(cv->battlerAtk, cv->battlerDef, cv->move))
    {
        gBattleScripting.moveendState++;
        return result;
    }
    ...
    case PROTECT_BANEFUL_BUNKER:
        if (CanBePoisoned(cv->battlerDef, cv->battlerAtk))
        {
    ...
    case PROTECT_BURNING_BULWARK:
        if (CanBeBurned(gBattlerTarget, gBattlerAttacker))
        {
    ...
    if (IsBattlerUsingBeakBlast(cv->battlerDef)
     ...
     && CanBeBurned(cv->battlerAtk, cv->battlerAtk))
    {
...
static void SetHealScript(struct BattleCalcValues *cv, s32 healAmount)
{
    ...
    if (BattlerHasTrait(cv->battlerDef, ABILITY_LIQUID_OOZE)
...
static enum MoveEndResult MoveEndAbsorb(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_MAX_HP_50_RECOIL:
        if (IsBattlerAlive(cv->battlerAtk)
         ...
         && !IsAbilityAndRecord(cv->battlerAtk, ABILITY_MAGIC_GUARD))
        {
...
static enum MoveEndResult MoveEndRage(struct BattleCalcValues *cv)
{
    ...
    if (gBattleMons[cv->battlerDef].volatiles.rage
     ...
     && IsBattlerTurnDamaged(cv->battlerDef, EXCLUDING_SUBSTITUTES)
     && !IsBattleMoveStatus(cv->move)
     && CompareStat(cv->battlerDef, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
    {
...
static enum MoveEndResult MoveEndAbilities(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_MOVE_END, cv->battlerDef, 0, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
    else if (TryClearIllusion(cv->battlerDef))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndFormChangeOnHit(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_FORM_CHANGE_ON_HIT, cv->battlerDef, 0, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndAbilitiesAttacker(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_MOVE_END_ATTACKER, cv->battlerAtk, 0, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndQueueDancer(struct BattleCalcValues *cv)
{
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        if (BattlerHasTrait(battler, ABILITY_DANCER))
...
static enum MoveEndResult MoveEndStatusImmunityAbilities(struct BattleCalcValues *cv)
{
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        if (AbilityBattleEffects(ABILITYEFFECT_IMMUNITY, battler, 0, TRUE))
            result = MOVEEND_RESULT_RUN_SCRIPT;
    }
...
static enum MoveEndResult MoveEndItemEffectsTarget(struct BattleCalcValues *cv)
{
    ...
    if (ItemBattleEffects(cv->battlerDef, cv->battlerAtk, IsOnTargetHitActivation)
     || ItemBattleEffects(cv->battlerDef, cv->battlerAtk, IsOnStatusChangeActivation))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndItemEffectsAttacker1(struct BattleCalcValues *cv)
{
    ...
    if (ItemBattleEffects(cv->battlerAtk, cv->battlerDef, IsOnAttackerAfterHitActivation)
     || ItemBattleEffects(cv->battlerAtk, cv->battlerDef, IsOnStatusChangeActivation)
     || ItemBattleEffects(cv->battlerAtk, cv->battlerDef, IsOnHpThresholdActivation))
        result = MOVEEND_RESULT_RUN_SCRIPT;
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
            ...
            && TryTriggerSymbiosis(battler, BATTLE_PARTNER(battler)))
        {
            ...
            for (i = 0; i < MAX_MON_ITEMS; i++)
            {
                ...
                && CanBattlerGetOrLoseItem(BATTLE_PARTNER(battler), BATTLE_PARTNER(battler), gBattleMons[BATTLE_PARTNER(battler)].items[i]))
                {
...
static enum MoveEndResult MoveEndHpThresholdItemsTarget(struct BattleCalcValues *cv)
{
    ...
    if (gMultiHitCounter &&
        ItemBattleEffects(cv->battlerDef, cv->battlerAtk, IsOnHpThresholdActivation))
    {
        result = MOVEEND_RESULT_RUN_SCRIPT;
    }
    ...
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        if ((CanFireMoveThawTarget(cv->move, GetBattleMoveType(cv->move)) || CanBurnHitThaw(cv->move)) && gBattleMons[battler].status1 & STATUS1_FREEZE)
        {
            ...
            return MOVEEND_RESULT_RUN_SCRIPT;
              ...
              && CanMoveThawTarget(cv->battlerAtk, cv->move))
        {
            DefrostBattler(battler, gBattleMons[battler].status1);
            BattleScriptCall(battleScript);
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_DEFROSTED;
...
static enum MoveEndResult MoveEndMoveBlockRecoil(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_CHLOROBLAST:
        ...
        {
            if (IsAbilityAndRecord(cv->battlerAtk, ABILITY_ROCK_HEAD)
             || IsAbilityAndRecord(cv->battlerAtk, ABILITY_MAGIC_GUARD))
                break;
...
{
    if (IsSheerForceAffected(cv->move, cv->battlerAtk))
...
static enum MoveEndResult MoveEndMoveBlock(struct BattleCalcValues *cv)
{
    ...
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++)
        {
            if (gBattleMons[battlerDef].items[i] != ITEM_NONE
             && IsBattlerTurnDamaged(battlerDef, EXCLUDING_SUBSTITUTES)
                 ...
                 && CanBattlerGetOrLoseItem(battlerDef, cv->battlerAtk, gBattleMons[battlerDef].items[i])
        ...
        if (slot != MAX_MON_ITEMS && !NoAliveMonsForEitherParty() )
        {
            ...
            if (BattlerHasTrait(battlerDef, ABILITY_STICKY_HOLD))
            {
            ...
            if (!BattlerHasTrait(battlerDef, ABILITY_GORILLA_TACTICS))
            ...
            CheckSetUnburden(battlerDef);
                ...
                if (B_KNOCK_OFF_REMOVAL >= GEN_5)
                {
                    ...
                    gBattleStruct->itemLost[side][gBattlerPartyIndexes[battlerDef]][slot].stolen = TRUE;
                }
                else
                {
    ...
    case EFFECT_STEAL_ITEM:
        ...
        for (i = 0; i < MAX_MON_ITEMS; i++) //Gather all stealable item slots
        {
            if (IsBattlerTurnDamaged(battlerDef, EXCLUDING_SUBSTITUTES)
                && gBattleMons[cv->battlerAtk].items[i] == ITEM_NONE
                && gBattleMons[battlerDef].items[i] != ITEM_NONE
                && IsBattlerAlive(cv->battlerAtk)
                && CanStealItem(cv->battlerAtk, battlerDef, gBattleMons[battlerDef].items[i]))
            {
                if (targetableSlots[0] != MAX_MON_ITEMS)
                    index++;
                targetableSlots[index] = i;
        ...
        case EFFECT_HIT_SWITCH_TARGET:
            ...
            {
                if (BattlerHasTrait(battlerDef, ABILITY_GUARD_DOG))
                {
                    gBattleStruct->eventState.moveEndBattler = 0;
                    gBattleScripting.moveendState++;
                    return MOVEEND_RESULT_CONTINUE;
                }
                if (BattlerHasTrait(battlerDef, ABILITY_SUCTION_CUPS))
                {
                    gBattlerAbility = battlerDef;
                    PushTraitStack(battlerDef, ABILITY_SUCTION_CUPS);
        ...
        case EFFECT_SMACK_DOWN:
             ...
             && gBattleMons[battlerDef].volatiles.semiInvulnerable != STATE_SKY_DROP_TARGET)
            {
                ...
                if (IsBattlerGrounded(battlerDef) && !onAir)
                    break;
        ...
        case EFFECT_FELL_STINGER:
            if (IsBattlerAlive(cv->battlerAtk)
             ...
             && IsBattlerTurnDamaged(battlerDef, EXCLUDING_SUBSTITUTES)
             && !NoAliveMonsForEitherParty()
             && CompareStat(cv->battlerAtk, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
            {
...
static enum MoveEndResult MoveEndItemEffectsAttacker2(struct BattleCalcValues *cv)
{
    ...
    if (ItemBattleEffects(cv->battlerAtk, cv->battlerDef, IsOnStatusChangeActivation)
     || ItemBattleEffects(cv->battlerAtk, cv->battlerDef, IsOnHpThresholdActivation))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndAbilityEffectFoesFainted(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_MOVE_END_FOES_FAINTED, cv->battlerAtk, cv->move, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndColorChange(struct BattleCalcValues *cv)
{
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        if (battler == cv->battlerAtk)
            continue;
        if (AbilityBattleEffects(ABILITYEFFECT_COLOR_CHANGE, battler, 0, TRUE))
            return MOVEEND_RESULT_RUN_SCRIPT;
    }
...
static enum MoveEndResult MoveEndKeeMarangaHpThresholdItemTarget(struct BattleCalcValues *cv)
{
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        if (ItemBattleEffects(battlerDef, cv->battlerAtk, IsKeeMarangaBerryActivation)
         || ItemBattleEffects(battlerDef, cv->battlerAtk, IsOnHpThresholdActivation))
            return MOVEEND_RESULT_RUN_SCRIPT;
    }
...
static enum MoveEndResult MoveEndFormChange(struct BattleCalcValues *cv)
{
    ...
    if (!gBattleStruct->battlerState[cv->battlerAtk].redCardSwitched
     && TryBattleFormChange(cv->battlerAtk, FORM_CHANGE_BATTLE_AFTER_MOVE))
    {
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveEndResult MoveEndLifeOrbShellBell(struct BattleCalcValues *cv)
{
    ...
    if (ItemBattleEffects(cv->battlerAtk, 0, IsLifeOrbShellBellActivation))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
{
    enum MoveEndResult result = MOVEEND_RESULT_CONTINUE;
    u32 slot, i, j, targetableSlots[MAX_MON_ITEMS], index;
    enum Item attackerItems[MAX_MON_ITEMS];
    bool32 attackerhasItem = FALSE;
    bool32 hasPendingStolenItem[MAX_MON_ITEMS];

    for (i = 0; i < MAX_MON_ITEMS; i++)
    {
        attackerItems[i] = gBattleMons[cv->battlerAtk].items[i];
     ...
     && !GetBattlerPartyState(cv->battlerAtk)->isKnockedOff) // Gen3 edge case where the knocked of item was not removed
    {
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            ...
            if (battlerDef != cv->battlerAtk                                                       // Cannot pickpocket yourself
                && !gBattleStruct->battlerState[battlerDef].notOnField
                && !IsBattlerUnaffectedByMove(battlerDef)                                             
                && BattlerHasTrait(battlerDef, ABILITY_PICKPOCKET)                                   // Target must have pickpocket ability
                && IsMoveMakingContact(cv->battlerAtk, battlerDef, gCurrentMove)                 // Pickpocket requires contact
                && IsBattlerTurnDamaged(battlerDef, EXCLUDING_SUBSTITUTES)                                                  // Target needs to have been damaged
                && !DoesSubstituteBlockMove(cv->battlerAtk, battlerDef, gCurrentMove)              // Subsitute unaffected
                && IsBattlerAlive(battlerDef))                                                       // Battler must be alive to pickpocket
            {
                index = 0;
                targetableSlots[0] = MAX_MON_ITEMS; // Invalid value for first slot if no valid slots found
                ...
                for (j = 0; j < MAX_MON_ITEMS; j++) //Gather all stealable item slots
                {
                    ...
                    && CanStealItem(battlerDef, cv->battlerAtk, attackerItems[j]) // Cannot steal plates, mega stones, etc
                    && gBattleMons[battlerDef].items[j] == ITEM_NONE                                   // Pickpocketer can't have an item already
                ...
                if (targetableSlots[0] != MAX_MON_ITEMS)
                {
                    ...
                    if (!BattlerHasTrait(cv->battlerAtk, ABILITY_STICKY_HOLD))
                        StealTargetItem(battlerDef, cv->battlerAtk, slot); // Target takes attacker's item

                gEffectBattler = cv->battlerAtk;
                PushTraitStack(battlerDef, ABILITY_PICKPOCKET);
...
static enum MoveEndResult MoveEndThirdMoveBlock(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_NATURAL_GIFT:
        if (!gBattleStruct->unableToUseMove)
        {
            ...
            {   
                if (GetItemPocket(gBattleMons[cv->battlerAtk].items[i]) == POCKET_BERRIES)
                {
                    item = gBattleMons[cv->battlerAtk].items[i];
                    break;
                }
            }
            if (item != ITEM_NONE)
            {
                ...
                GetBattlerPartyState(cv->battlerAtk)->usedHeldItems[i] = item;
                ...
                BtlController_EmitSetMonData(
                    ...
                    0,
                    sizeof(gBattleMons[cv->battlerAtk].items[i]),
                    &gBattleMons[cv->battlerAtk].items[i]);
                MarkBattlerForControllerExec(cv->battlerAtk);
                ClearBattlerItemEffectHistory(cv->battlerAtk);

                if (!TrySymbiosis(cv->battlerAtk, NULL))
                    result = MOVEEND_RESULT_RUN_SCRIPT;
            }
        }
    default:
...
static enum MoveEndResult MoveEndSprayLeppaBlunder(struct BattleCalcValues *cv)
{
    ...
    if (ItemBattleEffects(cv->battlerAtk, 0, IsSprayLeppaBlunderActivation))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
static enum MoveResult StatChangeSubstitute(struct BattleCalcValues *cv)
{
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        if (IsSubstituteProtected(cv->battlerAtk, battler, cv->move))
...
static void TryAcupressureStatChange(struct BattleCalcValues *cv)
{
    ...
    {
        if (CompareStat(cv->battlerDef, stat, MAX_STAT_STAGE, CMP_LESS_THAN))
            bits |= 1u << stat;
    }
...
static bool32 TryMagicBounce(struct BattleCalcValues *cv)
{
    ...
    if (!BattlerHasTrait(cv->battlerDef, ABILITY_MAGIC_BOUNCE))
        return FALSE;
