...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
    ...
    if (maxDamageTaken > gBattleMons[battler].hp
        && !(BattlerHasHeldItemEffect(battler, HOLD_EFFECT_FOCUS_SASH, TRUE) || (!IsMoldBreakerTypeAbility(opposingBattler, gBattleMons[opposingBattler].ability) && GetConfig(CONFIG_STURDY) >= GEN_5 && aiAbility == ABILITY_STURDY)))
    {
        getsOneShot = TRUE;
    }
    ...
    if (gAiThinkingStruct->aiFlags[battler] & AI_FLAG_SMART_SWITCHING)
    {
            ...
            && RandomPercentage(RNG_AI_SWITCH_YAWN, GetSwitchChance(SHOULD_SWITCH_YAWN)))
        {
            ...
            if ((monAbility == ABILITY_NATURAL_CURE
                ...
                || (IsBattlerGrounded(battler, monAbility)
...
static u32 GetSwitchinHazardsDamage(u32 battler, struct BattlePokemon *battleMon)
{
        ...
        !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || ability == ABILITY_KLUTZ)))
    {
        ...
        // Spikes
        if (IsHazardOnSide(side, HAZARDS_SPIKES) && IsMonGrounded(battler, ability, defType1, defType2))
        {
...
static s32 GetSwitchinWeatherImpact(void)
{
    ...
    {
        // Damage
        if (!SwitchInCandidateHeldItemWithEffect(gAiLogicData->switchinCandidate.battleMon, HOLD_EFFECT_SAFETY_GOGGLES) && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_OVERCOAT)
        {
...
static u32 GetSwitchinRecurringDamage(void)
{
    ...
    if (ability != ABILITY_MAGIC_GUARD && ability != ABILITY_KLUTZ)
    {
        ...
        else if (SwitchInCandidateHeldItemWithEffect(gAiLogicData->switchinCandidate.battleMon, HOLD_EFFECT_LIFE_ORB) && ability != ABILITY_SHEER_FORCE)
        {
...
static u32 GetSwitchinHitsToKO(s32 damageTaken, u32 battler)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (SwitchInCandidateHeldItemWithEffect(gAiLogicData->switchinCandidate.battleMon, HOLD_EFFECT_FOCUS_SASH) || (!opponentCanBreakMold && GetConfig(CONFIG_STURDY) >= GEN_5 && ability == ABILITY_STURDY)) && hitsToKO < 1)
            currentHP = 1;
...
static bool32 CanAbilityTrapOpponent(enum Ability ability, u32 opponent)
{
    ...
    else if (ability == ABILITY_ARENA_TRAP && IsBattlerGrounded(opponent, gAiLogicData->abilities[opponent]))
        return TRUE;
