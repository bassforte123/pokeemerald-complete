    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (gBattleResources->secretBase->party.species[i])
        {
            ...
            for (j = 0; j < MAX_MON_ITEMS; j++)
                SetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_HELD_ITEM + j, &gBattleResources->secretBase->party.heldItem[i][j]);
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    for (i = 0; i < MAX_MON_ITEMS; i++)
        dst->items[i] = GetMonData(src, MON_DATA_HELD_ITEM + i);
...
void SetWildMonHeldItem(void)
{
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE)))
    {
        ...
        for (i = 0; i < count; i++)
        {
            // prevent overwriting previously set item
            // handled with empty slot checks (Multi)
            ...
            if (gMapHeader.mapLayoutId == LAYOUT_ALTERING_CAVE)
            {
                ...
                if (alteringCaveId != 0)
                {
                    ...
                    if (rnd < chanceNotRare)
                        continue;
                    slot = GetMonNextEmptySlot(&gParties[B_TRAINER_OPPONENT_A][i], sAlteringCaveWildMonHeldItems[alteringCaveId].item);
                    if (slot != MAX_MON_ITEMS)
                        SetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_HELD_ITEM + slot, &sAlteringCaveWildMonHeldItems[alteringCaveId].item);
                }
                else
                {
                    ...
                    if (rnd < chanceNotRare)
                    {
                        slot = GetMonNextEmptySlot(&gParties[B_TRAINER_OPPONENT_A][i], gSpeciesInfo[species].itemCommon);
                        if (slot != MAX_MON_ITEMS)
                            SetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_HELD_ITEM + slot, &gSpeciesInfo[species].itemCommon);
                    }
                    else
                    {
                        slot = GetMonNextEmptySlot(&gParties[B_TRAINER_OPPONENT_A][i], gSpeciesInfo[species].itemRare);
                        if (slot != MAX_MON_ITEMS)
                            SetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_HELD_ITEM + slot, &gSpeciesInfo[species].itemRare);
                    }
                }
            }
            else
