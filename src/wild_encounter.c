...
u8 ChooseWildMonLevel(const struct WildPokemon *wildPokemon, u8 wildMonIndex, enum WildPokemonArea area)
{
    ...
    if (LURE_STEP_COUNT == 0)
    {
        ...
        {
            if (MonHasTrait(&gParties[B_TRAINER_PLAYER][0], ABILITY_HUSTLE) || MonHasTrait(&gParties[B_TRAINER_PLAYER][0], ABILITY_VITAL_SPIRIT) || MonHasTrait(&gParties[B_TRAINER_PLAYER][0], ABILITY_PRESSURE))
            {
                if (Random() % 2 == 0)
                    return max;
