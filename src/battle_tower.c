...
static void FillTentTrainerParty_(u16 trainerId, u8 firstMonId, u8 monCount)
{
    ...
    while (i != monCount)
    {
        ...
        for (j = 0; j < i + firstMonId; j++)
        {
            ...
            {
                if (GetMonData(&gParties[B_TRAINER_OPPONENT_A][j], MON_DATA_HELD_ITEM + k) != ITEM_NONE)
                {
                    for (l = 0; l < MAX_MON_ITEMS; l++)
                    {                            
                        if (GetMonData(&gParties[B_TRAINER_OPPONENT_A][j], MON_DATA_HELD_ITEM + k) == gFacilityTrainerMons[monId].heldItem[l])
                        {
                            goto tentmatch;
                        }
                    }
