...
bool32 IsAffectedByFollowMe(u32 battlerAtk, u32 defSide, u32 move)
{
    ...
    if (gSideTimers[defSide].followmePowder && !IsAffectedByPowderMove(battlerAtk, ability, BattlerHasHeldItemEffect(battlerAtk, HOLD_EFFECT_SAFETY_GOGGLES, TRUE)))
        return FALSE;
...
const u8 *CheckSkyDropState(u32 battler, enum SkyDropState skyDropState)
{
    ...
    if (gBattleMons[otherSkyDropper].volatiles.lockConfusionTurns && skyDropState != SKY_DROP_STATUS_YAWN)
    {
        ...
        if (!(gBattleMons[otherSkyDropper].volatiles.confusionTurns > 0
            ...
            || IsBattlerTerrainAffected(otherSkyDropper, ability, STATUS_FIELD_MISTY_TERRAIN)))
        {
            // Set confused status
...
static enum MoveCanceler CancelerChoiceLock(struct BattleContext *ctx)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (BattlerHasHoldEffectChoice(ctx->battlerAtk) || ctx->abilities[ctx->battlerAtk] == ABILITY_GORILLA_TACTICS))
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    if (battleScriptBlocksMove == NULL)
    {
        ...
        else if (IsBattlerTerrainAffected(battlerDef, abilityDef, STATUS_FIELD_PSYCHIC_TERRAIN) // Not an ability but similar conditions
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            enum Ability abilityAtk = GetBattlerAbility(gBattlerAttacker);
            if (IsAffectedByPowderMove(gBattlerAttacker, abilityAtk, BattlerHasHeldItemEffect(gBattlerAttacker, HOLD_EFFECT_SAFETY_GOGGLES, TRUE)))
            {
...
bool32 IsBattlerTerrainAffected(u32 battler, enum Ability ability, u32 terrainFlag)
{
    ...
    return IsBattlerGrounded(battler, ability);
}
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_SLEEP:
        ...
        else if (IsBattlerTerrainAffected(battlerDef, abilityDef, STATUS_FIELD_ELECTRIC_TERRAIN))
        {
    ...
    else if (IsBattlerTerrainAffected(battlerDef, abilityDef, STATUS_FIELD_MISTY_TERRAIN))
    {
...
bool32 CanBeConfused(u32 battler)
{
    ...
    if (gBattleMons[battler].volatiles.confusionTurns > 0
     || IsBattlerTerrainAffected(battler, ability, STATUS_FIELD_MISTY_TERRAIN)
...
bool32 CanBattlerAvoidContactEffects(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, u32 move)
{
...
bool32 IsMoveMakingContact(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, u32 move)
{
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, enum Ability ability, enum InverseBattleCheck checkInverse, bool32 isAnticipation, bool32 skipIronBall)
{
...
bool32 IsBattlerGrounded(u32 battler, enum Ability ability)
{
    return IsBattlerGroundedInverseCheck(battler, ability, NOT_INVERSE_BATTLE, FALSE, FALSE);
}
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_ELECTRO_BALL:
        speed = GetBattlerTotalSpeedStat(battlerAtk, ctx->abilityAtk) / GetBattlerTotalSpeedStat(battlerDef, ctx->abilityDef);
    ...
    case EFFECT_GYRO_BALL:
        {
            u32 attackerSpeed = GetBattlerTotalSpeedStat(battlerAtk, ctx->abilityAtk);
            if (attackerSpeed == 0)
            {
                basePower = 1;
            }
            ...
            {
                basePower = ((25 * GetBattlerTotalSpeedStat(battlerDef, ctx->abilityDef)) / attackerSpeed) + 1;
                if (basePower > 150)
                    basePower = 150;
            }
            break;
    ...
    case EFFECT_MISTY_EXPLOSION:
        if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_MISTY_TERRAIN))
    ...
    case EFFECT_TERRAIN_PULSE:
        if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_TERRAIN_ANY))
            basePower *= 2;
        break;
    case EFFECT_EXPANDING_FORCE:
        if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_PSYCHIC_TERRAIN))
            basePower = uq4_12_multiply(basePower, UQ_4_12(1.5));
        break;
    case EFFECT_RISING_VOLTAGE:
        if (IsBattlerTerrainAffected(battlerDef, ctx->abilityDef, STATUS_FIELD_ELECTRIC_TERRAIN))
            basePower *= 2;
        break;
    case EFFECT_BEAT_UP:
        if (GetConfig(CONFIG_BEAT_UP) >= GEN_5)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (GetMoveEffect(ctx->chosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_GRASSY_TERRAIN) && moveType == TYPE_GRASS)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerDef, ctx->abilityDef, STATUS_FIELD_MISTY_TERRAIN) && moveType == TYPE_DRAGON)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_ELECTRIC_TERRAIN) && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TYPE_PSYCHIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    if (GetMoveEffect(ctx->move) == EFFECT_SUPER_EFFECTIVE_ON_ARG && defType == GetMoveArgType(ctx->move) && !ctx->isAnticipation)
        mod = UQ_4_12(2.0);
    if (ctx->moveType == TYPE_GROUND && defType == TYPE_FLYING && IsBattlerGrounded(ctx->battlerDef, ctx->abilityDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_GROUND && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, INVERSE_BATTLE, ctx->isAnticipation, FALSE) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
    ...
    if (GetConfig(CONFIG_IRON_BALL) >= GEN_5
        ...
        && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_FLYING)
        && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, NOT_INVERSE_BATTLE, FALSE, TRUE) // We want to ignore Iron Ball so skip item check
...
void SortBattlersBySpeed(u8 *battlers, bool32 slowToFast)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        speeds[i] = GetBattlerTotalSpeedStat(battler, GetBattlerAbility(battler));
    }
...
u32 GetBattlerMoveTargetType(u32 battler, u32 move)
{
    ...
    if (effect == EFFECT_EXPANDING_FORCE && IsBattlerTerrainAffected(battler, GetBattlerAbility(battler), STATUS_FIELD_PSYCHIC_TERRAIN))
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, enum Ability atkAbility, enum Ability defAbility)
{
    u32 calc, moveAcc;
...
bool32 IsAffectedByPowderMove(u32 battler, u32 ability, bool32 hasGoggles)
{
