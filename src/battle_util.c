...
u32 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Encore
            ...
            unusableMoves |= 1u << i;
        // Choice Items
        else if (check & MOVE_LIMITATION_CHOICE_ITEM && BATTLER_IS_HOLDING_CHOICE_ITEM_WITH_NEGATION(battler) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
            unusableMoves |= 1u << i;
        // Assault Vest
...
static enum MoveCanceller CancellerMultihitMoves(void)
{
    ...
    {
        if (GetMoveEffect(gCurrentMove) == EFFECT_POPULATION_BOMB && BattlerHasHeldItemEffect(gBattlerAttacker, HOLD_EFFECT_LOADED_DICE, TRUE))
        {
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
            ...
            case ABILITY_PICKUP:
                for (i = 0; i < MAX_MON_ITEMS; i++)
                {
                    ...
                    && PickupHasValidTarget(battler))
                    {
                        ...
                        gLastUsedItem = GetUsedHeldItem(gBattlerTarget, i);
            ...
            case ABILITY_HARVEST:
                for (i = 0; i < MAX_MON_ITEMS; i++)
                {
                    if ((IsBattlerWeatherAffected(battler, B_WEATHER_SUN) || RandomPercentage(RNG_HARVEST, 50))
                     && gBattleMons[battler].items[i] == ITEM_NONE // berry slot
                     && gBattleStruct->changedItems[battler][i] == ITEM_NONE   // Will not inherit an item
                     && GetItemPocket(GetUsedHeldItem(battler, i)) == POCKET_BERRIES)
                    {
                        gLastUsedItem = GetUsedHeldItem(battler, i);
            ...
            case ABILITY_CUD_CHEW:
                for (slot = 0; slot < MAX_MON_ITEMS; slot++)
                {
                    if (gDisableStructs[battler].cudChew[slot])
                    {
                        ...
                        gLastUsedItem = gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)][slot];
                        gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)][slot] = ITEM_NONE;
                ...
                for (i = 0; i < MAX_MON_ITEMS; i++)
                {
                    if (!gDisableStructs[battler].cudChew[i] && GetItemPocket(GetUsedHeldItem(battler, i)) == POCKET_BERRIES)
                    {
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        bool32 hasShield = BattlerHasHeldItemEffectIgnoreAbility(gBattlerAttacker, HOLD_EFFECT_ABILITY_SHIELD, FALSE);
        ...
        case ABILITY_MUMMY:
            if (IsBattlerAlive(gBattlerAttacker)
             && IsBattlerTurnDamaged(gBattlerTarget)
             && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), move)
        ...
        case ABILITY_POISON_POINT:
            ...
            POISON_POINT:
            {
                ...
                && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, abilityAtk, move))
                {
                    gEffectBattler = gBattlerAttacker;
                    gBattleScripting.battler = gBattlerTarget;
...
static enum ItemEffect ConsumeBerserkGene(u32 battler, enum ItemCaseId caseID)
{
    ...
    {
        gLastUsedItem = GetBattlerHeldItemWithEffect(battler, HOLD_EFFECT_BERSERK_GENE, TRUE);
...
static u8 ItemEffectMoveEnd(u32 battler, enum ItemHoldEffect holdEffect)
{
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_HP) || holdEffect == HOLD_EFFECT_RESTORE_HP)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_HP);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_PCT_HP) || holdEffect == HOLD_EFFECT_RESTORE_PCT_HP)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_PCT_HP);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_PP) || holdEffect == HOLD_EFFECT_RESTORE_PP)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_RESTORE_PP);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_PAR) || holdEffect == HOLD_EFFECT_CURE_PAR)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_PAR);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_PSN) || holdEffect == HOLD_EFFECT_CURE_PSN)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_PSN);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_BRN) || holdEffect == HOLD_EFFECT_CURE_BRN)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_BRN);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_FRZ) || holdEffect == HOLD_EFFECT_CURE_FRZ)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_FRZ);
            gBattleMons[battler].status1 &= ~STATUS1_FREEZE;
            BattleScriptCall(BattleScript_BerryCureFrzRet);
            effect = ITEM_STATUS_CHANGE;
        }
        if (gBattleMons[battler].status1 & STATUS1_FROSTBITE && !UnnerveOn(battler, SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_FRZ)))
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_FRZ);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_SLP) || holdEffect == HOLD_EFFECT_CURE_SLP)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_SLP);
    ...
    {
        if (gBattleMons[battler].volatiles.confusionTurns && !UnnerveOn(battler, SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_CONFUSION)))
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_CONFUSION);
            RemoveConfusionStatus(battler);
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_MENTAL_HERB) || holdEffect == HOLD_EFFECT_MENTAL_HERB)
    {
        ...
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_MENTAL_HERB);
    ...
    {
        if ((gBattleMons[battler].status1 & STATUS1_ANY || gBattleMons[battler].volatiles.confusionTurns > 0) && !UnnerveOn(battler, SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_STATUS)))
        {
            if (!holdEffect)
    ...
    else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CRITICAL_UP) || holdEffect == HOLD_EFFECT_CRITICAL_UP) // lansat berry
    {
        if (B_BERRIES_INSTANT >= GEN_4
            ...
            && HasEnoughHpToEatBerry(battler, GetBattlerItemHoldEffectParam(battler, SearchItemSlots(battlerItems, HOLD_EFFECT_CRITICAL_UP)), SearchItemSlots(battlerItems, HOLD_EFFECT_CRITICAL_UP)))
        {
            if (!holdEffect)
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CRITICAL_UP);
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_ON_SWITCH_IN_FIRST_TURN: //First turn only activates one item so activations don't need to be separated out (Multi)
        ...
        if (!gSpecialStatuses[battler].switchInItemDone)
        {
            if (SearchItemSlots(battlerItems, HOLD_EFFECT_DOUBLE_PRIZE))
            {
                ...
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_DOUBLE_PRIZE);
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_ROOM_SERVICE))
            {
                ...
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_ROOM_SERVICE);
                    BattleScriptExecute(BattleScript_ConsumableStatRaiseEnd2);
                    effect = ITEM_STATS_CHANGE;
                }
            }
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_SEEDS))
            {
                ...
                if(GetBattlerItemHoldEffectParam(battler, gLastUsedItem) == HOLD_EFFECT_PARAM_ELECTRIC_TERRAIN)
                    effect = TryHandleSeed(battler, STATUS_FIELD_ELECTRIC_TERRAIN, STAT_DEF, gLastUsedItem, caseID);
                if(GetBattlerItemHoldEffectParam(battler, gLastUsedItem) == HOLD_EFFECT_PARAM_GRASSY_TERRAIN)
                    effect = TryHandleSeed(battler, STATUS_FIELD_GRASSY_TERRAIN, STAT_DEF, gLastUsedItem, caseID);
                if(GetBattlerItemHoldEffectParam(battler, gLastUsedItem) == HOLD_EFFECT_PARAM_MISTY_TERRAIN)
    ...
    case ITEMEFFECT_TRY_HEALING:
        ...
        {   
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CURE_CONFUSION))
            {
                ...
                if (gBattleMons[battler].volatiles.confusionTurns > 0 && !UnnerveOn(battler, atkItem))
                {
                    gLastUsedItem = atkItem;
                    RemoveConfusionStatus(battler);
    ...
    case ITEMEFFECT_MOVE_END:
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            if (gBattleStruct->battlerState[battler].itemCanBeKnockedOff
             && (BattlerHasBerry(battler)
              || BattlerHasHeldItemEffect(battler, HOLD_EFFECT_RESTORE_HP, TRUE)))
                continue;
    ...
    case ITEMEFFECT_KINGSROCK:
        ...
        else if (SearchItemSlots(battlerItems, HOLD_EFFECT_BLUNDER_POLICY))
        {
             ...
             && CompareStat(gBattlerAttacker, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN))
            {
                ...
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_BLUNDER_POLICY);
    ...
    case ITEMEFFECT_LIFEORB_SHELLBELL:
        ...
        else if (SearchItemSlots(battlerItems, HOLD_EFFECT_SHELL_BELL))
        {
            if (gBattleScripting.savedDmg > 0
                ...
                && (B_HEAL_BLOCKING < GEN_5 || !gBattleMons[battler].volatiles.healBlock))
            {
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CHOICE_SCARF);
                if(gLastUsedItem == ITEM_NONE)
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_SHELL_BELL);

                gPotentialItemEffectBattler = gBattlerAttacker;
                gBattleScripting.battler = gBattlerAttacker;
                gBattleStruct->moveDamage[gBattlerAttacker] = (gBattleScripting.savedDmg / GetBattlerItemHoldEffectParam(gBattlerAttacker, SearchItemSlots(battlerItems, HOLD_EFFECT_SHELL_BELL))) * -1;
                if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
                    gBattleStruct->moveDamage[gBattlerAttacker] = -1;
        ...
        else if (SearchItemSlots(battlerItems, HOLD_EFFECT_THROAT_SPRAY)) // Does NOT need to be a damaging move
        {
            ...
            {
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_THROAT_SPRAY);
                gBattleScripting.battler = gBattlerAttacker;
                SET_STATCHANGER(STAT_SPATK, 1, FALSE);
                effect = ITEM_STATS_CHANGE;
                BattleScriptCall(BattleScript_AttackerItemStatRaise);
        ...
        if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_ROCKY_HELMET))
            {
                    ...
                    && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
                {
                    ...
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_ROCKY_HELMET);
                    PREPARE_ITEM_BUFFER(gBattleTextBuff1, gLastUsedItem);
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_WEAKNESS_POLICY))
            {
                ...
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_WEAKNESS_POLICY);
                    effect = ITEM_STATS_CHANGE;
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_SNOWBALL))
            {
                if (IsBattlerAlive(battler)
                    ...
                    && moveType == TYPE_ICE)
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_SNOWBALL);
                    effect = ITEM_STATS_CHANGE;
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_LUMINOUS_MOSS))
            {
                if (IsBattlerAlive(battler)
                    ...
                    && moveType == TYPE_WATER)
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_LUMINOUS_MOSS);
                    effect = ITEM_STATS_CHANGE;
                    BattleScriptCall(BattleScript_TargetItemStatRaise);
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_CELL_BATTERY))
            {
                ...
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_CELL_BATTERY);
                    effect = ITEM_STATS_CHANGE;
                    BattleScriptCall(BattleScript_TargetItemStatRaise);
                    SET_STATCHANGER(STAT_ATK, 1, FALSE);
                }
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_ABSORB_BULB))
            {
                if (IsBattlerAlive(battler)
                    ...
                    && moveType == TYPE_WATER)
                {
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_ABSORB_BULB);
                    effect = ITEM_STATS_CHANGE;
                    BattleScriptCall(BattleScript_TargetItemStatRaise);
                    SET_STATCHANGER(STAT_SPATK, 1, FALSE);
                }
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_JABOCA_BERRY)) // consume and damage attacker if used physical move
            {
                 ...
                 && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
                {
                    ...
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_JABOCA_BERRY);
                    effect = ITEM_HP_CHANGE;
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_ROWAP_BERRY)) // consume and damage attacker if used special move
            {
                 ...
                 && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
                {
                    ...
                    gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_ROWAP_BERRY);
                    effect = ITEM_HP_CHANGE;
                    BattleScriptCall(BattleScript_JabocaRowapBerryActivates);
                    PREPARE_ITEM_BUFFER(gBattleTextBuff1, gLastUsedItem);
            ...
            else if (SearchItemSlots(battlerItems, HOLD_EFFECT_STICKY_BARB))
            {
                   ...
                   && gBattleMons[gBattlerAttacker].items[barbSlot] == ITEM_NONE)
                {
                    ...
                    StealTargetItem(gBattlerAttacker, gBattlerTarget, 0); // Attacker takes target's barb
    ...
    case ITEMEFFECT_ORBS:
        STORE_BATTLER_ITEMS(battler);
        u16 battlerAbility = GetBattlerAbility(battler);
        if (SearchItemSlots(battlerItems, HOLD_EFFECT_TOXIC_ORB))
        {
            if (CanBePoisoned(battler, battler, battlerAbility, battlerAbility)) // Can corrosion trigger toxic orb on itself?
            {
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_TOXIC_ORB);
                effect = ITEM_STATUS_CHANGE;
        ...
        else if (SearchItemSlots(battlerItems, HOLD_EFFECT_FLAME_ORB))
        {
            ...
            {
                gLastUsedItem = SearchItemSlots(battlerItems, HOLD_EFFECT_FLAME_ORB);
                effect = ITEM_STATUS_CHANGE;
                gBattleMons[battler].status1 = STATUS1_BURN;
        ...
        else if (SearchItemSlots(battlerItems, HOLD_EFFECT_STICKY_BARB))  // Not an orb per se, but similar effect, and needs to NOT activate with pickpocket
        {
            if (battlerAbility != ABILITY_MAGIC_GUARD)
            {
                ...
                BattleScriptExecute(BattleScript_ItemHurtEnd2);
                effect = ITEM_HP_CHANGE;
                RecordItemEffectBattle(battler, HOLD_EFFECT_STICKY_BARB);
                PREPARE_ITEM_BUFFER(gBattleTextBuff1, gLastUsedItem);
            }
        }
...
bool32 CanBattlerAvoidContactEffects(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 move)
{
    ...
    return !IsMoveMakingContact(battlerAtk, battlerDef, abilityAtk, move);
}
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (gProtectStructs[battlerDef].protected != PROTECT_MAX_GUARD && !MoveIgnoresProtect(move))
    {
        ...
        if (GetBattlerAbility(battlerAtk) == ABILITY_UNSEEN_FIST
         && IsMoveMakingContact(battlerAtk, battlerDef, ABILITY_UNSEEN_FIST, move))
            return FALSE;
    }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_TOUGH_CLAWS:
        if (IsMoveMakingContact(battlerAtk, battlerDef, ctx->abilityAtk, ctx->move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (ctx->moveType == TYPE_FIRE && !IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ABILITY_NONE, ctx->move))
        {
...
{
    bool32 ret = TRUE;
    enum ItemHoldEffect holdEffect = HOLD_EFFECT_HEAVY_DUTY_BOOTS;

    if (toxicSpikes && BattlerHasHeldItemEffect(battler, holdEffect, TRUE) && !IS_BATTLER_OF_TYPE(battler, TYPE_POISON))
    {
        ret = FALSE;
...
{
    return gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerSide(battler)][itemSlot];
}
...
bool32 TryRestoreHPBerries(u32 battler, enum ItemCaseId caseId)
{
    if (BattlerHasBerry(battler)
     || GetBattlerHeldItemWithEffect(battler, HOLD_EFFECT_RESTORE_HP, TRUE))  // Edge case for Berry Juice
    {
