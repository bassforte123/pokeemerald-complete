...
u8 GetEggCyclesToSubtract(void)
{
    ...
    for (count = CalculatePlayerPartyCount(), i = 0; i < count; i++)
    {
        ...
        {
            if (MonHasTrait(&gParties[B_TRAINER_PLAYER][i], ABILITY_MAGMA_ARMOR)
             || MonHasTrait(&gParties[B_TRAINER_PLAYER][i], ABILITY_FLAME_BODY)
             || MonHasTrait(&gParties[B_TRAINER_PLAYER][i], ABILITY_STEAM_ENGINE))
                return 2;
        }
    }
    return 1;
