...
static void DisplayPartyPokemonDataForBattlePyramidHeldItem(u8 slot)
{
    ...
    {
        if (GetMonData(&gParties[B_TRAINER_PLAYER][slot], MON_DATA_HELD_ITEM + i) != ITEM_NONE)
        {
            hasitem = TRUE;
            break;
        }
    ...
    for (int i = MAX_MON_ITEMS - 1; i >= 0; i--)
    {
        if (GetMonData(mon, MON_DATA_HELD_ITEM + i) != ITEM_NONE)
        {
            ...
            else
            {
                ...
                TryItemHoldFormChange(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId], gPartyMenu.slotId);
                return TAKE_OK;
            }
        }
    }
...
static void CB2_GiveHoldItem(void)
{
    ...
    else
    {
        ...
        if (ItemIsMail(sPartyMenuItemId)
         || (MonHasMail(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId]) && ItemIsMail(gSpecialVar_ItemId))) // Cannot hold more than one mail
        {
...
static void Task_HandleSwitchItemsYesNoInput(u8 taskId)
{
    ...
    case 0: // Yes, switch items
        ...
        {
            SetMonData(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId], MON_DATA_HELD_ITEM + slot, &item); //remove old item first (Multi)
            GiveItemToMon(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId], gSpecialVar_ItemId);
...
static void CursorCb_Toss(u8 taskId)
{
    ...
    for (int i = MAX_MON_ITEMS - 1; i >= 0; i--)
    {
        if (GetMonData(mon, MON_DATA_HELD_ITEM + i) != ITEM_NONE)
        {
            item = GetMonData(mon, MON_DATA_HELD_ITEM + i); //Battle Pyramid toss when bag is full, targets last held item.
            break;
        }
    } 
...
{
    enum Item item = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM);
    ...
    case 0: // Yes, lose mail message
        for (int i = MAX_MON_ITEMS - 1; i >= 0; i--)
        {
            if (ItemIsMail(GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM + i)))
            {
...
static void CursorCb_Cancel2(u8 taskId)
{
    ...
    for (int i = MAX_MON_ITEMS - 1; i >= 0; i--)
    {
        if (ItemIsMail(GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM + i)))
        {
            item = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM + i); //Battle Pyramid toss when bag is full, targets last held item.
            break;
        }
    }
...
void DrawHeldItemIconsForTrade(u8 *partyCounts, u8 *partySpriteIds, u8 whichParty)
{
    ...
    case TRADE_PLAYER:
        for (i = 0; i < partyCounts[TRADE_PLAYER]; i++)
        {
            ...
            {
                item = GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_HELD_ITEM + j);
                if (item != ITEM_NONE)
                {
    ...
    case TRADE_PARTNER:
        for (i = 0; i < partyCounts[TRADE_PARTNER]; i++)
        {
            for (j = 0; j < MAX_MON_ITEMS; j++)
            {
                item = GetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_HELD_ITEM + j);
                if (item != ITEM_NONE)
                {
...
static u8 CheckBattleEntriesAndGetMessage(void)
{
    ...
    for (i = 0; i < maxBattlers - 1; i++)
    {
        ...
        for (j = i + 1; j < maxBattlers; j++)
        {
            ...
            {
                item = GetMonData(&party[order[i] - 1], MON_DATA_HELD_ITEM + k);
                if (item != ITEM_NONE)
                {
                    for (l = 0; l < MAX_MON_ITEMS; l++)
                    {
                        if (item == GetMonData(&party[order[j] - 1], MON_DATA_HELD_ITEM + l))
...
void DoBattlePyramidMonsHaveHeldItem(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < MAX_MON_ITEMS; j++)
        {
            if (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_HELD_ITEM + j) != ITEM_NONE)
            {
                gSpecialVar_Result = TRUE;
                break;
            }
