...
DOUBLE_BATTLE_TEST("End Turn Effects: First Event Block is executed correctly (Multi)")
{
    s16 healed;
    s16 damage;
    ...
    GIVEN {
        PLAYER(SPECIES_WYNAUT) { HP(100); Speed(1); }
        PLAYER(SPECIES_EKANS) { HP(100); Ability(ABILITY_SHED_SKIN); Status1(STATUS1_BURN); Speed(2); }
        OPPONENT(SPECIES_WYNAUT) { HP(100); Items(ITEM_POTION, ITEM_LEFTOVERS); Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(100); Items(ITEM_POTION, ITEM_BLACK_SLUDGE); Speed(4); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_GRASSY_TERRAIN); }
    } SCENE {
        MESSAGE("The opposing Wobbuffet is healed by the grassy terrain!");
