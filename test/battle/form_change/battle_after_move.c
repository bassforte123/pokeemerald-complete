...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Relic Song transformation activates after target faints (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_MELOETTA_ARIA);
...
SINGLE_BATTLE_TEST("Relic Song transforms Meloetta after Magician was activated (Traits)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Item(ITEM_POTION); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); MOVE(player, MOVE_RELIC_SONG); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SKILL_SWAP, opponent);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Relic Song transforms Meloetta after Magician was activated (Items)")
{
    GIVEN {
        PLAYER(SPECIES_MELOETTA_ARIA);
        OPPONENT(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); Items(ITEM_PECHA_BERRY, ITEM_POTION); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); MOVE(player, MOVE_RELIC_SONG); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SKILL_SWAP, opponent);
...
SINGLE_BATTLE_TEST("Relic Song transforms Meloetta before taking Life Orb damage (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_LIFE_ORB) == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_MELOETTA_ARIA) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_RELIC_SONG); }
    } SCENE {
