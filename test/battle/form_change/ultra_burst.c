...
DOUBLE_BATTLE_TEST("Ultra Burst's order is determined by Speed - opponent faster (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_NECROZMA_DAWN_WINGS) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); MOVE(playerLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Ultra Burst's order is determined by Speed - player faster (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(5); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_NECROZMA_DAWN_WINGS) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); MOVE(playerLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ultra Burst affects turn order (Multi)")
{
    GIVEN {
        ASSUME(B_MEGA_EVO_TURN_ORDER >= GEN_7);
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(105); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(106); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Ultra Burst happens after switching, but before Focus Punch-like Moves (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FOCUS_PUNCH) == EFFECT_FOCUS_PUNCH);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Ultra Burst and Mega Evolution can happen on the same turn (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_NUGGET, ITEM_ULTRANECROZIUM_Z); Speed(3); }
