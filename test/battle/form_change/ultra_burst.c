...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Dusk Mane Necrozma can Ultra Burst holding Ultranecrozium Z (Items)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Ultra Burst's order is determined by Speed - opponent faster (Items)")
{
    GIVEN {
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z); Speed(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_NECROZMA_DAWN_WINGS) { Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z); Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); MOVE(playerLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Ultra Burst's order is determined by Speed - player faster (Items)")
{
    GIVEN {
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z); Speed(5); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_NECROZMA_DAWN_WINGS) { Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z); Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); MOVE(playerLeft, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ultra Burst affects turn order (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MEGA_EVO_TURN_ORDER, GEN_7);
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z);}
...
DOUBLE_BATTLE_TEST("Ultra Burst happens after switching, but before Focus Punch-like Moves (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FOCUS_PUNCH) == EFFECT_FOCUS_PUNCH);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Ultra Burst and Mega Evolution can happen on the same turn (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Necrozma returns its proper Form upon battle end after Ultra Bursting (Items)")
{
    u32 species;
    PARAMETRIZE { species = SPECIES_NECROZMA_DUSK_MANE; }
    PARAMETRIZE { species = SPECIES_NECROZMA_DAWN_WINGS; }
...
SINGLE_BATTLE_TEST("Necrozma returns its proper Form upon fainting after Ultra Bursting (Items)")
{
    u32 species;
    PARAMETRIZE { species = SPECIES_NECROZMA_DUSK_MANE; }
    PARAMETRIZE { species = SPECIES_NECROZMA_DAWN_WINGS; }
