...
SINGLE_BATTLE_TEST("Xerneas returns its Neutral Form upon battle end")
{
    GIVEN {
        ...
        EXPECT_EQ(GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES), SPECIES_XERNEAS_NEUTRAL);
    }
}
...
SINGLE_BATTLE_TEST("Aegislash reverts to Shield Form upon battle end after using an attack")
{
    GIVEN {
        ...
        EXPECT_EQ(GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES), SPECIES_AEGISLASH_SHIELD);
    }
}
...
SINGLE_BATTLE_TEST("Terapagos reverts to the correct form upon battle end after terastallizing")
{
    GIVEN {
        ...
        EXPECT_EQ(GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES), SPECIES_TERAPAGOS_NORMAL);
    }
}
...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Palafin returns to Zero form upon battle end (Traits)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Wishiwashi reverts to Solo form upon battle end after changing form at the beginning of the battle (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WISHIWASHI_SOLO) { Level(100); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SCHOOLING); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Minior Meteor reverts to Core form upon battle end after changing form at the beginning of the battle (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_MINIOR_CORE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); HP(51); MaxHP(101); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); SEND_OUT(player, 1); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Mimikyu Busted reverts to Disguised form upon battle end after busting its Disguise in battle (Traits)")
{
    u32 species;
    PARAMETRIZE { species = SPECIES_MIMIKYU_DISGUISED; }
    PARAMETRIZE { species = SPECIES_MIMIKYU_TOTEM_DISGUISED; }
...
SINGLE_BATTLE_TEST("Cramorant reverts to base Form upon battle end after using Surf in battle (Traits)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Power Construct Zygarde reverts to its original form upon battle end (Traits)")
{
    u16 baseSpecies;
    PARAMETRIZE { baseSpecies = SPECIES_ZYGARDE_10_POWER_CONSTRUCT; }
    PARAMETRIZE { baseSpecies = SPECIES_ZYGARDE_50_POWER_CONSTRUCT; }
    ...
    GIVEN {
        PLAYER(baseSpecies)
        {
            Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POWER_CONSTRUCT);
            HP((GetMonData(&PLAYER_PARTY[0], MON_DATA_MAX_HP) / 2) + 1);
        }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Zacian returns its Hero Form upon battle end (Items)")
{
    GIVEN {
        PLAYER(SPECIES_ZACIAN_HERO) { Items(ITEM_PECHA_BERRY, ITEM_RUSTED_SWORD); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
SINGLE_BATTLE_TEST("Zacian returns its Hero Form upon battle end (Items)")
{
    GIVEN {
        ASSUME(GetMovePP(MOVE_BEHEMOTH_BLADE) == 5);
        PLAYER(SPECIES_ZACIAN_HERO) { Items(ITEM_PECHA_BERRY, ITEM_RUSTED_SWORD); Moves(MOVE_IRON_HEAD, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
SINGLE_BATTLE_TEST("Zamazenta returns its Hero Form upon battle end (Items)")
{
    GIVEN {
        PLAYER(SPECIES_ZAMAZENTA_HERO) { Items(ITEM_PECHA_BERRY, ITEM_RUSTED_SHIELD); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
SINGLE_BATTLE_TEST("Zamazenta returns its Hero Form upon battle end (Items)")
{
    GIVEN {
        ASSUME(GetMovePP(MOVE_BEHEMOTH_BASH) == 5);
        PLAYER(SPECIES_ZAMAZENTA_HERO) { Items(ITEM_PECHA_BERRY, ITEM_RUSTED_SHIELD); Moves(MOVE_IRON_HEAD, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
SINGLE_BATTLE_TEST("Ogerpon reverts to the correct form upon battle end after terastallizing (Items)")
{
    u32 species, item;
    PARAMETRIZE { species = SPECIES_OGERPON_TEAL;        item = ITEM_NONE; }
    PARAMETRIZE { species = SPECIES_OGERPON_WELLSPRING;  item = ITEM_WELLSPRING_MASK; }
