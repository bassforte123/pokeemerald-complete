...
SINGLE_BATTLE_TEST("Zacian changes into its Crowned Form when holding the Rusted Sword upon battle start (Multi)")
{
    ...
    PARAMETRIZE { item = ITEM_RUSTED_SWORD; }
    GIVEN {
        PLAYER(SPECIES_ZACIAN_HERO) { Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ZACIAN_HERO) { Items(ITEM_NUGGET, ITEM_RUSTED_SWORD); Moves(MOVE_IRON_HEAD, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
SINGLE_BATTLE_TEST("Zamazenta changes into its Crowned Form when holding the Rusted Shield upon battle start (Multi)")
{
    ...
    PARAMETRIZE { item = ITEM_RUSTED_SHIELD; }
    GIVEN {
        PLAYER(SPECIES_ZAMAZENTA_HERO) { Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
...
{
    GIVEN {
        PLAYER(SPECIES_ZAMAZENTA_HERO) { Items(ITEM_NUGGET, ITEM_RUSTED_SHIELD); Moves(MOVE_IRON_HEAD, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } THEN {
