...
SINGLE_BATTLE_TEST("Primal reversion happens for Groudon only when holding Red Orb (Multi)")
{
    ...
    PARAMETRIZE { heldItem = ITEM_BLUE_ORB;}
    GIVEN {
        PLAYER(SPECIES_GROUDON) { Items(ITEM_NUGGET, heldItem); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Primal reversion happens for Kyogre only when holding Blue Orb (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_KYOGRE) { Items(ITEM_NUGGET, heldItem); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
        if (heldItem == ITEM_BLUE_ORB) {
...
{
    GIVEN {
        PLAYER(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); Speed(5); };
        PLAYER(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); Speed(15); };
        OPPONENT(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); Speed(10); }
        OPPONENT(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); Speed(20); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_PRIMAL_REVERSION, opponentRight);
...
{
    GIVEN {
        PLAYER(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); Speed(20); };
        PLAYER(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); Speed(30); };
        OPPONENT(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); Speed(10); }
        OPPONENT(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); Speed(2); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_CELEBRATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_PRIMAL_REVERSION, playerRight);
...
SINGLE_BATTLE_TEST("Primal reversion happens after a mon is sent out after a mon is fainted (Multi)")
{
    GIVEN {
        ASSUME(!IsBattleMoveStatus(MOVE_SCRATCH));
        PLAYER(SPECIES_WOBBUFFET) {HP(1); }
        PLAYER(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); SEND_OUT(player, 1); }
        TURN { MOVE(opponent, MOVE_SCRATCH); }
...
DOUBLE_BATTLE_TEST(" Primal reversion triggers for multiple battlers if multiple fainted the previous turn (Multi)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); }
        OPPONENT(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_EARTHQUAKE);
               SEND_OUT(opponentRight, 3);
               SEND_OUT(opponentLeft, 2);
...
DOUBLE_BATTLE_TEST("Primal reversion triggers for all battlers if multiple fainted the previous turn (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CATERPIE) { HP(1); }
        PLAYER(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); }
        PLAYER(SPECIES_GROUDON) { Items(ITEM_NUGGET, ITEM_RED_ORB); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_KYOGRE) { Items(ITEM_NUGGET, ITEM_BLUE_ORB); }
