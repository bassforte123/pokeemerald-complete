...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Primal reversion happens for Groudon only when holding Red Orb (Items)")
{
    u16 heldItem;
    PARAMETRIZE { heldItem = ITEM_NONE;}
    PARAMETRIZE { heldItem = ITEM_RED_ORB;}
...
SINGLE_BATTLE_TEST("Primal reversion happens for Kyogre only when holding Blue Orb (Items)")
{
    u16 heldItem;
    PARAMETRIZE { heldItem = ITEM_NONE;}
    PARAMETRIZE { heldItem = ITEM_RED_ORB;}
...
DOUBLE_BATTLE_TEST("Primal reversion's order is determined by Speed - opponent faster (Items)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Primal reversion's order is determined by Speed - player faster (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Primal reversion happens after a mon is sent out after a mon is fainted (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_WOBBUFFET) {HP(1); }
...
SINGLE_BATTLE_TEST("Primal reversion happens after a mon is switched in (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GROUDON) { Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
...
SINGLE_BATTLE_TEST("Primal reversion happens after a switch-in caused by Eject Button (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
...
SINGLE_BATTLE_TEST("Primal reversion happens after a switch-in caused by Red Card (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
        ASSUME(gItemsInfo[ITEM_RED_CARD].holdEffect == HOLD_EFFECT_RED_CARD);
...
SINGLE_BATTLE_TEST("Primal reversion happens after the entry hazards damage (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SPIKES) == EFFECT_SPIKES);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Primal reversion happens immediately if it was brought in by U-turn (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GROUDON) { Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
...
DOUBLE_BATTLE_TEST("Primal reversion triggers for multiple battlers if multiple fainted the previous turn (Items)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_EARTHQUAKE) == TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Primal reversion triggers for all battlers if multiple fainted the previous turn (Items)")
{
    GIVEN {
        ASSUME(IsExplosionMove(MOVE_EXPLOSION));
        ASSUME(GetMoveTarget(MOVE_EXPLOSION) == TARGET_FOES_AND_ALLY);
...
DOUBLE_BATTLE_TEST("Primal reversion and other switch-in effects trigger for all battlers if multiple fainted the previous turn (Items)")
{
    GIVEN {
        ASSUME(IsExplosionMove(MOVE_EXPLOSION));
        ASSUME(GetMoveTarget(MOVE_EXPLOSION) == TARGET_FOES_AND_ALLY);
...
SINGLE_BATTLE_TEST("Primal Reversion is reverted upon battle end (Items)")
{
    u32 species, item;
    PARAMETRIZE { species = SPECIES_GROUDON; item = ITEM_RED_ORB; }
    PARAMETRIZE { species = SPECIES_KYOGRE;  item = ITEM_BLUE_ORB; }
...
SINGLE_BATTLE_TEST("Primal Reversion is NOT reverted upon fainting (Items)")
{
    u32 species, item, targetSpecies;
    PARAMETRIZE { species = SPECIES_GROUDON; item = ITEM_RED_ORB;  targetSpecies = SPECIES_GROUDON_PRIMAL; }
    PARAMETRIZE { species = SPECIES_KYOGRE;  item = ITEM_BLUE_ORB; targetSpecies = SPECIES_KYOGRE_PRIMAL;  }
