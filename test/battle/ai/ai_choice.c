...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon only consider their own status moves when determining if they should switch (Multi)")
{
    GIVEN
    {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_RISKY | AI_FLAG_SMART_SWITCHING | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_MON_CHOICES);
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use stat boosting moves (Multi)")
{
    // Moves defined by MOVE_TARGET_USER (with exceptions?)
    u32 j, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they are the only party member (Multi)")
{
    u32 j, isAlive = 0, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
    static const u32 choiceItems[] = {
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they don't have a good switchin (Multi)")
{
    u32 j, move = MOVE_NONE, species = SPECIES_NONE, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
    static const u32 choiceItems[] = {
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they are trapped (Multi)")
{
    u32 j, aiAbility = ABILITY_NONE, playerAbility = MOVE_NONE, species = SPECIES_NONE, heldItem = ITEM_NONE;
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will switch if locked into a move the player is immune to (Multi)")
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GASTLY, 0) == TYPE_GHOST);
        ASSUME(GetMoveType(MOVE_SURF) == TYPE_WATER);
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will only see choiced moves when considering switching with ShouldSwitchIfHasBadOdds (Multi)")
{
    PASSES_RANDOMLY(SHOULD_SWITCH_HASBADODDS_PERCENTAGE, 100, RNG_AI_SWITCH_HASBADODDS);
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GASTLY, 0) == TYPE_GHOST);
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will only see choiced moves when considering switching with FindMonThatAbsorbsMove (Multi)")
{
    PASSES_RANDOMLY(SHOULD_SWITCH_ABSORBS_MOVE_PERCENTAGE, 100, RNG_AI_SWITCH_ABSORBING);
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_SANDSHREW, 0) == TYPE_GROUND);
