...
AI_DOUBLE_BATTLE_TEST("Choiced Pokémon won't switch out if they can still affect one opposing Pokémon in doubles (Traits)")
{
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_RISKY | AI_FLAG_SMART_SWITCHING | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_MON_CHOICES);
        PLAYER(SPECIES_CHARMANDER) { Level(5); Moves(MOVE_CELEBRATE); }
        PLAYER(SPECIES_VAPOREON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_WATER_ABSORB); Moves(MOVE_CELEBRATE); }
        PLAYER(defendingSpecies) { Ability(ABILITY_ADAPTABILITY); Innates(defendingAbility); SpDefense(500); Moves(MOVE_CELEBRATE); }
        OPPONENT(SPECIES_VAPOREON) { Moves(MOVE_SCALD); Item(ITEM_CHOICE_SPECS); }
        OPPONENT(SPECIES_VAPOREON) { Moves(MOVE_SCALD); Item(ITEM_CHOICE_SPECS); }
        OPPONENT(SPECIES_ZIGZAGOON);
        OPPONENT(SPECIES_ZIGZAGOON);
...
AI_DOUBLE_BATTLE_TEST("Choiced Pokémon won't switch out if they can still affect one opposing Pokémon in doubles (reversed) (Traits)")
{
    u32 defendingSpecies = SPECIES_NONE;
    enum Ability defendingAbility = ABILITY_NONE;
    PARAMETRIZE { defendingSpecies = SPECIES_VAPOREON; defendingAbility = ABILITY_WATER_ABSORB; }
...
#if MAX_MON_ITEMS > 1
AI_SINGLE_BATTLE_TEST("Choiced Pokémon switch out after using a status move once (Items)")
{
    u32 j, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_RHYDON)
        OPPONENT(SPECIES_LOPUNNY) { Moves(MOVE_YAWN, MOVE_SCRATCH); Items(ITEM_PECHA_BERRY, heldItem); Ability(ability); }
        OPPONENT(SPECIES_SWAMPERT) { Moves(MOVE_WATERFALL); }
    } WHEN {
        TURN { EXPECT_MOVE(opponent, MOVE_YAWN); }
        if (ability == ABILITY_KLUTZ) { // Klutz ignores item
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon only consider their own status moves when determining if they should switch (Items)")
{
    GIVEN
    {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_RISKY | AI_FLAG_SMART_SWITCHING | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_MON_CHOICES);
        PLAYER(SPECIES_ZIGZAGOON) { Speed(4); Moves(MOVE_TAIL_WHIP, MOVE_SCRATCH); }
        OPPONENT(SPECIES_ZIGZAGOON) { Speed(5); Moves(MOVE_SCRATCH); Items(ITEM_PECHA_BERRY, ITEM_CHOICE_BAND); }
        OPPONENT(SPECIES_ZIGZAGOON) { Speed(5); Moves(MOVE_SCRATCH); }
    } WHEN {
        TURN { EXPECT_MOVE(opponent, MOVE_SCRATCH); MOVE(player, MOVE_TAIL_WHIP); }
        TURN { EXPECT_MOVE(opponent, MOVE_SCRATCH); MOVE(player, MOVE_TAIL_WHIP); }
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use stat boosting moves (Items)")
{
    // Moves defined by TARGET_USER (with exceptions?)
    u32 j, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_RHYDON)
        OPPONENT(SPECIES_LOPUNNY) { Moves(MOVE_SWORDS_DANCE, MOVE_SCRATCH); Items(ITEM_PECHA_BERRY, heldItem); Ability(ability); }
        OPPONENT(SPECIES_SWAMPERT) { Moves(MOVE_WATERFALL); }
    } WHEN {
        if (ability == ABILITY_KLUTZ) { // Klutz ignores item
            TURN { EXPECT_MOVE(opponent, MOVE_SWORDS_DANCE); }
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they are the only party member (Items)")
{
    u32 j, isAlive = 0, heldItem = ITEM_NONE;
    enum Ability ability = ABILITY_NONE;
    static const u32 choiceItems[] = {
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they don't have a good switchin (Items)")
{
    u32 j;
    enum Move move = MOVE_NONE;
    u32 species = SPECIES_NONE;
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon won't use status move if they are trapped (Items)")
{
    u32 j;
    enum Ability aiAbility = ABILITY_NONE, playerAbility = ABILITY_NONE;
    u32 species = SPECIES_NONE;
    ...
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(species) { Ability(playerAbility); }
        OPPONENT(SPECIES_LOPUNNY) { Moves(MOVE_YAWN, MOVE_SCRATCH); Items(ITEM_PECHA_BERRY, heldItem); Ability(aiAbility); }
        OPPONENT(SPECIES_SWAMPERT) { Moves(MOVE_WATERFALL); }
    } WHEN {
        if (playerAbility != ABILITY_ARENA_TRAP || aiAbility == ABILITY_KLUTZ) {
            TURN { EXPECT_MOVE(opponent, MOVE_YAWN); }
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will switch if locked into a move the player is immune to (Items)")
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GASTLY, 0) == TYPE_GHOST);
        ASSUME(GetMoveType(MOVE_SURF) == TYPE_WATER);
        ...
        PLAYER(SPECIES_GASTLY) { Level(1); Moves(MOVE_CELEBRATE); }
        PLAYER(SPECIES_VAPOREON) { Ability(ABILITY_WATER_ABSORB); Moves(MOVE_SURF); }
        OPPONENT(SPECIES_ZIGZAGOON) { Items(ITEM_PECHA_BERRY, ITEM_CHOICE_BAND); Moves(MOVE_SURF, MOVE_BODY_SLAM); }
        OPPONENT(SPECIES_ZIGZAGOON) { Items(ITEM_PECHA_BERRY, ITEM_CHOICE_BAND); Moves(MOVE_SURF, MOVE_BODY_SLAM); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); EXPECT_MOVE(opponent, MOVE_SURF); SEND_OUT(player, 1); }
        TURN { MOVE(player, MOVE_SURF); EXPECT_SWITCH(opponent, 1); }
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will only see choiced moves when considering switching with ShouldSwitchIfHasBadOdds (Items)")
{
    PASSES_RANDOMLY(SHOULD_SWITCH_HASBADODDS_PERCENTAGE, 100, RNG_AI_SWITCH_HASBADODDS);
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GASTLY, 0) == TYPE_GHOST);
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES);
        PLAYER(SPECIES_GASTLY) { Level(1); Moves(MOVE_CELEBRATE); }
        PLAYER(SPECIES_ZIGZAGOON) { Items(ITEM_PECHA_BERRY, ITEM_CHOICE_BAND); Moves(MOVE_CLOSE_COMBAT); }
        OPPONENT(SPECIES_ZIGZAGOON) { Items(ITEM_PECHA_BERRY, ITEM_CHOICE_BAND); Moves(MOVE_SURF, MOVE_CLOSE_COMBAT); }
        OPPONENT(SPECIES_BRONZONG) { Moves(MOVE_CLOSE_COMBAT); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); EXPECT_MOVE(opponent, MOVE_SURF); SEND_OUT(player, 1); }
        TURN { MOVE(player, MOVE_CLOSE_COMBAT); EXPECT_SWITCH(opponent, 1); }
...
AI_SINGLE_BATTLE_TEST("Choiced Pokémon will only see choiced moves when considering switching with FindMonThatAbsorbsMove (Items)")
{
    PASSES_RANDOMLY(SHOULD_SWITCH_ABSORBS_MOVE_PERCENTAGE, 100, RNG_AI_SWITCH_ABSORBING);
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_SANDSHREW, 0) == TYPE_GROUND);
...
AI_DOUBLE_BATTLE_TEST("Choiced Pokémon won't switch out if they can still affect one opposing Pokémon in doubles (Items)")
{
    u32 defendingSpecies = SPECIES_NONE;
    enum Ability defendingAbility = ABILITY_NONE;
    PARAMETRIZE { defendingSpecies = SPECIES_VAPOREON; defendingAbility = ABILITY_WATER_ABSORB; }
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_VAPOREON) { Ability(ABILITY_WATER_ABSORB); Moves(MOVE_CELEBRATE); }
        PLAYER(defendingSpecies) { Ability(defendingAbility); SpDefense(500); Moves(MOVE_CELEBRATE); }
        OPPONENT(SPECIES_VAPOREON) { Moves(MOVE_SCALD); Items(ITEM_PECHA_BERRY, ITEM_CHOICE_SPECS); }
        OPPONENT(SPECIES_VAPOREON) { Moves(MOVE_SCALD); Items(ITEM_PECHA_BERRY, ITEM_CHOICE_SPECS); }
        OPPONENT(SPECIES_ZIGZAGOON);
        OPPONENT(SPECIES_ZIGZAGOON);
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("Choiced Pokémon won't switch out if they can still affect one opposing Pokémon in doubles (reversed) (Items)")
{
    u32 defendingSpecies = SPECIES_NONE;
    enum Ability defendingAbility = ABILITY_NONE;
    PARAMETRIZE { defendingSpecies = SPECIES_VAPOREON; defendingAbility = ABILITY_WATER_ABSORB; }
