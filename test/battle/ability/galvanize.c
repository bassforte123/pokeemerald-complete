...
SINGLE_BATTLE_TEST("Galvanize turns a normal type move into Electric (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_KRABBY);
        OPPONENT(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_STURDY); Innates(ABILITY_GALVANIZE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, opponent);
...
SINGLE_BATTLE_TEST("Galvanize boosts power of affected moves by 20% (Gen7+) or 30% (Gen1-6) (Traits)", s16 damage)
{
    ...
    GIVEN {
        WITH_CONFIG(B_ATE_MULTIPLIER, genConfig);
        PLAYER(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_MAGNET_PULL); Innates(ability); Moves(MOVE_TACKLE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Galvanize doesn't affect Weather Ball's type (Traits)", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveType(MOVE_WEATHER_BALL) == TYPE_NORMAL);
        ASSUME(GetSpeciesType(SPECIES_PINSIR, 0) == TYPE_BUG);
        PLAYER(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_MAGNET_PULL); Innates(ability); }
        OPPONENT(SPECIES_PINSIR);
    } WHEN {
        TURN { MOVE(player, move); }
        TURN { MOVE(player, MOVE_WEATHER_BALL); }
...
SINGLE_BATTLE_TEST("Galvanize doesn't affect Judgment / Techno Blast / Multi-Attack's type (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_WATER_MEMORY].secondaryId == TYPE_WATER);
        ASSUME(GetSpeciesType(SPECIES_VAPOREON, 0) == TYPE_WATER);
        PLAYER(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_MAGNET_PULL); Innates(ABILITY_GALVANIZE); Item(item); }
        OPPONENT(SPECIES_VAPOREON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_WATER_ABSORB); }
    } WHEN {
        TURN { MOVE(player, move); }
    } SCENE {
        NOT { ANIMATION(ANIM_TYPE_MOVE, move, player); }
...
SINGLE_BATTLE_TEST("Galvanize doesn't affect Hidden Power's type (Traits)")
{
    GIVEN {
        ...
        ASSUME(gTypesInfo[TYPE_ELECTRIC].isHiddenPowerType == TRUE);
        ASSUME(GetSpeciesType(SPECIES_DIGLETT, 0) == TYPE_GROUND);
        PLAYER(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_MAGNET_PULL); Innates(ABILITY_GALVANIZE); HPIV(31); AttackIV(31); DefenseIV(31); SpAttackIV(30); SpDefenseIV(31); SpeedIV(30); } // HP Water
        OPPONENT(SPECIES_VAPOREON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_WATER_ABSORB); }
    } WHEN {
        TURN { MOVE(player, MOVE_HIDDEN_POWER); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Galvanize doesn't affect Natural Gift's type (Items)")
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_STURDY; }
    PARAMETRIZE { ability = ABILITY_GALVANIZE; }
...
SINGLE_BATTLE_TEST("Galvanize doesn't affect Judgment / Techno Blast / Multi-Attack's type (Items)")
{
    enum Move move;
    enum Item item;
    PARAMETRIZE { move = MOVE_JUDGMENT; item = ITEM_SPLASH_PLATE; }
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_WATER_MEMORY].secondaryId == TYPE_WATER);
        ASSUME(GetSpeciesType(SPECIES_VAPOREON, 0) == TYPE_WATER);
        PLAYER(SPECIES_GEODUDE_ALOLA) { Ability(ABILITY_GALVANIZE); Items(ITEM_GREAT_BALL, item); }
        OPPONENT(SPECIES_VAPOREON) { Ability(ABILITY_WATER_ABSORB); }
    } WHEN {
        TURN { MOVE(player, move); }
    } SCENE {
