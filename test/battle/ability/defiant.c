...
DOUBLE_BATTLE_TEST("Defiant sharply raises player's Attack after Intimidate (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(abilityLeft); }
        PLAYER(SPECIES_PRIMEAPE) { Ability(ABILITY_ANGER_POINT); Innates(abilityRight); }
...
DOUBLE_BATTLE_TEST("Defiant sharply raises opponent's Attack after Intimidate (Traits)")
{
    ...
    GIVEN {
        OPPONENT(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(abilityLeft); }
        OPPONENT(SPECIES_PRIMEAPE) { Ability(ABILITY_ANGER_POINT); Innates(abilityRight); }
...
DOUBLE_BATTLE_TEST("Defiant is activated by Cotton Down for non-ally Pokémon (Traits)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, playerLeft);
...
SINGLE_BATTLE_TEST("Defiant activates when opposing Syrup Bomb lowers Speed at end of turn (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); }
...
DOUBLE_BATTLE_TEST("Defiant doesn't activate when partner's Syrup Bomb lowers Speed at end of turn (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); }
...
DOUBLE_BATTLE_TEST("Defiant activates when opposing Octolock lowers stats at end of turn (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_OCTOLOCK) == EFFECT_OCTOLOCK);
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); }
...
DOUBLE_BATTLE_TEST("Defiant doesn't activate when partner's Octolock lowers stats at end of turn (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_OCTOLOCK) == EFFECT_OCTOLOCK);
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); }
...
SINGLE_BATTLE_TEST("Defiant activates before White Herb (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_MANKEY) { Ability(ABILITY_ANGER_POINT); Innates(ABILITY_DEFIANT); Item(ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Defiant activates before White Herb (Items)")
{
    enum Move move;
