...
SINGLE_BATTLE_TEST("Guard Dog raises Attack before Adrenaline Orb when Intimidated (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ADRENALINE_ORB].holdEffect == HOLD_EFFECT_ADRENALINE_ORB);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); Item(ITEM_ADRENALINE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Guard Dog still raises Attack against Intimidate when holding Clear Amulet (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_CLEAR_AMULET].holdEffect == HOLD_EFFECT_CLEAR_AMULET);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); Item(ITEM_CLEAR_AMULET); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { SWITCH(opponent, 1); }
...
SINGLE_BATTLE_TEST("Guard Dog does not activate if Intimidate is blocked by Substitute (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SUBSTITUTE) == EFFECT_SUBSTITUTE);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); }
...
SINGLE_BATTLE_TEST("Guard Dog does not activate if Intimidate is blocked by Mist (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MIST) == EFFECT_MIST);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); }
...
SINGLE_BATTLE_TEST("Guard Dog does not activate if Intimidate cannot lower Attack at minimum stage (Traits)")
{
    GIVEN {
        ASSUME_STAT_CHANGE(MOVE_CHARM, attack: -2);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); }
...
DOUBLE_BATTLE_TEST("Guard Dog does not activate if Intimidate is blocked by Flower Veil (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FORESTS_CURSE) == EFFECT_THIRD_TYPE);
        ASSUME(GetMoveArgType(MOVE_FORESTS_CURSE) == TYPE_GRASS);
        PLAYER(SPECIES_COMFEY) { Ability(ABILITY_NATURAL_CURE); Innates(ABILITY_FLOWER_VEIL); Speed(40); }
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); Speed(30); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); Speed(5); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_FORESTS_CURSE, target: playerRight); }
        TURN { SWITCH(opponentRight, 2); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Guard Dog activates before Flower Veil if it has higher unmodified Speed (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FORESTS_CURSE) == EFFECT_THIRD_TYPE);
        ASSUME(GetMoveArgType(MOVE_FORESTS_CURSE) == TYPE_GRASS);
        PLAYER(SPECIES_COMFEY) { Ability(ABILITY_NATURAL_CURE); Innates(ABILITY_FLOWER_VEIL); Speed(30); }
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_TOXIC_CHAIN); Innates(ABILITY_GUARD_DOG); Speed(40); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); Speed(5); }
    } WHEN {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Guard Dog raises Attack before Adrenaline Orb when Intimidated (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ADRENALINE_ORB].holdEffect == HOLD_EFFECT_ADRENALINE_ORB);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_GUARD_DOG); Items(ITEM_NUGGET, ITEM_ADRENALINE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { SWITCH(opponent, 1); }
...
SINGLE_BATTLE_TEST("Guard Dog still raises Attack against Intimidate when holding Clear Amulet (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_CLEAR_AMULET].holdEffect == HOLD_EFFECT_CLEAR_AMULET);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_GUARD_DOG); Items(ITEM_NUGGET, ITEM_CLEAR_AMULET); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { SWITCH(opponent, 1); }
