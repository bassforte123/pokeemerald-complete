...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Infiltrator bypasses the opponent's Light Screen/Reflect/Aurora Veil (Traits)", s16 damage)
{
    u32 screenMove, attackingMove, ability;
    ...
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_WATER_GUN) == DAMAGE_CATEGORY_SPECIAL);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_DRAGAPULT) { Ability(ABILITY_CLEAR_BODY); Innates(ability); }
        OPPONENT(SPECIES_ABOMASNOW) { Ability(ABILITY_SOUNDPROOF); Innates(ABILITY_SNOW_WARNING); }
    } WHEN {
        TURN { MOVE(opponent, screenMove); MOVE(player, attackingMove); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, screenMove, opponent);
...
DOUBLE_BATTLE_TEST("Infiltrator doesn't bypass an ally's Light Screen/Reflect/Aurora Veil (Traits)", s16 damage)
{
    u32 screenMove, attackingMove, ability;
...
SINGLE_BATTLE_TEST("Infiltrator bypasses the opponent's Mist (Traits)")
{
    enum Ability ability;
...
DOUBLE_BATTLE_TEST("Infiltrator doesn't bypass an ally's Mist (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MIST) == EFFECT_MIST);
        ASSUME_STAT_CHANGE(MOVE_SCREECH, defense: -2);
        PLAYER(SPECIES_DRAGAPULT) { Ability(ABILITY_CLEAR_BODY); Innates(ABILITY_INFILTRATOR); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
SINGLE_BATTLE_TEST("Infiltrator bypasses the opponent's Safeguard (Traits)")
{
    enum Ability ability;
...
DOUBLE_BATTLE_TEST("Infiltrator doesn't bypass an ally's Safeguard - Thunder Wave (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SAFEGUARD) == EFFECT_SAFEGUARD);
        ASSUME(GetMoveEffect(MOVE_THUNDER_WAVE) == EFFECT_NON_VOLATILE_STATUS);
...
DOUBLE_BATTLE_TEST("Infiltrator doesn't bypass an ally's Safeguard - Confuse (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SAFEGUARD) == EFFECT_SAFEGUARD);
        ASSUME(GetMoveEffect(MOVE_CONFUSE_RAY) == EFFECT_CONFUSE);
...
SINGLE_BATTLE_TEST("Infiltrator bypasses the opponent's Substitute (Gen 6+) (Traits)")
{
    enum Ability ability; u32 config;
...
DOUBLE_BATTLE_TEST("Infiltrator bypasses an ally's Substitute (Gen 6+) (Traits)")
{
    enum Ability ability; u32 config;
...
SINGLE_BATTLE_TEST("Infiltrator doesn't ignore a battler's Substitute when using Transform or Sky Drop (Traits)")
{
    enum Ability ability; enum Move move;
