...
DOUBLE_BATTLE_TEST("Symbiosis does not transfer its item after an ally's Eject Button activates (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_TELEPATHY); Innates(ABILITY_SYMBIOSIS); Item(ITEM_POTION); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Symbiosis does not transfer its item after an ally's Eject Pack activates (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_CHARM, attack: -2);
...
DOUBLE_BATTLE_TEST("Symbiosis triggers after an ally's Cheek Pouch activates (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SUPER_FANG) == EFFECT_FIXED_PERCENT_DAMAGE);
        ASSUME(gItemsInfo[ITEM_ORAN_BERRY].holdEffect == HOLD_EFFECT_RESTORE_HP);
        ASSUME(gItemsInfo[ITEM_ORAN_BERRY].holdEffectParam == 10);
        PLAYER(SPECIES_DEDENNE) { Ability(ABILITY_PLUS); Innates(ABILITY_CHEEK_POUCH); MaxHP(60); HP(31); Item(ITEM_ORAN_BERRY); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_TELEPATHY); Innates(ABILITY_SYMBIOSIS); Item(ITEM_POTION); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Symbiosis transfers its item to an ally after it consumes an item (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TRICK_ROOM) == EFFECT_TRICK_ROOM);
        ASSUME(gItemsInfo[ITEM_ROOM_SERVICE].holdEffect == HOLD_EFFECT_ROOM_SERVICE);
        ASSUME(gItemsInfo[ITEM_TOXIC_ORB].holdEffect == HOLD_EFFECT_TOXIC_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ROOM_SERVICE); }
...
DOUBLE_BATTLE_TEST("Symbiosis triggers after partners berry eaten from bug bite (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_BUG_BITE, MOVE_EFFECT_BUG_BITE));
        ASSUME(gItemsInfo[ITEM_LIECHI_BERRY].holdEffect == HOLD_EFFECT_ATTACK_UP);
        ASSUME(gItemsInfo[ITEM_TOXIC_ORB].holdEffect == HOLD_EFFECT_TOXIC_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIECHI_BERRY); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_SYMBIOSIS); Items(ITEM_PECHA_BERRY, ITEM_TOXIC_ORB); }
        OPPONENT(SPECIES_STARAVIA);
        OPPONENT(SPECIES_SHUCKLE);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Symbiosis triggers after partner bestows its item (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_BESTOW) == EFFECT_BESTOW);
        ASSUME(gItemsInfo[ITEM_FLAME_ORB].holdEffect == HOLD_EFFECT_FLAME_ORB);
...
DOUBLE_BATTLE_TEST("Symbiosis transfers its item to an ally after it consumes a weakness berry (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_CHILAN_BERRY].holdEffect == HOLD_EFFECT_RESIST_BERRY);
        ASSUME(gItemsInfo[ITEM_TOXIC_ORB].holdEffect == HOLD_EFFECT_TOXIC_ORB);
...
DOUBLE_BATTLE_TEST("Symbiosis transfers its item after Gem consumption and move execution (Gen7+) (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_NORMAL_GEM) == HOLD_EFFECT_GEMS);
        WITH_CONFIG(B_SYMBIOSIS_GEMS, GEN_7);
...
DOUBLE_BATTLE_TEST("Symbiosis transfers its item after Gem consumption, but before move execution (Gen6) (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_NORMAL_GEM) == HOLD_EFFECT_GEMS);
        WITH_CONFIG(B_SYMBIOSIS_GEMS, GEN_6);
...
DOUBLE_BATTLE_TEST("Symbiosis does not transfer its item after an ally's Eject Button activates (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_SYMBIOSIS); Items(ITEM_PECHA_BERRY, ITEM_POTION); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Symbiosis does not transfer its item after an ally's Eject Pack activates (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_CHARM, attack: -2);
...
DOUBLE_BATTLE_TEST("Symbiosis triggers after an ally's Cheek Pouch activates (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SUPER_FANG) == EFFECT_FIXED_PERCENT_DAMAGE);
        ASSUME(gItemsInfo[ITEM_ORAN_BERRY].holdEffect == HOLD_EFFECT_RESTORE_HP);
        ASSUME(gItemsInfo[ITEM_ORAN_BERRY].holdEffectParam == 10);
        PLAYER(SPECIES_DEDENNE) { Ability(ABILITY_CHEEK_POUCH); MaxHP(60); HP(31); Items(ITEM_PECHA_BERRY, ITEM_ORAN_BERRY); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_SYMBIOSIS); Items(ITEM_PECHA_BERRY, ITEM_POTION); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
