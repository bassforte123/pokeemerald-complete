...
DOUBLE_BATTLE_TEST("Competitive sharply raises player's Sp. Atk after Intimidate (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(abilityLeft); }
        PLAYER(SPECIES_JIGGLYPUFF) { Ability(ABILITY_CUTE_CHARM); Innates(abilityRight); }
...
DOUBLE_BATTLE_TEST("Competitive sharply raises opponent's Sp. Atk after Intimidate (Traits)")
{
    ...
    GIVEN {
        OPPONENT(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(abilityLeft); }
        OPPONENT(SPECIES_JIGGLYPUFF) { Ability(ABILITY_CUTE_CHARM); Innates(abilityRight); }
...
DOUBLE_BATTLE_TEST("Competitive is activated by Cotton Down for non-ally pokemon (Traits)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, playerLeft);
...
SINGLE_BATTLE_TEST("Competitive activates when opposing Syrup Bomb lowers Speed at end of turn (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); }
...
DOUBLE_BATTLE_TEST("Competitive doesn't activate when partner's Syrup Bomb lowers Speed at end of turn (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); }
...
DOUBLE_BATTLE_TEST("Competitive activates when opposing Octolock lowers stats at end of turn (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_OCTOLOCK) == EFFECT_OCTOLOCK);
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); }
...
DOUBLE_BATTLE_TEST("Competitive doesn't activate when partner's Octolock lowers stats at end of turn (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_OCTOLOCK) == EFFECT_OCTOLOCK);
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); }
...
SINGLE_BATTLE_TEST("Competitive activates before White Herb (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_IGGLYBUFF) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_COMPETITIVE); Item(ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Competitive activates before White Herb (Items)")
{
    enum Move move;
