...
// Listed on Bulbapedia as "Moves that target all Pokémon (except Perish Song, Flower Shield, and Rototiller),"
// Despite the fact that there's only 2 remaining moves from that list, being Haze and Teatime
SINGLE_BATTLE_TEST("Dazzling, Queenly Majesty and Armor Tail do not block Haze (Traits)")
{
    u32 species;
    enum Ability ability;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_HAZE) == EFFECT_HAZE);
        PLAYER(SPECIES_MURKROW) { Ability(ABILITY_INSOMNIA); Innates(ABILITY_PRANKSTER); }
...
SINGLE_BATTLE_TEST("Dazzling, Queenly Majesty and Armor Tail do not block Teatime (Traits)")
{
    u32 species;
    enum Ability ability;
    ...
    GIVEN {
        ...
        OPPONENT(species) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(ITEM_ORAN_BERRY); HP(1); MaxHP(100); }
    } WHEN {
        TURN { MOVE(player, MOVE_TEATIME); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_TEATIME, player);
...
SINGLE_BATTLE_TEST("Dazzling, Queenly Majesty and Armor Tail do not block a move's Z-Status effect (Traits)")
{
    u32 species;
    enum Ability ability;
...
SINGLE_BATTLE_TEST("Mold Breaker ignores Dazzling, Queenly Majesty and Armor Tail (Traits)")
{
    u32 species;
    enum Ability ability;
...
DOUBLE_BATTLE_TEST("Instruct-called moves keep their priority, which is considered for Dazzling, Queenly Majesty and Armor Tail (Traits)")
{
    u32 species;
    enum Ability ability;
...
SINGLE_BATTLE_TEST("Dazzling, Queenly Majesty and Armor Tail do not block high-priority moves called by other moves (Traits)")
{
    u32 species;
    enum Ability ability;
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Dazzling, Queenly Majesty and Armor Tail do not block Teatime (Multi)")
{
    u32 species;
    enum Ability ability;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TEATIME) == EFFECT_TEATIME);
        ASSUME(GetItemHoldEffect(ITEM_ORAN_BERRY) == HOLD_EFFECT_RESTORE_HP);
        PLAYER(SPECIES_MURKROW) { Ability(ABILITY_PRANKSTER); Items(ITEM_NUGGET, ITEM_ORAN_BERRY); HP(1); MaxHP(100); }
...
DOUBLE_BATTLE_TEST("Instruct-called moves keep their priority, which is considered for Dazzling, Queenly Majesty and Armor Tail (Multi)")
{
    u32 species;
    enum Ability ability;
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); }
        OPPONENT(species) { Ability(ABILITY_LIGHT_METAL); Speed(15); }
    } WHEN {
        TURN { MOVE(playerRight, MOVE_QUICK_ATTACK, target: opponentLeft); MOVE(playerLeft, MOVE_INSTRUCT, target: playerRight); SEND_OUT(opponentLeft, 2); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_QUICK_ATTACK, playerRight);
