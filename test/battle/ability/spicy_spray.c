...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Spicy Spray burns the attacker (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_SCRATCH; }
    PARAMETRIZE { move = MOVE_SWIFT; }
...
SINGLE_BATTLE_TEST("Spicy Spray burns the attacker even if the attacker is behind a Substitute (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_SCRATCH; }
    PARAMETRIZE { move = MOVE_SWIFT; }
...
SINGLE_BATTLE_TEST("Spicy Spray does not burn the attacker if the defender is behind a Substitute (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_SCRATCH; }
    PARAMETRIZE { move = MOVE_SWIFT; }
...
SINGLE_BATTLE_TEST("Spicy Spray burns the attacker even if the attacker has Sheer Force (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_CRUNCH; }
    PARAMETRIZE { move = MOVE_ICE_BEAM; }
...
SINGLE_BATTLE_TEST("Spicy Spray burns the attacker even if the defender faints (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SCOVILLAIN) { Item(ITEM_SCOVILLAINITE); HP(1); }
...
SINGLE_BATTLE_TEST("Spicy Spray burns the attacker even if the defender behind a Substitute takes damage (Traits)")
{
    GIVEN {
        ASSUME(IsSoundMove(MOVE_HYPER_VOICE));
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Spicy Spray does not activate if Future Sight attacker is not on the field (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_WOBBUFFET);
