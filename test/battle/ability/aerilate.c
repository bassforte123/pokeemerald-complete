...
SINGLE_BATTLE_TEST("Aerilate turns a Normal-type move into Flying-type move (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_MEGANIUM);
        OPPONENT(SPECIES_SALAMENCE) { Items(ITEM_LEAF_STONE, ITEM_SALAMENCITE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH, gimmick: GIMMICK_MEGA); }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_MEGA_EVOLUTION, opponent);
...
SINGLE_BATTLE_TEST("Aerilate boosts power of affected moves by 20% (Gen7+) or 30% (Gen1-6) (Multi)", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SALAMENCE) { Ability(ABILITY_MOXIE); Items(ITEM_LEAF_STONE, ITEM_SALAMENCITE); }
    } WHEN {
        TURN { MOVE(opponent, move, gimmick: GIMMICK_MEGA); MOVE(player, MOVE_TACKLE); }
    } SCENE {
        HP_BAR(opponent, captureDamage: &results[i].damage);
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Weather Ball's type (Multi)", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(gSpeciesInfo[SPECIES_PINSIR].types[0] == TYPE_BUG);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_PINSIR) { Ability(ABILITY_HYPER_CUTTER); Items(ITEM_LEAF_STONE, ITEM_PINSIRITE); }
    } WHEN {
        TURN { MOVE(opponent, move2, gimmick: GIMMICK_MEGA); MOVE(player, move1); }
        TURN { MOVE(player, MOVE_WEATHER_BALL); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Natural Gift's type (Multi)")
{
    ...
    GIVEN {
        ...
        ASSUME(gNaturalGiftTable[ITEM_TO_BERRY(ITEM_PERSIM_BERRY)].type == TYPE_GROUND);
        ASSUME(gSpeciesInfo[SPECIES_SALAMENCE_MEGA].types[0] == TYPE_FLYING || gSpeciesInfo[SPECIES_SALAMENCE_MEGA].types[1] == TYPE_FLYING);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LEAF_STONE, ITEM_PERSIM_BERRY); }
        OPPONENT(SPECIES_SALAMENCE) { Items(ITEM_LEAF_STONE, ITEM_SALAMENCITE); }
    } WHEN {
        TURN { MOVE(opponent, move, gimmick: GIMMICK_MEGA); MOVE(player, MOVE_NATURAL_GIFT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Judgment / Techno Blast / Multi-Attack's type (Multi)")
{
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_ELECTRIC_MEMORY].secondaryId == TYPE_ELECTRIC);
        ASSUME(gSpeciesInfo[SPECIES_DIGLETT].types[0] == TYPE_GROUND);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LEAF_STONE, item); }
        OPPONENT(SPECIES_SALAMENCE) { Items(ITEM_LEAF_STONE, ITEM_SALAMENCITE); }
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP, gimmick: GIMMICK_MEGA); }
