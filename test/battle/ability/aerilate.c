...
SINGLE_BATTLE_TEST("Aerilate turns a Normal-type move into Flying-type move")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_SALAMENCE) { Items(ITEM_SALAMENCITE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate boosts power of affected moves by 20% (Gen7+) or 30% (Gen1-6)", s16 damage)
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_SALAMENCE) { Ability(ABILITY_MOXIE); Items(ITEM_SALAMENCITE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Weather Ball's type", s16 damage)
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_PINSIR) { Ability(ABILITY_HYPER_CUTTER); Items(ITEM_PINSIRITE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Natural Gift's type")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PERSIM_BERRY); }
        OPPONENT(SPECIES_SALAMENCE) { Items(ITEM_SALAMENCITE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate can not turn certain moves into Flying type moves (Multi)")
{
    u32 move;
    PARAMETRIZE { move = MOVE_WEATHER_BALL; }
    // PARAMETRIZE { move = MOVE_NATURAL_GIFT; } TODO: handle this case via Skill Swap
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Natural Gift's type (Multi)")
{
    u16 move;
    PARAMETRIZE { move = MOVE_CELEBRATE; }
    PARAMETRIZE { move = MOVE_SKILL_SWAP; }
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Judgment / Techno Blast / Multi-Attack's type (Multi)")
{
    u16 move, item;
    PARAMETRIZE { move = MOVE_JUDGMENT; item = ITEM_ZAP_PLATE; }
    PARAMETRIZE { move = MOVE_TECHNO_BLAST; item = ITEM_SHOCK_DRIVE; }
