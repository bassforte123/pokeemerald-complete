...
SINGLE_BATTLE_TEST("Poison Puppeteer does not trigger when poison came from Poison Point (Traits)")
{
    GIVEN {
        ASSUME(MoveMakesContact(MOVE_SCRATCH));
        ASSUME(GetMoveEffect(MOVE_SOAK) == EFFECT_SOAK);
        ...
        ASSUME(GetSpeciesType(SPECIES_SKRELP, 0) == TYPE_POISON || GetSpeciesType(SPECIES_SKRELP, 1) == TYPE_POISON);
        PLAYER(SPECIES_PECHARUNT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POISON_PUPPETEER); }
        OPPONENT(SPECIES_SKRELP) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_POISON_POINT); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SOAK); }
        TURN { MOVE(player, MOVE_SCRATCH, WITH_RNG(RNG_POISON_POINT, TRUE)); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Poison Puppeteer confuses target (not user) after Flame Body also triggers (Traits)")
{
    GIVEN {
        ASSUME(MoveMakesContact(MOVE_MORTAL_SPIN));
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        PLAYER(SPECIES_PECHARUNT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POISON_PUPPETEER); }
        OPPONENT(SPECIES_MOLTRES) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FLAME_BODY); }
    } WHEN {
        TURN { MOVE(player, MOVE_MORTAL_SPIN, WITH_RNG(RNG_FLAME_BODY, TRUE)); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_MORTAL_SPIN, player);
...
SINGLE_BATTLE_TEST("Poison Puppeteer consumes its pending target after triggering (Traits)")
{
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_TWINEEDLE) == 2);
        ASSUME(MoveHasAdditionalEffect(MOVE_TWINEEDLE, MOVE_EFFECT_POISON));
...
DOUBLE_BATTLE_TEST("Poison Puppeteer does not leak confusion to second target (Traits)")
{
    GIVEN {
        ASSUME(MoveMakesContact(MOVE_MORTAL_SPIN));
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        ...
        PLAYER(SPECIES_PECHARUNT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POISON_PUPPETEER); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SLOWBRO) { Ability(ABILITY_REGENERATOR); Innates(ABILITY_OWN_TEMPO); } // can be poisoned, cannot be confused
        OPPONENT(SPECIES_FERROSEED); // Steel-type, cannot be poisoned, can be confused
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_MORTAL_SPIN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Poison Puppeteer consumes its pending target after triggering (Traits)")
{
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_PERSIM_BERRY].holdEffect == HOLD_EFFECT_CURE_CONFUSION);
        PLAYER(SPECIES_PECHARUNT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POISON_PUPPETEER); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NUGGET, ITEM_PERSIM_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_TWINEEDLE, WITH_RNG(RNG_SECONDARY_EFFECT, TRUE)); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_TWINEEDLE, player);
