...
SINGLE_BATTLE_TEST("Normalize turns a move into a Normal-type move (Traits)")
{
    ...
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GASTLY, 0) == TYPE_GHOST);
        PLAYER(SPECIES_GASTLY);
        OPPONENT(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); Moves(MOVE_WATER_GUN);}
    } WHEN {
        TURN { MOVE(opponent, MOVE_WATER_GUN); }
    } SCENE {
        if (ability == ABILITY_CUTE_CHARM)
...
SINGLE_BATTLE_TEST("Normalize affects status moves (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_DRILBUR, 0) == TYPE_GROUND);
        PLAYER(SPECIES_DRILBUR);
        OPPONENT(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); Moves(MOVE_THUNDER_WAVE);}
    } WHEN {
        TURN { MOVE(opponent, MOVE_THUNDER_WAVE); }
    } SCENE {
        if (ability == ABILITY_CUTE_CHARM)
...
SINGLE_BATTLE_TEST("Normalize still makes Freeze-Dry do super effective damage to Water-type Pokémon (Traits)", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_MUDKIP, 0) == TYPE_WATER);
        PLAYER(SPECIES_MUDKIP);
        OPPONENT(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); Moves(MOVE_FREEZE_DRY);}
    } WHEN {
        TURN { MOVE(opponent, MOVE_FREEZE_DRY); }
    } SCENE {
        MESSAGE("It's super effective!");
...
SINGLE_BATTLE_TEST("Normalize doesn't boost power of unaffected moves by 20% (< Gen7) (Traits)", s16 damage)
{
    ...
    GIVEN {
        WITH_CONFIG(B_ATE_MULTIPLIER, GEN_6);
        PLAYER(SPECIES_DELCATTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_POUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize doesn't boost power of affected moves by 20% (< Gen7) (Traits)", s16 damage)
{
    ...
    GIVEN {
        WITH_CONFIG(B_ATE_MULTIPLIER, GEN_6);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); Moves(MOVE_WATER_GUN); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_WATER_GUN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize-affected moves become Electric-type under Electrify's effect (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ELECTRIFY) == EFFECT_ELECTRIFY);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); }
        OPPONENT(SPECIES_ROOKIDEE) { Item(ITEM_WACAN_BERRY); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ELECTRIFY); MOVE(player, MOVE_WATER_GUN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize-affected moves become Electric-type under Ion Deluge's effect (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ION_DELUGE) == EFFECT_ION_DELUGE);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); Moves(MOVE_WATER_GUN); }
        OPPONENT(SPECIES_ROOKIDEE) { Item(ITEM_WACAN_BERRY); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ION_DELUGE); MOVE(player, MOVE_WATER_GUN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Weather Ball's type (Traits)", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveType(MOVE_WEATHER_BALL) == TYPE_NORMAL);
        ASSUME(GetSpeciesType(SPECIES_MEGANIUM, 0) == TYPE_GRASS);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); }
        OPPONENT(SPECIES_MEGANIUM);
    } WHEN {
        TURN { MOVE(player, move); }
        TURN { MOVE(player, MOVE_WEATHER_BALL); }
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Judgment / Techno Blast / Multi-Attack's type (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_ELECTRIC_MEMORY].secondaryId == TYPE_ELECTRIC);
        ASSUME(GetSpeciesType(SPECIES_DIGLETT, 0) == TYPE_GROUND);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); Item(item); }
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
        TURN { MOVE(player, move); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Hidden Power's type (Traits)")
{
    GIVEN {
        ...
        ASSUME(gTypesInfo[TYPE_ELECTRIC].isHiddenPowerType == TRUE);
        ASSUME(GetSpeciesType(SPECIES_DIGLETT, 0) == TYPE_GROUND);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); HPIV(31); AttackIV(31); DefenseIV(31); SpAttackIV(30); SpDefenseIV(31); SpeedIV(31); } // HP Electric
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
        TURN { MOVE(player, MOVE_HIDDEN_POWER); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize doesn't change Tera Blast's type when Terastallized (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TERA_BLAST) == EFFECT_TERA_BLAST);
        ASSUME(GetMoveType(MOVE_TERA_BLAST) == TYPE_NORMAL);
        ASSUME(GetSpeciesType(SPECIES_MISDREAVUS, 0) == TYPE_GHOST);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); TeraType(TYPE_DARK); }
        OPPONENT(SPECIES_MISDREAVUS);
    } WHEN {
        TURN { MOVE(player, MOVE_TERA_BLAST, gimmick: GIMMICK_TERA); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize makes Flying Press do Normal/Flying damage (Traits)")
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_CUTE_CHARM; }
    PARAMETRIZE { ability = ABILITY_NORMALIZE; }
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GOLEM, 0) == TYPE_ROCK || GetSpeciesType(SPECIES_GOLEM, 1) == TYPE_ROCK);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ability); Moves(MOVE_FLYING_PRESS); }
        OPPONENT(SPECIES_GOLEM);
    } WHEN {
        TURN { MOVE(player, MOVE_FLYING_PRESS); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Terrain Pulse's type (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TERRAIN_PULSE) == EFFECT_TERRAIN_PULSE);
        ASSUME(GetMoveType(MOVE_TERRAIN_PULSE) == TYPE_NORMAL);
...
SINGLE_BATTLE_TEST("Normalize doesn't affect damaging Z-Move types (Traits)")
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_WATER_GUN) == TYPE_WATER);
        ASSUME(GetSpeciesType(SPECIES_GOLEM, 0) == TYPE_ROCK || GetSpeciesType(SPECIES_GOLEM, 1) == TYPE_ROCK);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_WONDER_SKIN); Innates(ABILITY_NORMALIZE); Item(ITEM_WATERIUM_Z); Moves(MOVE_WATER_GUN); }
        OPPONENT(SPECIES_GOLEM);
    } WHEN {
        TURN { MOVE(player, MOVE_WATER_GUN, gimmick: GIMMICK_Z_MOVE); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Normalize-affected moves become Electric-type under Electrify's effect (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ELECTRIFY) == EFFECT_ELECTRIFY);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_NORMALIZE); }
...
SINGLE_BATTLE_TEST("Normalize-affected moves become Electric-type under Ion Deluge's effect (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ION_DELUGE) == EFFECT_ION_DELUGE);
        PLAYER(SPECIES_SKITTY) { Ability(ABILITY_NORMALIZE); Moves(MOVE_WATER_GUN); }
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Natural Gift's type (Items)")
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_CUTE_CHARM; }
    PARAMETRIZE { ability = ABILITY_NORMALIZE; }
...
SINGLE_BATTLE_TEST("Normalize doesn't affect Judgment / Techno Blast / Multi-Attack's type (Items)")
{
    enum Move move;
    enum Item item;
    PARAMETRIZE { move = MOVE_JUDGMENT; item = ITEM_ZAP_PLATE; }
