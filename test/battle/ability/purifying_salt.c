...
SINGLE_BATTLE_TEST("Purifying Salt halves damage from Ghost-type moves (Traits)", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_SHADOW_BALL) == TYPE_GHOST);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_GARGANACL) { Ability(ABILITY_CLEAR_BODY); Innates(ability); }
    } WHEN {
        TURN { MOVE(player, MOVE_SHADOW_BALL); }
    } SCENE {
        HP_BAR(opponent, captureDamage: &results[i].damage);
...
SINGLE_BATTLE_TEST("Purifying Salt grants immunity to status effects (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveNonVolatileStatus(MOVE_TOXIC) == MOVE_EFFECT_TOXIC);
        ASSUME(MoveHasAdditionalEffect(MOVE_POWDER_SNOW, MOVE_EFFECT_FREEZE_OR_FROSTBITE) == TRUE);
        PLAYER(SPECIES_WOBBUFFET) { Ability(ABILITY_CLEAR_BODY); Innates(ABILITY_PURIFYING_SALT); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Purifying Salt user can't be poisoned by Toxic Spikes (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GARGANACL) { Ability(ABILITY_CLEAR_BODY); Innates(ABILITY_PURIFYING_SALT); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_TOXIC_SPIKES); }
        TURN { SWITCH(player, 1); }
