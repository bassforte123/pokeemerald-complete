...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Magic Bounce bounces back status moves (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TOXIC) == EFFECT_NON_VOLATILE_STATUS);
        ASSUME(GetMoveNonVolatileStatus(MOVE_TOXIC) == MOVE_EFFECT_TOXIC);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_ESPEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_MAGIC_BOUNCE); }
    } WHEN {
        TURN { MOVE(player, MOVE_TOXIC); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_MAGIC_BOUNCE);
...
SINGLE_BATTLE_TEST("Magic Bounce wont activate if ability user protects (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_PROTECT) == EFFECT_PROTECT);
        ASSUME(GetMoveEffect(MOVE_TOXIC) == EFFECT_NON_VOLATILE_STATUS);
...
SINGLE_BATTLE_TEST("Magic Bounce wont activate if ability user is in a semi-invulnerable position (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_DIG) == EFFECT_SEMI_INVULNERABLE);
        ASSUME(GetMoveEffect(MOVE_TOXIC) == EFFECT_NON_VOLATILE_STATUS);
...
SINGLE_BATTLE_TEST("Magic Bounce bounces back powder moves (Traits)")
{
    GIVEN {
        ASSUME(IsPowderMove(MOVE_STUN_SPORE));
        ASSUME(GetMoveEffect(MOVE_STUN_SPORE) == EFFECT_NON_VOLATILE_STATUS);
...
SINGLE_BATTLE_TEST("Magic Bounce cannot bounce back powder moves against Grass Types (Traits)")
{
    GIVEN {
        WITH_CONFIG(B_POWDER_GRASS, GEN_6);
        ASSUME(IsPowderMove(MOVE_STUN_SPORE));
...
DOUBLE_BATTLE_TEST("Magic Bounce bounces back moves hitting both foes at two foes (Traits)")
{
    GIVEN {
        ASSUME_STAT_CHANGE(MOVE_LEER, defense: -1);
        ASSUME(GetMoveTarget(MOVE_LEER) == TARGET_BOTH);
...
DOUBLE_BATTLE_TEST("Magic Bounce activates on all opposing mons (Traits)")
{
    GIVEN {
        ASSUME_STAT_CHANGE(MOVE_LEER, defense: -1);
        ASSUME(GetMoveTarget(MOVE_LEER) == TARGET_BOTH);
        ...
        PLAYER(SPECIES_KADABRA);
        OPPONENT(SPECIES_ESPEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_MAGIC_BOUNCE); }
        OPPONENT(SPECIES_NATU) { Ability(ABILITY_EARLY_BIRD); Innates(ABILITY_MAGIC_BOUNCE); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_LEER); }
    } SCENE {
        ABILITY_POPUP(opponentLeft, ABILITY_MAGIC_BOUNCE);
...
DOUBLE_BATTLE_TEST("Magic Bounce bounces back moves hitting foes field (Traits)")
{
    u32 battlerOne, battlerTwo, abilityBattlerOne, abilityBattlerTwo;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_ABRA);
        PLAYER(SPECIES_KADABRA);
        OPPONENT(battlerOne) { Ability(ABILITY_EARLY_BIRD); Innates(abilityBattlerOne); }
...
SINGLE_BATTLE_TEST("Magic Bounce bounced back status moves can not be bounced back by Magic Bounce (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TOXIC) == EFFECT_NON_VOLATILE_STATUS);
        ASSUME(GetMoveNonVolatileStatus(MOVE_TOXIC) == MOVE_EFFECT_TOXIC);
        PLAYER(SPECIES_ESPEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_MAGIC_BOUNCE); }
        OPPONENT(SPECIES_ESPEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_MAGIC_BOUNCE); }
    } WHEN {
        TURN { MOVE(player, MOVE_TOXIC); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Magic Bounce can't reflect back Stealth Rock from a semi-invulnerable posistion even with No Guard (Traits)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_STEALTH_ROCK) == TARGET_OPPONENTS_FIELD);
        ASSUME(GetMoveEffect(MOVE_DIG) == EFFECT_SEMI_INVULNERABLE);
        PLAYER(SPECIES_MACHAMP) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_NO_GUARD); }
        OPPONENT(SPECIES_ESPEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_MAGIC_BOUNCE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_DIG); MOVE(player, MOVE_STEALTH_ROCK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Magic Bounce bounces back status moves before Magic Coat (Traits)")
{
    GIVEN {
        ASSUME_STAT_CHANGE(MOVE_LEER, defense: -1);
        PLAYER(SPECIES_WYNAUT);
...
DOUBLE_BATTLE_TEST("Magic Bounce will trigger after all valid targets have been targetted (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_LEER, defense: -1);
...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Magic Bounce will trigger after all valid targets have been targetted (ITEMS)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_LEER, defense: -1);
        ASSUME(GetMoveTarget(MOVE_LEER) == TARGET_BOTH);
        PLAYER(SPECIES_ABRA);
        PLAYER(SPECIES_KADABRA) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ESPEON) { Ability(ABILITY_MAGIC_BOUNCE); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
