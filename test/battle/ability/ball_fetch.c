...
WILD_BATTLE_TEST("Ball Fetch doesn't trigger if the Pokémon is already holding an item")
{
    ...
    GIVEN {
        PLAYER(SPECIES_YAMPER) { Ability(ABILITY_BALL_FETCH); Items(item, ITEM_LEADERS_CREST); }
        OPPONENT(SPECIES_METAGROSS);
    } WHEN {
...
WILD_BATTLE_TEST("Ball Fetch causes the Pokémon to pick up the last failed Ball at the end of the turn (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_YAMPER) { Ability(ABILITY_BALL_FETCH); Items(ITEM_LEADERS_CREST, ITEM_NONE); }
        OPPONENT(SPECIES_METAGROSS);
    } WHEN {
        TURN { USE_ITEM(player, item); }
    } SCENE {
        ...
        EXPECT_EQ(player->items[0], ITEM_LEADERS_CREST);
        if (item != ITEM_X_ACCURACY)
...
WILD_BATTLE_TEST("Ball Fetch doesn't trigger if the Pokémon is already holding an item (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_YAMPER) { Ability(ABILITY_BALL_FETCH); Items(ITEM_LEADERS_CREST, item); }
        OPPONENT(SPECIES_METAGROSS);
    } WHEN {
        TURN { USE_ITEM(player, ITEM_GREAT_BALL); }
    } SCENE {
...
WILD_BATTLE_TEST("Ball Fetch only picks up the first failed ball, once per battle (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_YAMPER) { Ability(ABILITY_BALL_FETCH); Items(ITEM_LEADERS_CREST); }
        OPPONENT(SPECIES_METAGROSS);
    } WHEN {
        TURN { USE_ITEM(player, item); }
        TURN { MOVE(player, MOVE_BESTOW); }
