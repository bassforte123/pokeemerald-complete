...
DOUBLE_BATTLE_TEST("Aroma Veil protects the Pokémon's side from ally Taunt (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
        ASSUME(GetMoveCategory(MOVE_HARDEN) == DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_HEALER); Innates(ABILITY_AROMA_VEIL); Speed(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WYNAUT) { Speed(4); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Aroma Veil protects the Pokémon's side from Heal Block (Traits)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_HEAL_BLOCK) == EFFECT_HEAL_BLOCK);
        ASSUME(GetMoveEffect(MOVE_RECOVER) == EFFECT_RESTORE_HP);
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_HEALER); Innates(ABILITY_AROMA_VEIL); HP(1); }
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Aroma Veil protects the Pokémon's side from Destiny Knot infatuation (Traits)")
{
    struct BattlePokemon *moveTarget = NULL;
    PARAMETRIZE { moveTarget = playerLeft; }
    PARAMETRIZE { moveTarget = playerRight; }
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ATTRACT) == EFFECT_ATTRACT);
        ASSUME(gItemsInfo[ITEM_DESTINY_KNOT].holdEffect == HOLD_EFFECT_DESTINY_KNOT);
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_HEALER); Innates(ABILITY_AROMA_VEIL); Gender(MON_MALE); }
        PLAYER(SPECIES_WOBBUFFET) { Gender(MON_MALE); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_DESTINY_KNOT); Gender(MON_FEMALE); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Aroma Veil protects the Pokémon's side from Destiny Knot infatuation (Items)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_AROMA_VEIL); Gender(MON_MALE); }
        PLAYER(SPECIES_WOBBUFFET) { Gender(MON_MALE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NUGGET, ITEM_DESTINY_KNOT); Gender(MON_FEMALE); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(moveTarget, MOVE_ATTRACT, target: opponentLeft); }
    } SCENE {
