...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Soundproof makes sound moves fail against the ability user (Traits)")
{
    GIVEN {
        ASSUME(IsSoundMove(MOVE_BOOMBURST));
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EXPLOUD) { Ability(ABILITY_SCRAPPY); Innates(ABILITY_SOUNDPROOF); }
    } WHEN {
        TURN { MOVE(player, MOVE_BOOMBURST); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SOUNDPROOF);
