...
DOUBLE_BATTLE_TEST("Tailwind does not trigger Wind Rider on an absent ally battler (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
        PLAYER(SPECIES_BRAMBLIN) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_WIND_RIDER); HP(1); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_WATER_GUN, target: playerRight); }
