...
DOUBLE_BATTLE_TEST("Commander Tatsugiri will still take residual damage from a field effect while inside Dondozo (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); }
        PLAYER(SPECIES_DONDOZO);
        OPPONENT(SPECIES_TYRANITAR) { Ability(ABILITY_MOXIE); Innates(ABILITY_SAND_STREAM); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Commander prevents Emergency Exit from switching out Dondozo (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); }
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Button from switching out Dondozo (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); }
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Pack from switching out Dondozo (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_CHARM, attack: -2);
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Pack from activating after a switch-in stat drop (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); }
        ...
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_INCINEROAR) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN {
            MOVE(playerRight, MOVE_SCRATCH, target: opponentLeft);
            SEND_OUT(opponentLeft, 2);
...
DOUBLE_BATTLE_TEST("Commander prevents pivot moves from switching out Dondozo (Traits)")
{
    enum Move move;
    enum BattleMoveEffects effect;
...
DOUBLE_BATTLE_TEST("Commander clears when Dondozo is replaced and Tatsugiri can be hit (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_VOLT_SWITCH) == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_DONDOZO) { HP(1); Speed(1); }
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); MaxHP(400); HP(400); Speed(2); }
        PLAYER(SPECIES_SEADRA) { Speed(3); }
        OPPONENT(SPECIES_VENUSAUR) { Speed(5); }
        OPPONENT(SPECIES_LUXRAY) { Speed(6); }
        OPPONENT(SPECIES_BUTTERFREE) { Speed(4); }
...
DOUBLE_BATTLE_TEST("Commander does not clear semi-invulnerability of non-Tatsugiri partner (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLY) == EFFECT_SEMI_INVULNERABLE);
        PLAYER(SPECIES_DONDOZO) { HP(1); Speed(1); }
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); HP(1); Status1(STATUS1_POISON); Speed(2); }
        PLAYER(SPECIES_PIDGEOT) { Speed(100); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(90); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(80); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Commander still blocks forced switch after swallowed Tatsugiri faints (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_DRAGON_TAIL; }
    PARAMETRIZE { move = MOVE_WHIRLWIND; }
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_DONDOZO);
        OPPONENT(SPECIES_TATSUGIRI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMMANDER); HP(1); Status1(STATUS1_POISON); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { SKIP_TURN(opponentRight); SEND_OUT(opponentRight, 2); }
...
DOUBLE_BATTLE_TEST("Red Card is still consumed but cannot force out Dondozo after swallowed Tatsugiri faints (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_RED_CARD].holdEffect == HOLD_EFFECT_RED_CARD);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_RED_CARD); }
...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Commander prevents Red Card from working while Commander is active (Items)")
{
    GIVEN {
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_COMMANDER); }
        PLAYER(SPECIES_DONDOZO);
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Button from switching out Dondozo (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_COMMANDER); }
        PLAYER(SPECIES_DONDOZO) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Pack from switching out Dondozo (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        ASSUME_STAT_CHANGE(MOVE_CHARM, attack: -2);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_COMMANDER); }
        PLAYER(SPECIES_DONDOZO) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Commander prevents Eject Pack from activating after a switch-in stat drop (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_PACK].holdEffect == HOLD_EFFECT_EJECT_PACK);
        PLAYER(SPECIES_TATSUGIRI) { Ability(ABILITY_COMMANDER); }
