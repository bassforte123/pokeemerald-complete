...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Mega Sol multiplies the power of Fire-type moves by 1.5x (Traits)")
{
    s16 damage[2];
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_EMBER) == TYPE_FIRE);
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
...
SINGLE_BATTLE_TEST("Mega Sol halves the power of the user's Water-type moves (Traits)")
{
    s16 damage[2];
    ...
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_WATER_GUN); MOVE(opponent, MOVE_WATER_GUN); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_WATER_GUN, player);
        HP_BAR(opponent, captureDamage: &damage[0]);
...
SINGLE_BATTLE_TEST("Weather Ball stays Fire-type under real weather if user has Mega Sol (Traits)")
{
    enum Move weatherMove;
    PARAMETRIZE { weatherMove = MOVE_RAIN_DANCE; }
    PARAMETRIZE { weatherMove = MOVE_SANDSTORM; }
...
SINGLE_BATTLE_TEST("Weather Ball doubles its power and turns to a Fire-type move if user has Mega Sol (Traits)")
{
    s16 damage[2];
...
SINGLE_BATTLE_TEST("Synthesis, Morning Sun and Moonlight recover 2/3 of the user's max HP if user has Mega Sol (Traits)")
{
    enum Move move;
    enum BattleMoveEffects effect;
    PARAMETRIZE { move = MOVE_SYNTHESIS;   effect = EFFECT_SYNTHESIS; }
    ...
    GIVEN {
        ASSUME(GetMoveEffect(move) == effect);
        PLAYER(SPECIES_MEGANIUM) { HP(1); MaxHP(300); Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Mega Sol ignores Sandstorm's solarbeam power reduction, and its rock defense boost (Traits)")
{
    s16 damage[2];
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_BASTIODON, 0) == TYPE_ROCK || GetSpeciesType(SPECIES_BASTIODON, 1) == TYPE_ROCK);
        PLAYER(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
        OPPONENT(SPECIES_BASTIODON) { Ability(ABILITY_SOUNDPROOF); Innates(ABILITY_SAND_STREAM);}
    } WHEN {
        TURN { MOVE(player, MOVE_SOLAR_BEAM); MOVE(opponent, MOVE_SKILL_SWAP); }
        TURN { MOVE(player, MOVE_SOLAR_BEAM); }
        TURN { SKIP_TURN(player); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SAND_STREAM);
...
SINGLE_BATTLE_TEST("Mega Sol ignores Snow's Ice-type Defense boost (Traits)")
{
    s16 damage[2];
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_VANILLUXE, 0) == TYPE_ICE || GetSpeciesType(SPECIES_VANILLUXE, 1) == TYPE_ICE);
        PLAYER(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
        OPPONENT(SPECIES_VANILLUXE) { Ability(ABILITY_ICE_BODY); Innates(ABILITY_SNOW_WARNING); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); }
        TURN { MOVE(player, MOVE_SCRATCH); }
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
...
SINGLE_BATTLE_TEST("Mega Sol doesn't trigger the foe's Leaf Guard (Traits)", s16 damage)
{
    enum Move move;
    PARAMETRIZE { move = MOVE_CELEBRATE;}
    PARAMETRIZE { move = MOVE_SUNNY_DAY;}
    ...
    GIVEN {
        ...
        ASSUME(GetMoveNonVolatileStatus(MOVE_WILL_O_WISP) == MOVE_EFFECT_BURN);
        PLAYER(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ;}
        OPPONENT(SPECIES_LEAFEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_LEAF_GUARD);}
    } WHEN {
        TURN { MOVE(player, move); }
        TURN { MOVE(player, MOVE_WILL_O_WISP); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Mega Sol ignores Cloud Nine (Traits)")
{
    s16 damage[2];
    u16 species;
    enum Ability ability;
    ...
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_EMBER); MOVE(opponent, MOVE_SKILL_SWAP); }
        TURN { MOVE(player, MOVE_EMBER); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_EMBER, player);
        HP_BAR(opponent, captureDamage: &damage[0]);
...
SINGLE_BATTLE_TEST("Mega Sol lowers the user's Thunder and Hurricane accuracy to 50% (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_THUNDER; }
    PARAMETRIZE { move = MOVE_HURRICANE; }
...
SINGLE_BATTLE_TEST("Mega Sol lowers the user's Thunder and Hurricane accuracy to 50% in Rain (Traits)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_THUNDER; }
    PARAMETRIZE { move = MOVE_HURRICANE; }
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
    } WHEN {
...
SINGLE_BATTLE_TEST("Mega Sol ignores Sand Veil's accuracy drop (Traits)")
{
    PASSES_RANDOMLY(5, 5, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_SCRATCH) == 100);
...
SINGLE_BATTLE_TEST("Mega Sol ignores Snow Cloak's accuracy drop (Traits)")
{
    PASSES_RANDOMLY(5, 5, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_SCRATCH) == 100);
        PLAYER(SPECIES_GLACEON) { Ability(ABILITY_ADAPTABILITY); Innates(ABILITY_SNOW_CLOAK); }
        OPPONENT(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
    } WHEN {
        TURN { MOVE(player, MOVE_HAIL); }
        TURN { MOVE(opponent, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Mega Sol: Solar Beam does not need a charging turn if user has Mega Sol (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SOLARBEAM) == EFFECT_SOLAR_BEAM);
        ASSUME(GetMoveType(MOVE_SOLARBEAM) == TYPE_GRASS);
...
// Not clear if this is a bug or not
SINGLE_BATTLE_TEST("Mega Sol: Growth increases Attack and Sp. Atk by 2 stages under Mega Sol (Gen 5+) (Traits)")
{
    GIVEN {
        WITH_CONFIG(B_GROWTH_STAT_RAISE, GEN_1);
        ASSUME(GetMoveEffect(MOVE_GROWTH) == EFFECT_GROWTH);
...
// Not clear if this is a bug or not
SINGLE_BATTLE_TEST("Mega Sol doesn't prevent other weather based activations (Electro Shot) (Traits)")
{
    GIVEN {
        ASSUME(GetMoveTwoTurnAttackWeather(MOVE_ELECTRO_SHOT) == B_WEATHER_RAIN);
        ASSUME(GetMoveEffect(MOVE_ELECTRO_SHOT) == EFFECT_TWO_TURNS_ATTACK);
        PLAYER(SPECIES_KYOGRE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRIZZLE); }
        OPPONENT(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ELECTRO_SHOT); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_ELECTRO_SHOT, opponent);
        HP_BAR(player);
    }
...
// Not clear if this is a bug or not
SINGLE_BATTLE_TEST("Mega Sol doesn't prevent other weather based activations (Aurora Veil) (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_AURORA_VEIL) == EFFECT_AURORA_VEIL);
        PLAYER(SPECIES_ABOMASNOW) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SNOW_WARNING); }
        OPPONENT(SPECIES_MEGANIUM) { Ability(ABILITY_OVERGROW); Innates(ABILITY_MEGA_SOL); ; }
    } WHEN {
        TURN { MOVE(opponent, MOVE_AURORA_VEIL); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_AURORA_VEIL, opponent);
    }
}
