...
SINGLE_BATTLE_TEST("Orichalcum Pulse boost applies even if the target holds Utility Umbrella (Multi)", s16 damage)
{
    u16 targetItem;
    PARAMETRIZE { targetItem = ITEM_NONE; }
    PARAMETRIZE { targetItem = ITEM_UTILITY_UMBRELLA; }
    ...
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_KORAIDON) { Ability(ABILITY_ORICHALCUM_PULSE); Moves(MOVE_SCRATCH); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); Speed(10); Items(ITEM_PECHA_BERRY, targetItem); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_CELEBRATE); MOVE(player, MOVE_SCRATCH); }
    } SCENE {
        ABILITY_POPUP(player, ABILITY_ORICHALCUM_PULSE);
...
SINGLE_BATTLE_TEST("Orichalcum Pulse does not boost physical moves if holder has Utility Umbrella (Multi)", s16 damage)
{
    u16 holdItem;
    PARAMETRIZE { holdItem = ITEM_NONE; }
    PARAMETRIZE { holdItem = ITEM_UTILITY_UMBRELLA; }
    ...
    GIVEN {
        ASSUME(gItemsInfo[ITEM_UTILITY_UMBRELLA].holdEffect == HOLD_EFFECT_UTILITY_UMBRELLA);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_KORAIDON) { Ability(ABILITY_ORICHALCUM_PULSE); Moves(MOVE_SCRATCH); Speed(5); Items(ITEM_PECHA_BERRY, holdItem); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); Speed(10); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_CELEBRATE); MOVE(player, MOVE_SCRATCH); }
    } SCENE {
