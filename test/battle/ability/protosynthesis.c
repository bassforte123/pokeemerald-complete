...
SINGLE_BATTLE_TEST("Protosynthesis activates in Sun before Booster Energy (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GREAT_TUSK) { Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_MAX_ETHER, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_NINETALES) { Ability(ABILITY_DROUGHT); }
    } WHEN {
        TURN { SWITCH(player, 1); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Protosynthesis doesn't activate for a transformed battler (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GREAT_TUSK) { Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_MAX_ETHER, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_NINETALES) { Ability(ABILITY_DROUGHT); Items(ITEM_MAX_ETHER, ITEM_BOOSTER_ENERGY); }
    } WHEN {
        TURN { SWITCH(player, 1); MOVE(opponent, MOVE_TRANSFORM); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_DROUGHT);
...
{
    GIVEN {
        PLAYER(SPECIES_GREAT_TUSK) { Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_MAX_ETHER, ITEM_UTILITY_UMBRELLA); }
        OPPONENT(SPECIES_NINETALES) { Ability(ABILITY_DROUGHT); }
    } WHEN {
        TURN { }
    } SCENE {
