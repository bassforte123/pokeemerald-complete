...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Gulp Missile: Cramorant cannot change between Gorging and Gulping Forms (Traits)")
{
    u32 species, hp;
    enum Move move;
    PARAMETRIZE { species = SPECIES_CRAMORANT_GULPING; hp = 240; move = MOVE_BELLY_DRUM; }
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_BELLY_DRUM) == EFFECT_BELLY_DRUM);
        PLAYER(SPECIES_CRAMORANT) { HP(hp); MaxHP(250); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_GULP_MISSILE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SURF); }
        TURN { MOVE(player, move); }
...
SINGLE_BATTLE_TEST("Gulp Missile: Transformed Cramorant Gulping lowers defense and still triggers other effects after (Traits)")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_DRAGAPULT) { Ability(ABILITY_INFILTRATOR); Innates(ability); }
    } WHEN {
        TURN { MOVE(player, MOVE_SURF); MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SURF, player);
...
SINGLE_BATTLE_TEST("Gulp Missile triggered by explosion doesn't freeze the game (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CRAMORANT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_GULP_MISSILE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SURF); MOVE(player, MOVE_EXPLOSION); }
    }
}

SINGLE_BATTLE_TEST("(Gulp Missile) Cramorant in Gorging damages an electric type without paralysing (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_CRAMORANT) { HP(120); MaxHP(250); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_GULP_MISSILE); }
        OPPONENT(SPECIES_EELEKTROSS);
...
SINGLE_BATTLE_TEST("Gulp Missile only changes forms for Cramorant (Traits)")
{
    GIVEN {
        ASSUME(!gAbilitiesInfo[ABILITY_GULP_MISSILE].cantBeSwapped);
        ASSUME(!gAbilitiesInfo[ABILITY_LIGHTNING_ROD].cantBeSwapped);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Gulp Missile: Power Herb does not prevent Cramaront from transforming (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Gulp Missile: Transformed Cramorant Gulping lowers defense and still triggers other effects after (Items)")
{
    // Make sure attacker and target are correct after triggering the ability
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_INFILTRATOR; }
