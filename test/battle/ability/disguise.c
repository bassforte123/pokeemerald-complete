...
SINGLE_BATTLE_TEST("Disguised Mimikyu takes damage from secondary damage without breaking the disguise - Weather (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SANDSTORM) == EFFECT_WEATHER);
        ASSUME(GetMoveWeatherType(MOVE_SANDSTORM) == BATTLE_WEATHER_SANDSTORM);
...
SINGLE_BATTLE_TEST("Disguise does not break from a teammate's Wish (Traits)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WISH) == EFFECT_WISH);
        PLAYER(SPECIES_JIRACHI);
        PLAYER(species) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DISGUISE); HP(219); MaxHP(220); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_WISH); }
        TURN { SWITCH(player, 1); }
