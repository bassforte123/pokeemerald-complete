...
SINGLE_BATTLE_TEST("Synchronize does not inflict status on a target with status immunity ability (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_POISON_GAS) == EFFECT_NON_VOLATILE_STATUS);
        ASSUME(GetMoveNonVolatileStatus(MOVE_POISON_GAS) == MOVE_EFFECT_POISON);
        PLAYER(SPECIES_ABRA) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); }
...
SINGLE_BATTLE_TEST("Synchronize does not trigger when holder inflicts status with its own move (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_THUNDER_WAVE) == EFFECT_NON_VOLATILE_STATUS);
        ASSUME(GetMoveNonVolatileStatus(MOVE_THUNDER_WAVE) == MOVE_EFFECT_PARALYSIS);
...
SINGLE_BATTLE_TEST("Synchronize does not trigger from Toxic Spikes (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TOXIC_SPIKES) == EFFECT_TOXIC_SPIKES);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Synchronize does not trigger from Toxic Orb or Flame Orb (Traits)")
{
    enum Item item;
    u32 status1 = STATUS1_NONE;
    ...
    GIVEN {
        ASSUME(gItemsInfo[ITEM_TOXIC_ORB].holdEffect == HOLD_EFFECT_TOXIC_ORB);
        ASSUME(gItemsInfo[ITEM_FLAME_ORB].holdEffect == HOLD_EFFECT_FLAME_ORB);
        PLAYER(SPECIES_ABRA) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); Item(item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {}
    } SCENE {
...
DOUBLE_BATTLE_TEST("Synchronize will trigger on both targets (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_LUM_BERRY); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_ABRA) { Item(ITEM_LUM_BERRY); Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); }
        OPPONENT(SPECIES_ABRA) { Item(ITEM_LUM_BERRY); Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_MORTAL_SPIN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Synchronize can trigger again during the same attack if user cured it's status (Traits)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        ASSUME(MoveMakesContact(MOVE_MORTAL_SPIN));
        PLAYER(SPECIES_SEISMITOAD) { Ability(ABILITY_SWIFT_SWIM); Innates(ABILITY_POISON_TOUCH); Item(ITEM_LUM_BERRY); }
        OPPONENT(SPECIES_ABRA) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); Item(ITEM_LUM_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_MORTAL_SPIN); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_MORTAL_SPIN, player);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Synchronize does not trigger from Toxic Orb or Flame Orb (Items)")
{
    enum Item item;
    u32 status1 = STATUS1_NONE;
    ...
    GIVEN {
        ASSUME(gItemsInfo[ITEM_TOXIC_ORB].holdEffect == HOLD_EFFECT_TOXIC_ORB);
        ASSUME(gItemsInfo[ITEM_FLAME_ORB].holdEffect == HOLD_EFFECT_FLAME_ORB);
        PLAYER(SPECIES_ABRA) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); Items(ITEM_POTION, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {}
    } SCENE {
...
DOUBLE_BATTLE_TEST("Synchronize will trigger on both targets (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_LUM_BERRY); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_ABRA) { Items(ITEM_POTION, ITEM_LUM_BERRY); Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); }
        OPPONENT(SPECIES_ABRA) { Items(ITEM_POTION, ITEM_LUM_BERRY); Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_MORTAL_SPIN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Synchronize can trigger again during the same attack if user cured it's status (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_MORTAL_SPIN, MOVE_EFFECT_POISON));
        ASSUME(MoveMakesContact(MOVE_MORTAL_SPIN));
        PLAYER(SPECIES_SEISMITOAD) { Ability(ABILITY_SWIFT_SWIM); Innates(ABILITY_POISON_TOUCH); Items(ITEM_POTION, ITEM_LUM_BERRY); }
        OPPONENT(SPECIES_ABRA) { Ability(ABILITY_INNER_FOCUS); Innates(ABILITY_SYNCHRONIZE); Items(ITEM_POTION, ITEM_LUM_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_MORTAL_SPIN); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_MORTAL_SPIN, player);
