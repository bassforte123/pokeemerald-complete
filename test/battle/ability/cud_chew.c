...
SINGLE_BATTLE_TEST("Cud Chew will activate Kee Berry effect again on the next turn")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { Ability(ABILITY_CUD_CHEW); Items(ITEM_KEE_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Cud Chew will activate Oran Berry effect again on the next turn")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { MaxHP(60); HP(60); Ability(ABILITY_CUD_CHEW); Items(ITEM_ORAN_BERRY); }
    } WHEN {
