...
SINGLE_BATTLE_TEST("Cud Chew will activate Kee Berry effect again on the next turn (Multi)")
{
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { Ability(ABILITY_CUD_CHEW); Items(ITEM_GREAT_BALL, ITEM_KEE_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_CELEBRATE);}
    } SCENE {
...
SINGLE_BATTLE_TEST("Cud Chew will activate Oran Berry effect again on the next turn (Multi)")
{
    GIVEN {
        ...
        ASSUME(GetMoveFixedDamage(MOVE_DRAGON_RAGE) == 40);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { MaxHP(60); HP(60); Ability(ABILITY_CUD_CHEW); Items(ITEM_GREAT_BALL, ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_DRAGON_RAGE); }
        TURN { MOVE(player, MOVE_CELEBRATE);}
        TURN {}
