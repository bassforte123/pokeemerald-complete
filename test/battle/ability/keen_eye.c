...
SINGLE_BATTLE_TEST("Keen Eye, Gen9+ Illuminate & Minds Eye prevent accuracy stage reduction from moves (Traits)")
{
    ...
    PARAMETRIZE { species = SPECIES_HITMONCHAN; ability = ABILITY_KEEN_EYE; ability2 = ABILITY_IRON_FIST; }
    PARAMETRIZE { species = SPECIES_STARYU; ability = ABILITY_ILLUMINATE; ability2 = ABILITY_NATURAL_CURE; }
...
SINGLE_BATTLE_TEST("Keen Eye, Gen9+ Illuminate & Minds Eye are ignored by Mold Breaker abilities (Traits)")
{
    ...
    for (j = 0; j < ARRAY_COUNT(moldBreakerAbilities); j++) {
        speciesPlayer = moldBreakerAbilities[j][0]; abilityPlayer = moldBreakerAbilities[j][1];
        PARAMETRIZE { speciesOpponent = SPECIES_HITMONCHAN; abilityOpponent = ABILITY_KEEN_EYE; ability2 = ABILITY_IRON_FIST; }
        PARAMETRIZE { speciesOpponent = SPECIES_STARYU; abilityOpponent = ABILITY_ILLUMINATE; ability2 = ABILITY_NATURAL_CURE; }
        PARAMETRIZE { speciesOpponent = SPECIES_URSALUNA_BLOODMOON; abilityOpponent = ABILITY_MINDS_EYE; ability2 = ABILITY_GUTS; }
    }
    ...
    GIVEN {
        WITH_CONFIG(B_ILLUMINATE_EFFECT, GEN_9);
        PLAYER(speciesPlayer) { Ability(abilityPlayer); }
        OPPONENT(speciesOpponent) { Ability(ability2); Innates(abilityOpponent); }
    } WHEN {
        TURN { MOVE(player, MOVE_SAND_ATTACK); MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
        ABILITY_POPUP(player, abilityPlayer);
