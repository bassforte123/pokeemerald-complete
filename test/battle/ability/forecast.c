...
DOUBLE_BATTLE_TEST("Forecast transforms all Castforms present in weather (Traits)")
{
    ...
    PARAMETRIZE { move = MOVE_SNOWSCAPE; }
    GIVEN {
        PLAYER(SPECIES_CASTFORM_NORMAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); Speed(10); }
        PLAYER(SPECIES_CASTFORM_NORMAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); Speed(5); }
        OPPONENT(SPECIES_CASTFORM_NORMAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); Speed(7); }
        OPPONENT(SPECIES_CASTFORM_NORMAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); Speed(1); }
    } WHEN {
        TURN { MOVE(playerRight, move); }
    } SCENE {
        ABILITY_POPUP(playerLeft, ABILITY_FORECAST);
...
DOUBLE_BATTLE_TEST("Forecast: All Forecast mons revert on the field if Primal Weather is removed due to fainting (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_KYOGRE) { Speed(20); HP(1); Item(ITEM_BLUE_ORB); }
        PLAYER(SPECIES_CASTFORM) { Speed(10); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); }
        OPPONENT(SPECIES_CASTFORM) { Speed(40); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); }
        OPPONENT(SPECIES_CASTFORM) { Speed(30); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FORECAST); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_POUND, target: playerLeft); }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_PRIMAL_REVERSION, playerLeft);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Forecast transforms Castform in primal weather (Items)")
{
    u32 species, item;
    enum Ability ability;
    PARAMETRIZE { species = SPECIES_KYOGRE; ability = ABILITY_PRIMORDIAL_SEA; item = ITEM_BLUE_ORB; }
...
DOUBLE_BATTLE_TEST("Forecast: All Forecast mons revert on the field if Primal Weather is removed due to fainting (Items)")
{
    GIVEN {
        PLAYER(SPECIES_KYOGRE) { Speed(20); HP(1); Items( ITEM_PECHA_BERRY, ITEM_BLUE_ORB); }
        PLAYER(SPECIES_CASTFORM) { Speed(10); Ability(ABILITY_FORECAST); }
        OPPONENT(SPECIES_CASTFORM) { Speed(40); Ability(ABILITY_FORECAST); }
        OPPONENT(SPECIES_CASTFORM) { Speed(30); Ability(ABILITY_FORECAST); }
    } WHEN {
