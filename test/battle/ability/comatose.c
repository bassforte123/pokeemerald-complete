...
SINGLE_BATTLE_TEST("Comatose makes Rest fail (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_REST) == EFFECT_REST);
        PLAYER(SPECIES_KOMALA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMATOSE); HP(1); MaxHP(100); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Comatose isn't affected by Mold Breaker, Turboblaze or Teravolt (Traits)")
{
...
SINGLE_BATTLE_TEST("Comatose isn't affected by Poison Touch + Sunsteel Strike (Traits)")
{
    GIVEN {
        ASSUME(MoveIgnoresTargetAbility(MOVE_SUNSTEEL_STRIKE));
        ASSUME(MoveMakesContact(MOVE_SUNSTEEL_STRIKE));
        PLAYER(SPECIES_KOMALA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_COMATOSE); }
        OPPONENT(SPECIES_CROAGUNK) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_POISON_TOUCH); }
    } WHEN {
...
WILD_BATTLE_TEST("Comatose boosts Dream Ball's multiplier (Traits)")
{
    enum Ability ability;
    u16 species;
    bool32 shouldCatch;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(species) { Ability(ABILITY_LIGHT_METAL); Innates(ability); MaxHP(100); HP(1); }
    } WHEN {
