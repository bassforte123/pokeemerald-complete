...
SINGLE_BATTLE_TEST("Toxic Chain is checked before the move's additional effect (Traits)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_NUZZLE) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMovePower(MOVE_NUZZLE) > 0);
...
SINGLE_BATTLE_TEST("Toxic Chain does not trigger if attack is blocked by Substitute (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SUBSTITUTE) == EFFECT_SUBSTITUTE);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
...
SINGLE_BATTLE_TEST("Toxic Chain is blocked by Shield Dust (Traits)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_GUARD_DOG); Innates(ABILITY_TOXIC_CHAIN); }
...
SINGLE_BATTLE_TEST("Toxic Chain is blocked by Covert Cloak (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_COVERT_CLOAK].holdEffect == HOLD_EFFECT_COVERT_CLOAK);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Toxic Chain inflicts bad poison on any hit of a multi-hit move (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_DOUBLE_SLAP) != DAMAGE_CATEGORY_STATUS);
        ASSUME(IsMultiHitMove(MOVE_DOUBLE_SLAP));
...
SINGLE_BATTLE_TEST("Toxic Chain makes Lum/Pecha Berry trigger before being knocked off (Items)")
{
    enum Item item = ITEM_NONE;
...
SINGLE_BATTLE_TEST("Toxic Chain is blocked by Covert Cloak (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_COVERT_CLOAK].holdEffect == HOLD_EFFECT_COVERT_CLOAK);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
