...
SINGLE_BATTLE_TEST("Moody randomly raises the user's Attack, Defense, Sp. Atk, Sp. Def, or Speed by two stages (Traits)")
{
    ...
    GIVEN {
        WITH_CONFIG(B_MOODY_ACC_EVASION, config);
        PLAYER(SPECIES_OCTILLERY) { Ability(ABILITY_SUCTION_CUPS); Innates(ABILITY_MOODY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Moody activates at the end of the turn when switched in (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_OCTILLERY) { Ability(ABILITY_SUCTION_CUPS); Innates(ABILITY_MOODY); }
