...
SINGLE_BATTLE_TEST("Beads of Ruin reduces Sp. Def if opposing mon's ability doesn't match (Traits)")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_BEADS_OF_RUIN); }
    } WHEN {
        TURN { MOVE(player, MOVE_WATER_GUN); }
...
SINGLE_BATTLE_TEST("Beads of Ruin doesn't activate when dragged out by Mold Breaker attacker (Traits)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CHI_YU) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_BEADS_OF_RUIN); }
        OPPONENT(SPECIES_EXCADRILL) { Ability(ABILITY_SAND_FORCE); Innates(ability); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_DRAGON_TAIL); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_DRAGON_TAIL, opponent);
...
SINGLE_BATTLE_TEST("Beads of Ruin does not apply any damage reduction on an opposing Beads of Ruin user even if it is deactivated (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_DRAGON_TAIL) == EFFECT_HIT_SWITCH_TARGET);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CHI_YU) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_BEADS_OF_RUIN); Moves(MOVE_CELEBRATE); }
