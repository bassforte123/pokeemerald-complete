...
SINGLE_BATTLE_TEST("Stench does not stack with King's Rock")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_GRIMER) { Ability(ABILITY_STENCH); Items(ITEM_KINGS_ROCK); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Stench does not stack with King's Rock (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_GRIMER) { Ability(ABILITY_STICKY_HOLD); Innates(ABILITY_STENCH); Item(ITEM_KINGS_ROCK); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Stench only triggers if target takes damage (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_GRIMER) { Ability(ABILITY_STICKY_HOLD); Innates(ABILITY_STENCH); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_FAKE_OUT, target: opponentLeft);
...
DOUBLE_BATTLE_TEST("Stench doesn't trigger if partner uses a move (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(20); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(SPECIES_GRIMER) { Speed(100); Ability(ABILITY_STICKY_HOLD); Innates(ABILITY_STENCH); }
        OPPONENT(SPECIES_WOBBUFFET) {Speed(50); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_FAKE_OUT, target: opponentLeft);
