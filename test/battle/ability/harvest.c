...
SINGLE_BATTLE_TEST("Harvest has a 50% chance to restore a Berry at the end of the turn")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_EXEGGUTOR) { Ability(ABILITY_HARVEST); MaxHP(500); HP(251); Items(ITEM_SITRUS_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Harvest restores a Berry consumed by Fling")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EXEGGUTOR) { Ability(ABILITY_HARVEST); Items(ITEM_SITRUS_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Harvest restores a Berry consumed by Fling (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        PLAYER(SPECIES_WOBBUFFET);
