...
AI_SINGLE_BATTLE_TEST("Levitate is seen correctly by switch AI")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_PONYTA) { Level(1); Items(ITEM_EJECT_PACK); Moves(MOVE_OVERHEAT); Speed(4); } // Forces switchout
        OPPONENT(SPECIES_VIKAVOLT) { HP(1); Speed(1); Ability(ABILITY_LEVITATE); Moves(MOVE_FLAMETHROWER); Items(item); }
...
SINGLE_BATTLE_TEST("Levitate activates when targeted by ground type moves (Multi)")
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_MUD_SLAP) == TYPE_GROUND);
        PLAYER(SPECIES_LUNATONE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_LEVITATE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_MUD_SLAP); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Levitate does not activate if attacked by an opponent with Mold Breaker (Multi)")
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_MUD_SLAP) == TYPE_GROUND);
        PLAYER(SPECIES_LUNATONE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_LEVITATE); }
        OPPONENT(SPECIES_TINKATON) { Ability(ABILITY_PICKPOCKET); Innates(ABILITY_MOLD_BREAKER); }
    } WHEN {
        TURN { MOVE(player, MOVE_PROTECT); MOVE(opponent, MOVE_MUD_SLAP); }
    } SCENE {
        NONE_OF {
...
DOUBLE_BATTLE_TEST("Levitate does not cause single remaining target to take higher damage (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_REGIROCK)  { Speed(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(4); }
        OPPONENT(SPECIES_LUNATONE) { Speed(3); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_LEVITATE); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); }
    } WHEN {
        TURN { MOVE(playerRight, MOVE_CELEBRATE); MOVE(playerLeft, MOVE_EARTHQUAKE); }
        TURN { MOVE(playerRight, MOVE_MEMENTO, target:opponentLeft); MOVE(playerLeft, MOVE_EARTHQUAKE); }
