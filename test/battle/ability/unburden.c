...
SINGLE_BATTLE_TEST("Unburden doubles speed once user uses item (Multi)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_GRASSY_SEED) == HOLD_EFFECT_SEEDS);
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_UNBURDEN); Items(ITEM_ANTIDOTE, ITEM_GRASSY_SEED); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
        OPPONENT(SPECIES_RILLABOOM) { Speed(7); Ability(ABILITY_GRASSY_SURGE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_U_TURN); SEND_OUT(opponent, 1); }
...
SINGLE_BATTLE_TEST("Unburden doubles speed once user gets their item knocked off (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_UNBURDEN); Items(ITEM_ANTIDOTE, ITEM_POTION); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_KNOCK_OFF); }
        TURN { }
...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is consumed in Neutralizing Gas (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden doubling speed effect is ignored by Neutralizing Gas (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
