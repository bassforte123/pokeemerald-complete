...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is removed in Magic Room (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MAGIC_ROOM) == EFFECT_MAGIC_ROOM);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is removed in Embargo (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EMBARGO) == EFFECT_EMBARGO);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Recycle (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Trick (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        ASSUME(GetMoveEffect(MOVE_TRICK) == EFFECT_TRICK);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Thief (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        ASSUME(GetMoveEffect(MOVE_THIEF) == EFFECT_STEAL_ITEM);
...
DOUBLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Symbiosis (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_FLARE_BOOST); Innates(ABILITY_UNBURDEN); Item(ITEM_POTION); Speed(5); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_TELEPATHY); Innates(ABILITY_SYMBIOSIS); Item(ITEM_ORAN_BERRY); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_FLING, target: opponentLeft); }
...
SINGLE_BATTLE_TEST("Unburden reactivates after consuming an item again (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Unburden doubles speed once user uses item (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_GRASSY_SEED) == HOLD_EFFECT_TERRAIN_SEED);
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_UNBURDEN); Items(ITEM_NONE, ITEM_GRASSY_SEED); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
        OPPONENT(SPECIES_RILLABOOM) { Speed(7); Ability(ABILITY_GRASSY_SURGE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_U_TURN); SEND_OUT(opponent, 1); }
...
SINGLE_BATTLE_TEST("Unburden doubles speed once user gets their item knocked off (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_UNBURDEN); Items(ITEM_NONE, ITEM_POTION); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_KNOCK_OFF); }
        TURN { }
...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is consumed in Neutralizing Gas (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden doubling speed effect is ignored by Neutralizing Gas (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is removed in Magic Room (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MAGIC_ROOM) == EFFECT_MAGIC_ROOM);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden doesn't activate when item is removed in Embargo (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EMBARGO) == EFFECT_EMBARGO);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Recycle (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Trick (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        ASSUME(GetMoveEffect(MOVE_TRICK) == EFFECT_TRICK);
...
SINGLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Thief (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        ASSUME(GetMoveEffect(MOVE_THIEF) == EFFECT_STEAL_ITEM);
...
DOUBLE_BATTLE_TEST("Unburden speed boost is removed after regaining an item with Symbiosis (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_UNBURDEN); Items(ITEM_NONE, ITEM_POTION); Speed(5); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_SYMBIOSIS); Items(ITEM_NONE, ITEM_ORAN_BERRY); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(7); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_FLING, target: opponentLeft); }
...
SINGLE_BATTLE_TEST("Unburden reactivates after consuming an item again (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
