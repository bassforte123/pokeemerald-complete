...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Shed Skin triggers 33% of the time (Gen 3) (Traits)")
{
    u32 status;
    PARAMETRIZE { status = STATUS1_SLEEP; }
    PARAMETRIZE { status = STATUS1_POISON; }
    ...
    GIVEN {
        ...
        ASSUME(MoveMakesContact(MOVE_SCRATCH));
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Status1(status); Ability(ABILITY_UNNERVE); Innates(ABILITY_SHED_SKIN); }
    } WHEN {
        TURN;
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SHED_SKIN);
...
SINGLE_BATTLE_TEST("Shed Skin triggers 30% of the time (Gen 4) (Traits)")
{
    u32 status;
    PARAMETRIZE { status = STATUS1_SLEEP; }
    PARAMETRIZE { status = STATUS1_POISON; }
...
SINGLE_BATTLE_TEST("Shed Skin triggers 33% of the time (Gen 5) (Traits)")
{
    u32 status;
    PARAMETRIZE { status = STATUS1_SLEEP; }
    PARAMETRIZE { status = STATUS1_POISON; }
