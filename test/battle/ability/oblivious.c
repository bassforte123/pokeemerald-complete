...
SINGLE_BATTLE_TEST("Oblivious prevents Infatuation (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ATTRACT) == EFFECT_ATTRACT);
        PLAYER(SPECIES_SLOWPOKE) { Ability(ABILITY_REGENERATOR); Innates(ABILITY_OBLIVIOUS); Gender(MON_MALE); }
        OPPONENT(SPECIES_WOBBUFFET) { Gender(MON_FEMALE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ATTRACT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Oblivious cured infatuation should not persist toxic counter after switching (Traits)")
{
    s16 firstTick, secondTick, postSwitchTick;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Gender(MON_MALE); Status1(STATUS1_TOXIC_POISON); MaxHP(160); HP(160); Speed(100); }
        PLAYER(SPECIES_WYNAUT) { Speed(90); }
        OPPONENT(SPECIES_SLOWPOKE) { Gender(MON_FEMALE); Ability(ABILITY_REGENERATOR); Innates(ABILITY_OBLIVIOUS); Speed(80); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ATTRACT); }
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
        TURN { SWITCH(player, 1); }
