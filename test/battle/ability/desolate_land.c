...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Desolate Land blocks damaging Water-type moves (Items)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Desolate Land blocks damaging Water-type moves and prints the message only once with moves hitting multiple targets (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SURF) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveType(MOVE_SURF) == TYPE_WATER);
...
SINGLE_BATTLE_TEST("Desolate Land does not block a move if Pokémon is asleep and uses a Water-type move (Items)") // Sleep/confusion/paralysis all happen before the check for primal weather
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) {Items(ITEM_PECHA_BERRY, ITEM_RED_ORB);}
        OPPONENT(SPECIES_WOBBUFFET) {Status1(STATUS1_SLEEP);}
...
SINGLE_BATTLE_TEST("Desolate Land will not create a softlock when move in semi invulnerable position is blocked (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Desolate Land is removed immediately if user faints (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Desolate Land blocks weather-setting moves (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_SUNNY_DAY; }
    PARAMETRIZE { move = MOVE_RAIN_DANCE; }
...
SINGLE_BATTLE_TEST("Desolate Land can be replaced by Primordial Sea (Items)")
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) { Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
