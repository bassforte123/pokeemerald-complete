...
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) { HP(1); Items(ITEM_RED_ORB); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
DOUBLE_BATTLE_TEST("Desolate Land blocks damaging Water-type moves and prints the message only once with moves hitting multiple targets (Multi)")
{
    GIVEN {
        ASSUME(!IsBattleMoveStatus(MOVE_SURF));
        ASSUME(GetMoveType(MOVE_SURF) == TYPE_WATER);
        ASSUME(GetMoveTarget(MOVE_SURF) == MOVE_TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_GROUDON) {Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); {Speed(5);}}
        PLAYER(SPECIES_WOBBUFFET) {Speed(5);}
        OPPONENT(SPECIES_WOBBUFFET) {Speed(10);}
        OPPONENT(SPECIES_WOBBUFFET) {Speed(8);}
    } WHEN {
...
SINGLE_BATTLE_TEST("Desolate Land does not block a move if Pokémon is asleep and uses a Water-type move (Multi)") // Sleep/confusion/paralysis all happen before the check for primal weather
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) {Items(ITEM_PECHA_BERRY, ITEM_RED_ORB);}
        OPPONENT(SPECIES_WOBBUFFET) {Status1(STATUS1_SLEEP);}
...
SINGLE_BATTLE_TEST("Desolate Land will not create a softlock when move in semi invulnerable position is blocked (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Desolate Land is removed immediately if user faints (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_POUND); SEND_OUT(player, 1); }
...
SINGLE_BATTLE_TEST("Desolate Land blocks weather-setting moves (Multi)")
{
    u16 move;
    PARAMETRIZE { move = MOVE_SUNNY_DAY; }
    PARAMETRIZE { move = MOVE_RAIN_DANCE; }
...
SINGLE_BATTLE_TEST("Desolate Land can be replaced by Delta Stream (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) { Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Desolate Land can be replaced by Primordial Sea (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_GROUDON) { Items(ITEM_PECHA_BERRY, ITEM_RED_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
