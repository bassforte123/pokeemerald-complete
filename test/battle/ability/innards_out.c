...
SINGLE_BATTLE_TEST("Innards Out counters accumulated multihit damage after all strikes even if fainting on first hit (Traits)")
{
    s16 captured;
    ...
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_DOUBLE_KICK) != DAMAGE_CATEGORY_STATUS);
        ASSUME(gItemsInfo[ITEM_FOCUS_SASH].holdEffect == HOLD_EFFECT_FOCUS_SASH);
        PLAYER(SPECIES_PYUKUMUKU) { HP(2); MaxHP(2); Ability(ABILITY_UNAWARE); Innates(ABILITY_INNARDS_OUT); Item(ITEM_FOCUS_SASH); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_DOUBLE_KICK); SEND_OUT(player, 1); }
...
SINGLE_BATTLE_TEST("Innards Out counters accumulated multihit damage for Parental Bond strikes even if fainting on first hit (Traits)")
{
    s16 captured;
...
SINGLE_BATTLE_TEST("Innards Out includes mid-move Sitrus Berry recovery in accumulated multihit total (Traits)")
{
    s16 captured;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SEISMIC_TOSS) == EFFECT_LEVEL_DAMAGE);
        ASSUME(gItemsInfo[ITEM_SITRUS_BERRY].holdEffect == HOLD_EFFECT_RESTORE_PCT_HP);
        PLAYER(SPECIES_PYUKUMUKU) { HP(30); MaxHP(40); Ability(ABILITY_UNAWARE); Innates(ABILITY_INNARDS_OUT); Item(ITEM_SITRUS_BERRY); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_KANGASKHAN) { Level(20); Item(ITEM_KANGASKHANITE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SEISMIC_TOSS, gimmick: GIMMICK_MEGA); SEND_OUT(player, 1); }
...
SINGLE_BATTLE_TEST("Innards Out should not include Substitute damage in accumulated multihit total (Traits)")
{
    s16 captured;
    ...
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_DOUBLE_KICK) == 2);
        ASSUME(GetMoveCategory(MOVE_DOUBLE_KICK) != DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_PYUKUMUKU) { HP(4); MaxHP(4); Ability(ABILITY_UNAWARE); Innates(ABILITY_INNARDS_OUT); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SUBSTITUTE); }
...
SINGLE_BATTLE_TEST("Innards Out does not damage Magic Guard Pokemon (Traits)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CLEFABLE) { Ability(ABILITY_CUTE_CHARM); Innates(ABILITY_MAGIC_GUARD); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_PSYCHIC); SEND_OUT(player, 1); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_PSYCHIC, opponent);
...
SINGLE_BATTLE_TEST("Innards Out counters accumulated multihit damage after all strikes even if fainting on first hit (Items)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_DOUBLE_KICK) != DAMAGE_CATEGORY_STATUS);
        ASSUME(gItemsInfo[ITEM_FOCUS_SASH].holdEffect == HOLD_EFFECT_FOCUS_SASH);
        PLAYER(SPECIES_PYUKUMUKU) { HP(2); MaxHP(2); Ability(ABILITY_UNAWARE); Innates(ABILITY_INNARDS_OUT); Items(ITEM_NUGGET, ITEM_FOCUS_SASH); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_DOUBLE_KICK); SEND_OUT(player, 1); }
...
SINGLE_BATTLE_TEST("Innards Out counters accumulated multihit damage for Parental Bond strikes even if fainting on first hit (Items)")
{
    s16 captured;
...
SINGLE_BATTLE_TEST("Innards Out includes mid-move Sitrus Berry recovery in accumulated multihit total (Items)")
{
    s16 captured;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SEISMIC_TOSS) == EFFECT_LEVEL_DAMAGE);
        ASSUME(gItemsInfo[ITEM_SITRUS_BERRY].holdEffect == HOLD_EFFECT_RESTORE_PCT_HP);
        PLAYER(SPECIES_PYUKUMUKU) { HP(30); MaxHP(40); Ability(ABILITY_UNAWARE); Innates(ABILITY_INNARDS_OUT); Items(ITEM_NUGGET, ITEM_SITRUS_BERRY); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_KANGASKHAN) { Level(20); Item(ITEM_KANGASKHANITE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SEISMIC_TOSS, gimmick: GIMMICK_MEGA); SEND_OUT(player, 1); }
