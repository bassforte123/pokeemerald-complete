...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Dragonize turns a Normal-type move into a dragon-type move (Trait)")
{
    GIVEN {
        PLAYER(SPECIES_DRUDDIGON);
        OPPONENT(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, opponent);
...
SINGLE_BATTLE_TEST("Dragonize boosts power of affected moves by 20% (Gen7+) or 30% (Gen1-6) (Trait)", s16 damage)
{
    enum Ability ability;
    u32 genConfig;
    PARAMETRIZE { ability = ABILITY_NONE;   genConfig = GEN_7; }
    ...
    GIVEN {
        WITH_CONFIG(B_ATE_MULTIPLIER, genConfig);
        PLAYER(SPECIES_DRUDDIGON) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Moves(MOVE_TACKLE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect Weather Ball's type (Trait)", s16 damage)
{
    enum Move move;
    enum Ability ability;
    PARAMETRIZE { move = MOVE_CELEBRATE; ability = ABILITY_NONE; }
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WEATHER_BALL) == EFFECT_WEATHER_BALL);
        ASSUME(GetSpeciesType(SPECIES_PINSIR, 0) == TYPE_BUG);
        PLAYER(SPECIES_DRUDDIGON) { Ability(ABILITY_LIGHT_METAL); Innates(ability); }
        OPPONENT(SPECIES_PINSIR);
    } WHEN {
        TURN { MOVE(player, move); }
        TURN { MOVE(player, MOVE_WEATHER_BALL); }
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect Natural Gift's type (Trait)")
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_NONE; }
    PARAMETRIZE { ability = ABILITY_DRAGONIZE; }
    GIVEN {
        ...
        ASSUME(gBerries[ItemIdToBerryType(ITEM_ORAN_BERRY)].naturalGiftType == TYPE_POISON);
        ASSUME(GetSpeciesType(SPECIES_BELDUM, 0) == TYPE_STEEL);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(ITEM_ORAN_BERRY); }
        OPPONENT(SPECIES_BELDUM);
    } WHEN {
        TURN { MOVE(player, MOVE_NATURAL_GIFT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect Judgment / Techno Blast / Multi-Attack's type (Trait)")
{
    enum Move move;
    enum Item item;
    PARAMETRIZE { move = MOVE_JUDGMENT; item = ITEM_ZAP_PLATE; }
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_ELECTRIC_MEMORY].secondaryId == TYPE_ELECTRIC);
        ASSUME(GetSpeciesType(SPECIES_DIGLETT, 0) == TYPE_GROUND);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); Item(item); }
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
        TURN { MOVE(player, move); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect Hidden Power's type (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_HIDDEN_POWER) == EFFECT_HIDDEN_POWER);
        ASSUME(gTypesInfo[TYPE_ELECTRIC].isHiddenPowerType == TRUE);
        ASSUME(GetSpeciesType(SPECIES_DIGLETT, 0) == TYPE_GROUND);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); HPIV(31); AttackIV(31); DefenseIV(31); SpAttackIV(30); SpDefenseIV(31); SpeedIV(31); } // HP Electric
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
        TURN { MOVE(player, MOVE_HIDDEN_POWER); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize doesn't override Electrify (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ELECTRIFY) == EFFECT_ELECTRIFY);
        ASSUME(GetSpeciesType(SPECIES_SANDSHREW, 0) == TYPE_GROUND || GetSpeciesType(SPECIES_SANDSHREW, 1) == TYPE_GROUND);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); }
        OPPONENT(SPECIES_SANDSHREW);
    } WHEN {
        TURN { MOVE(opponent, MOVE_ELECTRIFY); MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize overrides Ion Deluge (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ION_DELUGE) == EFFECT_ION_DELUGE);
        ASSUME(GetSpeciesType(SPECIES_DRUDDIGON, 0) == TYPE_DRAGON || GetSpeciesType(SPECIES_DRUDDIGON, 1) == TYPE_DRAGON);
...
SINGLE_BATTLE_TEST("Dragonize changes Tera Blast's type when not Terastallized (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TERA_BLAST) == EFFECT_TERA_BLAST);
        ASSUME(GetMoveType(MOVE_TERA_BLAST) == TYPE_NORMAL);
...
SINGLE_BATTLE_TEST("Dragonize doesn't change Tera Blast's type when Terastallized (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TERA_BLAST) == EFFECT_TERA_BLAST);
        ASSUME(GetMoveType(MOVE_TERA_BLAST) == TYPE_NORMAL);
        ASSUME(GetSpeciesType(SPECIES_MISDREAVUS, 0) == TYPE_GHOST);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); TeraType(TYPE_NORMAL); }
        OPPONENT(SPECIES_MISDREAVUS);
    } WHEN {
        TURN { MOVE(player, MOVE_TERA_BLAST, gimmick: GIMMICK_TERA); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect Terrain Pulse's type (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TERRAIN_PULSE) == EFFECT_TERRAIN_PULSE);
        ASSUME(GetMoveType(MOVE_TERRAIN_PULSE) == TYPE_NORMAL);
...
SINGLE_BATTLE_TEST("Dragonize doesn't affect damaging Z-Move types (Trait)")
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_SCRATCH) == TYPE_NORMAL);
        ASSUME(GetSpeciesType(SPECIES_BAGON, 0) == TYPE_DRAGON || GetSpeciesType(SPECIES_BAGON, 1) == TYPE_DRAGON);
        PLAYER(SPECIES_FERALIGATR_MEGA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DRAGONIZE); Item(ITEM_NORMALIUM_Z); }
        OPPONENT(SPECIES_BAGON);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH, gimmick: GIMMICK_Z_MOVE); }
    } SCENE {
...
TO_DO_BATTLE_TEST("Dragonize doesn't affect Max Strike's type (Trait)");
TO_DO_BATTLE_TEST("Confirm behavioural match with other -ate abilities (Trait)");// we assume that it behaves like Pixilate.
#endif
