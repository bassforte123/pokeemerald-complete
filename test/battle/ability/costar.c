...
DOUBLE_BATTLE_TEST("Costar copies an ally's stat stages after their ability activates upon entering battle (Traits)")
{
    u32 speedLeft, speedRight = 0;
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); HP(1); };
        OPPONENT(SPECIES_WYNAUT) { Speed(10); HP(1); };
        OPPONENT(SPECIES_FLAMIGO) { Speed(speedLeft); Ability(ABILITY_SCRAPPY); Innates(ABILITY_COSTAR); }
        OPPONENT(SPECIES_ZACIAN) { Speed(speedRight); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_INTREPID_SWORD); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_HYPER_VOICE);
            SEND_OUT(opponentLeft, 2);
