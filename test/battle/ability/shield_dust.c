...
AI_SINGLE_BATTLE_TEST("AI will score secondary effects against shield dust correctly (Traits)")
{
    AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_OMNISCIENT);
    GIVEN {
        PLAYER(SPECIES_DUSTOX){ Ability(ABILITY_COMPOUND_EYES); Innates(ABILITY_SHIELD_DUST); Moves(MOVE_GUST); }
        OPPONENT(SPECIES_SUNFLORA){ Ability(ABILITY_CHLOROPHYLL); Innates(ABILITY_EARLY_BIRD); Moves(MOVE_MYSTICAL_FIRE, MOVE_FIERY_DANCE); }
    } WHEN {
        TURN {
            MOVE(player, MOVE_GUST);
            EXPECT_MOVE(opponent, MOVE_FIERY_DANCE);
...
AI_SINGLE_BATTLE_TEST("AI will score secondary effects against shield dust correctly when it has Mold Breaker (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_DUSTOX){ Ability(ABILITY_COMPOUND_EYES); Innates(ABILITY_SHIELD_DUST); Moves(MOVE_GUST); }
        OPPONENT(SPECIES_SUNFLORA){ Ability(ABILITY_CHLOROPHYLL); Innates(ABILITY_MOLD_BREAKER); Moves(MOVE_MYSTICAL_FIRE, MOVE_FIERY_DANCE); }
    } WHEN {
        TURN {
            MOVE(player, MOVE_GUST);
            EXPECT_MOVE(opponent, MOVE_MYSTICAL_FIRE);
