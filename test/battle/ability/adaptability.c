...
SINGLE_BATTLE_TEST("(TERA) Adaptability does not increase non-Tera base STAB beyond 1.5x (Traits)", s16 damage)
{
    u32 move;
    PARAMETRIZE { move = MOVE_GUST; }
    PARAMETRIZE { move = MOVE_WATER_GUN; }
...
SINGLE_BATTLE_TEST("(TERA) Adaptability does not affect Stellar-type moves (Traits)", s16 damage)
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_HYPER_CUTTER; }
    PARAMETRIZE { ability = ABILITY_ADAPTABILITY; }
    GIVEN {
        PLAYER(SPECIES_CRAWDAUNT) { Ability(ABILITY_SHELL_ARMOR); Innates(ability); TeraType(TYPE_STELLAR); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_TERA_BLAST, gimmick: GIMMICK_TERA); }
    } SCENE {
