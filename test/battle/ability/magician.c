...
DOUBLE_BATTLE_TEST("Magician prioritizes opponents over allies among valid targets by speed (Traits)")
{
    u32 playerRightSpeed = 0;
    u32 opponentLeftSpeed = 0;
    u32 opponentRightSpeed = 0;
...
DOUBLE_BATTLE_TEST("Magician follows Trick Room order when choosing between foe targets (Traits)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_SURF) == TARGET_FOES_AND_ALLY);
        ASSUME(GetMoveEffect(MOVE_TRICK_ROOM) == EFFECT_TRICK_ROOM);
        PLAYER(SPECIES_DELPHOX) { Speed(3); Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); Item(ITEM_GREAT_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); Item(ITEM_ULTRA_BALL); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Magician can steal from ally if no opponent is a valid target (Traits)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_SURF) == TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_DELPHOX) { Speed(1); Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); Item(ITEM_POKE_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Magician is blocked by Sticky Hold unless the target faints (Traits)")
{
    bool32 stickyHoldTargetFaints = FALSE;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_DELPHOX) { Speed(1); Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); Item(ITEM_POKE_BALL); }
        OPPONENT(SPECIES_MUK) { Speed(4); HP(stickyHoldTargetFaints ? 1 : 600); Ability(ABILITY_LIQUID_OOZE); Innates(ABILITY_STICKY_HOLD); Item(ITEM_GREAT_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); Item(ITEM_ULTRA_BALL); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SURF); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Magician does not activate with Fling (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); Item(ITEM_PECHA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_POKE_BALL); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Magician does not activate with Natural Gift (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_NATURAL_GIFT) == EFFECT_NATURAL_GIFT);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); Item(ITEM_PECHA_BERRY); }
...
SINGLE_BATTLE_TEST("Magician does not activate with Future Sight or Doom Desire (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_POKE_BALL); }
    } WHEN {
        TURN { MOVE(player, MOVE_FUTURE_SIGHT); }
        TURN {}
...
SINGLE_BATTLE_TEST("Magician activates after the last hit of a multi-hit move (Traits)")
{
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_DOUBLE_KICK) == 2);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician steals before switching with U-turn (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician does not activate if user faints from Rocky Helmet recoil (Traits)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ROCKY_HELMET].holdEffect == HOLD_EFFECT_ROCKY_HELMET);
        PLAYER(SPECIES_DELPHOX) { HP(1); Ability(ABILITY_BLAZE); Innates(ABILITY_MAGICIAN); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_ROCKY_HELMET); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Magician allows activation of stolen Throat Spray (Traits)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_THROAT_SPRAY) == HOLD_EFFECT_THROAT_SPRAY);
        ASSUME(IsSoundMove(MOVE_HYPER_VOICE));
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Magician gets self-damage recoil after stealing Life Orb (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) != DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); Items(ITEM_NONE, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LIFE_ORB); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Magician prioritizes opponents over allies among valid targets by speed (Items)")
{
    u32 playerRightSpeed = 0;
    u32 opponentLeftSpeed = 0;
    u32 opponentRightSpeed = 0;
    ...
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_SURF) == TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_DELPHOX) { Speed(1); Ability(ABILITY_MAGICIAN); Items(ITEM_NONE, ITEM_NONE); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(playerRightSpeed); Items(ITEM_NONE, ITEM_POKE_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(opponentLeftSpeed); Items(ITEM_NONE, ITEM_GREAT_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(opponentRightSpeed); Items(ITEM_NONE, ITEM_ULTRA_BALL); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SURF); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SURF, playerLeft);
...
DOUBLE_BATTLE_TEST("Magician follows Trick Room order when choosing between foe targets (Items)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_SURF) == TARGET_FOES_AND_ALLY);
        ASSUME(GetMoveEffect(MOVE_TRICK_ROOM) == EFFECT_TRICK_ROOM);
        PLAYER(SPECIES_DELPHOX) { Speed(3); Ability(ABILITY_MAGICIAN); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); Items(ITEM_NONE, ITEM_GREAT_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); Items(ITEM_NONE, ITEM_ULTRA_BALL); }
    } WHEN {
        TURN { MOVE(playerRight, MOVE_TRICK_ROOM); }
        TURN { MOVE(playerLeft, MOVE_SURF); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Magician can steal from ally if no opponent is a valid target (Items)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_SURF) == TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_DELPHOX) { Speed(1); Ability(ABILITY_MAGICIAN); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); Items(ITEM_NONE, ITEM_POKE_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SURF); }
...
DOUBLE_BATTLE_TEST("Magician is blocked by Sticky Hold unless the target faints (Items)")
{
    bool32 stickyHoldTargetFaints = FALSE;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); Items(ITEM_NONE, ITEM_POKE_BALL); }
        OPPONENT(SPECIES_MUK) { Speed(4); HP(stickyHoldTargetFaints ? 1 : 600); Ability(ABILITY_STICKY_HOLD); Item(ITEM_GREAT_BALL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); Items(ITEM_NONE, ITEM_ULTRA_BALL); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SURF); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SURF, playerLeft);
...
SINGLE_BATTLE_TEST("Magician does not activate with Fling (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLING) == EFFECT_FLING);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); Items(ITEM_NONE, ITEM_PECHA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_POKE_BALL); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_FLING, player);
...
SINGLE_BATTLE_TEST("Magician does not activate with Natural Gift (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_NATURAL_GIFT) == EFFECT_NATURAL_GIFT);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); Items(ITEM_NONE, ITEM_PECHA_BERRY); }
...
SINGLE_BATTLE_TEST("Magician does not activate with Future Sight or Doom Desire (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician activates after the last hit of a multi-hit move (Items)")
{
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_DOUBLE_KICK) == 2);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician steals before switching with U-turn (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_U_TURN) == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician does not activate if user faints from Rocky Helmet recoil (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ROCKY_HELMET].holdEffect == HOLD_EFFECT_ROCKY_HELMET);
        PLAYER(SPECIES_DELPHOX) { HP(1); Ability(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("Magician allows activation of stolen Throat Spray (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_THROAT_SPRAY) == HOLD_EFFECT_THROAT_SPRAY);
        ASSUME(IsSoundMove(MOVE_HYPER_VOICE));
        PLAYER(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); Items(ITEM_NONE, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_THROAT_SPRAY); }
    } WHEN {
        TURN { MOVE(player, MOVE_HYPER_VOICE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_HYPER_VOICE, player);
        ABILITY_POPUP(player, ABILITY_MAGICIAN);
        MESSAGE("Delphox stole the opposing Wobbuffet's Throat Spray! (Items)");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
    } THEN {
        EXPECT_EQ(player->item, ITEM_NONE);
        EXPECT_EQ(opponent->item, ITEM_NONE);
