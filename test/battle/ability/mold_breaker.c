...
SINGLE_BATTLE_TEST("Mold Breaker cancels damage reduction from Ice Scales (Traits)", s16 damage)
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_FROSMOTH) { Ability(ABILITY_SHIELD_DUST); Innates(ABILITY_ICE_SCALES); }
    } WHEN {
        TURN { MOVE(player, MOVE_PSYCHIC); }
    } SCENE {
        HP_BAR(opponent, captureDamage: &results[i].damage);
...
DOUBLE_BATTLE_TEST("Mold Breaker will deactivate if user faints (Traits)")
{
    u32 move;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STEEL_BEAM) == EFFECT_MAX_HP_50_RECOIL);
        PLAYER(SPECIES_PINSIR) { HP(1); Ability(ABILITY_HYPER_CUTTER); Innates(ABILITY_MOLD_BREAKER); }
        PLAYER(SPECIES_ALTARIA) { Ability(ABILITY_NATURAL_CURE); Innates(ABILITY_COTTON_DOWN); }
        OPPONENT(SPECIES_BELDUM) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CLEAR_BODY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, move, target: playerRight); }
    } SCENE {
...
TO_DO_BATTLE_TEST("TODO: Write more Mold Breaker (Ability) test titles (Traits)")
#endif
