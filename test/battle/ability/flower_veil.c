...
#if MAX_MON_TRAITS > 1
DOUBLE_BATTLE_TEST("Flower Veil prevents status on allied Grass-types - right target (Traits)")
{
    enum Move move;
...
DOUBLE_BATTLE_TEST("Flower Veil prevents status on allied Grass-types - left target (Traits)")
{
    enum Move move;
...
DOUBLE_BATTLE_TEST("Flower Veil's stat reduction protection considers Contrary (Traits)") // Eg. If a move would reduce stats due to Contrary, it will be protected by Mist.
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SWAGGER) == EFFECT_SWAGGER);
        ASSUME(GetSpeciesType(SPECIES_SNIVY, 0) == TYPE_GRASS || GetSpeciesType(SPECIES_SNIVY, 1) == TYPE_GRASS);
...
DOUBLE_BATTLE_TEST("Flower Veil does not prevent self-inflicted stat drops - moves (Traits)")
{
    enum Move move;
    ...
    GIVEN {
        ...
        ASSUME_STAT_CHANGE(MOVE_CURSE, attack: +1, defense: +1, speed: -1);
        PLAYER(SPECIES_CHIKORITA);
        PLAYER(SPECIES_COMFEY) { Ability(ABILITY_NATURAL_CURE); Innates(ABILITY_FLOWER_VEIL); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(playerLeft, move, target: opponentLeft); }
...
DOUBLE_BATTLE_TEST("Flower Veil does not prevent self-inflicted stat drops - abilities (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FORESTS_CURSE) == EFFECT_THIRD_TYPE);
        ASSUME(GetMoveArgType(MOVE_FORESTS_CURSE) == TYPE_GRASS);
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_SLUGMA) { Ability(ABILITY_MAGMA_ARMOR); Innates(ABILITY_WEAK_ARMOR); }
        PLAYER(SPECIES_COMFEY) { Ability(ABILITY_NATURAL_CURE); Innates(ABILITY_FLOWER_VEIL); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
