...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Slow Start halves physical damage during the first five turns (Traits)", s16 damage)
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_OVERCOAT; }
    PARAMETRIZE { ability = ABILITY_SLOW_START; }
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_VAROOM) { Ability(ABILITY_OVERCOAT); Innates(ability); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Slow Start does not reduce special damage (Traits)", s16 damage)
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_OVERCOAT; }
    PARAMETRIZE { ability = ABILITY_SLOW_START; }
...
SINGLE_BATTLE_TEST("Slow Start lasts five turns and resets after switching out (Traits)", s16 damageBefore, s16 damageAfter)
{
    bool32 switchOut;
    PARAMETRIZE { switchOut = FALSE; }
    PARAMETRIZE { switchOut = TRUE; }
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_VAROOM) { Ability(ABILITY_OVERCOAT); Innates(ABILITY_SLOW_START); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Slow Start halves Body Press damage (Traits)", s16 damage)
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_OVERCOAT; }
    PARAMETRIZE { ability = ABILITY_SLOW_START; }
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_BODY_PRESS) == EFFECT_BODY_PRESS);
        ASSUME(GetMoveCategory(MOVE_BODY_PRESS) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_VAROOM) { Ability(ABILITY_OVERCOAT); Innates(ability); Defense(200); }
        OPPONENT(SPECIES_WOBBUFFET) { Defense(100); }
    } WHEN {
        TURN { MOVE(player, MOVE_BODY_PRESS); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Slow Start halves Foul Play damage (Traits)", s16 damage)
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_OVERCOAT; }
    PARAMETRIZE { ability = ABILITY_SLOW_START; }
