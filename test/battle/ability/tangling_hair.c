...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Tangling Hair does not cause Rocky Helmet miss activation (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ROCKY_HELMET].holdEffect == HOLD_EFFECT_ROCKY_HELMET);
...
SINGLE_BATTLE_TEST("Tangling Hair Speed stat drop triggers defiant and keeps original attacker/target (Items)")
{
    GIVEN {
        PLAYER(SPECIES_DUGTRIO) { Ability(ABILITY_TANGLING_HAIR); Items(ITEM_PECHA_BERRY, ITEM_ROCKY_HELMET); }
        OPPONENT(SPECIES_PAWNIARD) { Ability(ABILITY_DEFIANT); }
