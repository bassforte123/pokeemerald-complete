...
AI_MULTI_BATTLE_TEST("Trainer Slide: Multi: Last Low Hp")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(4); HP(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(4); }
        PARTNER(SPECIES_WOBBUFFET) { Speed(3); MaxHP(400); HP(200); }
        OPPONENT_A(SPECIES_WOBBUFFET) { Speed(2); MaxHP(400); HP(200); Moves(MOVE_SURF); }
        OPPONENT_B(SPECIES_WOBBUFFET) { Speed(1); MaxHP(400); HP(200); Moves(MOVE_SUPER_FANG); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SUPER_FANG, target: opponentLeft); SEND_OUT(playerLeft, 1);
            MOVE(playerRight, MOVE_SUPER_FANG, target: opponentRight); }
    } SCENE {
