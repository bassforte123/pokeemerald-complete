...
WILD_BATTLE_TEST("Large exp gains are supported", s32 exp) // #1455
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(1); Items(ITEM_LUCKY_EGG); OTName("Test"); } // OT Name is different so it gets more exp as a traded mon
...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WYNAUT) { Level(40); Items(item); }
        OPPONENT(SPECIES_CATERPIE) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
WILD_BATTLE_TEST("Lucky Egg boosts gained exp points by 50% (Multi)", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(20); Items(ITEM_ORAN_BERRY, item); }
        OPPONENT(SPECIES_CATERPIE) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
WILD_BATTLE_TEST("Large exp gains are supported (Multi)", s32 exp) // #1455
{
    u8 level = 0;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(1); Items(ITEM_ORAN_BERRY, ITEM_LUCKY_EGG); OTName("Test"); } // OT Name is different so it gets more exp as a traded mon
        OPPONENT(SPECIES_BLISSEY) { Level(level); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle (Multi)")
{
    u32 item = 0;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Level(40); Items(ITEM_ORAN_BERRY, item); }
        OPPONENT(SPECIES_CATERPIE) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
