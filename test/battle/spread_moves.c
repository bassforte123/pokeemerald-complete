...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MOON_STONE, ITEM_LUM_BERRY); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MOON_STONE, ITEM_COVERT_CLOAK); }
        OPPONENT(SPECIES_GOLISOPOD) { Ability(ABILITY_EMERGENCY_EXIT); MaxHP(260); HP(131); };
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_MOON_STONE, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_PIKACHU);
    } WHEN {
        TURN {
...
DOUBLE_BATTLE_TEST("Spread Moves: A spread move attack will activate both resist berries (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_GARDEVOIR);
        PLAYER(SPECIES_RALTS);
        OPPONENT(SPECIES_RAICHU) { Items(ITEM_MOON_STONE, ITEM_CHILAN_BERRY); }
        OPPONENT(SPECIES_SANDSLASH) { Items(ITEM_MOON_STONE, ITEM_CHILAN_BERRY); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_HYPER_VOICE); }
        TURN { MOVE(playerLeft, MOVE_HYPER_VOICE); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Spread Moves: If a spread move attack will activate a resist berries on one pokemon, only the damage for that mon will be reduced (Multi)")
{
    s16 opponentLeftDmg[2];
    s16 opponentRightDmg[2];
...
DOUBLE_BATTLE_TEST("Spread Moves: Spread move, Gem Boosted, vs Resist Berries (Multi)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_HYPER_VOICE) == MOVE_TARGET_BOTH);
        PLAYER(SPECIES_WOBBUFFET) { Speed(40); Items(ITEM_MOON_STONE, ITEM_NORMAL_GEM); }
        PLAYER(SPECIES_WYNAUT) { Speed(30); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(20); Items(ITEM_MOON_STONE, ITEM_CHILAN_BERRY); }
        OPPONENT(SPECIES_WYNAUT) { Speed(10); Items(ITEM_MOON_STONE, ITEM_CHILAN_BERRY); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_HYPER_VOICE); }
    } SCENE {
        MESSAGE("The Normal Gem strengthened Wobbuffet's power!");
...
DOUBLE_BATTLE_TEST("Spread Moves: Spread move vs Eiscue and Mimikyu with 1 Eject Button (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(40); }
        PLAYER(SPECIES_WYNAUT) { Speed(30); }
        OPPONENT(SPECIES_MIMIKYU) { Speed(20); Items(ITEM_MOON_STONE, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_EISCUE) { Speed(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_RAZOR_LEAF); SEND_OUT(opponentLeft, 2); }
