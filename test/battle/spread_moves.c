...
DOUBLE_BATTLE_TEST("Spread Moves: A spread move attack will activate both resist berries (Multi)")
{
    s16 opponentLeftDmg[2];
    s16 opponentRightDmg[2];
...
DOUBLE_BATTLE_TEST("Spread Moves: If a spread move attack will activate a resist berries on one Pokémon, only the damage for that mon will be reduced (Multi)")
{
    s16 opponentLeftDmg[2];
    s16 opponentRightDmg[2];
...
DOUBLE_BATTLE_TEST("Spread Moves: Spread move, Gem Boosted, vs Resist Berries (Multi)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_HYPER_VOICE) == MOVE_TARGET_BOTH);
...
DOUBLE_BATTLE_TEST("Spread Moves: Explosion, Gem Boosted, vs Resist Berries (Multi)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_EXPLOSION) == MOVE_TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_WOBBUFFET) { Speed(40); Items(ITEM_ORAN_BERRY, ITEM_NORMAL_GEM); }
