...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Damage calculation matches Gen6+ (Muscle Band, crit) (Items)")
{
    s16 dmg;
    s16 expectedDamage;
    PARAMETRIZE { expectedDamage = 324; }
...
SINGLE_BATTLE_TEST("Punching Glove vs Muscle Band Damage calculation (Items)")
{
    s16 dmgPlayer, dmgOpponent;
    s16 expectedDamagePlayer, expectedDamageOpponent;
    PARAMETRIZE { expectedDamagePlayer = 204, expectedDamageOpponent = 201; }
...
SINGLE_BATTLE_TEST("Gem boosted Damage calculation (Items)")
{
    s16 dmg;
    s16 expectedDamage;
#if I_GEM_BOOST_POWER >= GEN_6
