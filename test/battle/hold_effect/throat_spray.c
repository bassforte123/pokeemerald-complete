...
SINGLE_BATTLE_TEST("Throat Spray increases Sp. Atk by one stage (Multi)")
{
    s16 normalHit;
    s16 boostedHit;
...
SINGLE_BATTLE_TEST("Throat Spray activates when a sound move is used (Multi)")
{
    u32 move;
...
SINGLE_BATTLE_TEST("Throat Spray does not activate if move fails (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_THROAT_SPRAY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Throat Spray does not activate if user flinches (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_THROAT_SPRAY); }
        OPPONENT(SPECIES_WOBBUFFET);
