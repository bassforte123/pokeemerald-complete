...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Throat Spray activates after both hits of a spread move (Items)")
{
    s16 firstHit, secondHit;
...
SINGLE_BATTLE_TEST("Throat Spray increases Sp. Atk by one stage (Items)")
{
    s16 normalHit;
    s16 boostedHit;
...
SINGLE_BATTLE_TEST("Throat Spray activates when a sound move is used (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Throat Spray does not activate if move fails (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_THROAT_SPRAY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Throat Spray does not activate if user flinches (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_THROAT_SPRAY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Throat Spray is not blocked by Sheer Force (Items)")
{
    GIVEN {
        PLAYER(SPECIES_NIDOKING) { Ability(ABILITY_SHEER_FORCE); Items(ITEM_PECHA_BERRY, ITEM_THROAT_SPRAY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_BUG_BUZZ); }
    } SCENE {
