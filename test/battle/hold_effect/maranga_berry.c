...
SINGLE_BATTLE_TEST("Maranga Berry raises the holder's Sp. Def by one stage when hit by a special move")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_MARANGA_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Maranga Berry raises the holder's Sp. Def by two stages with Ripen when hit by a special move")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SWIFT) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_APPLIN) { Items(ITEM_MARANGA_BERRY); Ability(ABILITY_RIPEN); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MARANGA_BERRY); }
...
DOUBLE_BATTLE_TEST("Maranga Berry doesn't trigger if partner was hit")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_MARANGA_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Maranga Berry doesn't trigger if the item hold user used a special move (Multi)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Maranga Berry doesn't trigger if partner was hit (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
