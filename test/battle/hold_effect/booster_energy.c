...
{
    GIVEN {
        PLAYER(SPECIES_IRON_MOTH) { Attack(100); Defense(100); Speed(100); SpAttack(110); SpDefense(100); Ability(ABILITY_QUARK_DRIVE); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_TAPU_KOKO) { Speed(100); Ability(ABILITY_ELECTRIC_SURGE); };
    } WHEN {
        TURN {}
        TURN {}
...
{
    GIVEN {
        PLAYER(SPECIES_RAGING_BOLT) { Attack(100); Defense(100); Speed(100); SpAttack(110); SpDefense(100); Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_TORKOAL) { Speed(100); Ability(ABILITY_DROUGHT); };
    } WHEN {
        TURN {}
        TURN {}
...
SINGLE_BATTLE_TEST("Booster Energy activates Protosynthesis and increases highest stat (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_RAGING_BOLT) { Attack(attack); Defense(defense); Speed(speed); SpAttack(spAttack); SpDefense(spDefense); Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(50); };
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Booster Energy activates Quark Drive and increases highest stat (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_IRON_MOTH) { Attack(attack); Defense(defense); Speed(speed); SpAttack(spAttack); SpDefense(spDefense); Ability(ABILITY_QUARK_DRIVE); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(50); };
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Booster Energy increases special attack by 30% if it is the highest stat (Multi)", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_ROUND) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(species) { Attack(100); Defense(100); Speed(100); SpAttack(110); SpDefense(100); Ability(ability); Items(ITEM_REMEDY, item); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); };
    } WHEN {
        TURN { MOVE(player, MOVE_ROUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Booster Energy increases special defense by 30% if it is the highest stat (Multi)", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_ROUND) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(species) { Attack(100); Defense(100); Speed(100); SpAttack(100); SpDefense(110); Ability(ability); Items(ITEM_REMEDY, item); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); };
    } WHEN {
        TURN { MOVE(opponent, MOVE_ROUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Booster Energy can't be flung if a Paradox species is involved (Multi)")
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_IRON_MOTH].isParadox == TRUE);
        PLAYER(SPECIES_IRON_MOTH);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_FLING); }
        TURN { MOVE(opponent, MOVE_FLING); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Booster Energy triggers correctly for all battlers if multiple fainted the previous turn (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CATERPIE) { HP(1); }
        PLAYER(SPECIES_GOUGING_FIRE) { Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        PLAYER(SPECIES_IRON_MOTH) { Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_FLUTTER_MANE) { Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_CATERPIE);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_EXPLOSION);
               SEND_OUT(opponentRight, 3);
...
{
    GIVEN {
        PLAYER(SPECIES_IRON_MOTH) { Speed(110); Ability(ABILITY_QUARK_DRIVE); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(80); }
        OPPONENT(SPECIES_TAPU_BULU) { Speed(100); Ability(ABILITY_GRASSY_SURGE); }
        OPPONENT(SPECIES_TAPU_KOKO) { Speed(10); Ability(ABILITY_ELECTRIC_SURGE); };
    } WHEN {
...
DOUBLE_BATTLE_TEST("Booster Energy activates on air locked sun (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_RAGING_BOLT) { Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_PSYDUCK) { Ability(ABILITY_CLOUD_NINE); }
        OPPONENT(SPECIES_TORKOAL) { Ability(ABILITY_DROUGHT); };
    } WHEN {
        TURN { SWITCH(playerLeft, 2); }
...
{
    GIVEN {
        PLAYER(SPECIES_RAGING_BOLT) { Speed(110); Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_REMEDY, ITEM_BOOSTER_ENERGY); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(80); }
        OPPONENT(SPECIES_TAPU_BULU) { Speed(100); Ability(ABILITY_GRASSY_SURGE); }
        OPPONENT(SPECIES_TAPU_KOKO) { Speed(10); Ability(ABILITY_ELECTRIC_SURGE); };
    } WHEN {
