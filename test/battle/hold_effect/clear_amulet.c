...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Clear Amulet prevents Intimidate (Items)")
{
    s16 turnOneHit;
    s16 turnTwoHit;
...
SINGLE_BATTLE_TEST("Clear Amulet prevents stat reducing effects (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Clear Amulet prevents secondary effects that reduce stats (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Clear Amulet protects from Protect's secondary effects (Items)")
{
    enum Move move;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_CLEAR_AMULET); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); MOVE(player, MOVE_SCRATCH); }
    } SCENE {
