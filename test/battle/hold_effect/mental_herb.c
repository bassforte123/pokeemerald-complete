...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Mental Herb cures infatuation (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ATTRACT) == EFFECT_ATTRACT);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_MENTAL_HERB); Gender(MON_MALE); }
        OPPONENT(SPECIES_WOBBUFFET) { Gender(MON_FEMALE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_ATTRACT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Mental Herb cures Torment volatile status (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        ASSUME(GetMoveEffect(MOVE_TORMENT) == EFFECT_TORMENT);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_MENTAL_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_TORMENT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Mental Herb clears the Torment timer set by G-Max Meltdown (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        ASSUME(MoveHasAdditionalEffect(MOVE_G_MAX_MELTDOWN, MOVE_EFFECT_TORMENT_SIDE));
...
SINGLE_BATTLE_TEST("Mental Herb cures Disable volatile status (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        ASSUME(GetMoveEffect(MOVE_DISABLE) == EFFECT_DISABLE);
...
SINGLE_BATTLE_TEST("Mental Herb cures Heal Block volatile status (Gen 5+) (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Mental Herb cures Encore volatile status (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        ASSUME(GetMoveEffect(MOVE_ENCORE) == EFFECT_ENCORE);
...
SINGLE_BATTLE_TEST("Mental Herb cures Taunt volatile status (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
...
DOUBLE_BATTLE_TEST("Mental Herb cures volatile statuses in the following order - Infatuation, Torment, Disable, Heal Block, Encore, Taunt (Gen 5+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MENTAL_HERB, GEN_5);
        PLAYER(SPECIES_WOBBUFFET) { Gender(MON_MALE); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET) { Gender(MON_FEMALE); }
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_GREAT_BALL, ITEM_MENTAL_HERB); };
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_CELEBRATE);
               MOVE(opponentLeft, MOVE_HEAL_BLOCK, target: playerLeft);
               MOVE(playerRight, MOVE_DISABLE, target: playerLeft);
