...
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT) { MaxHP(100); HP(2); Items(ITEM_TM_ICE_BEAM, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_ENDURE); MOVE(opponent, MOVE_BITE); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_MIGHTYENA) { MaxHP(100); HP(2); Items(ITEM_TM_ICE_BEAM, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_ENDURE); MOVE(opponent, MOVE_BITE); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Enigma Berry doesn't trigger if partner was hit (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_TM_ICE_BEAM, ITEM_ENIGMA_BERRY); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        NOT ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, opponentRight);
