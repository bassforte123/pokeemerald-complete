...
SINGLE_BATTLE_TEST("Enigma Berry does nothing if not hit by super effective move (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Enigma Berry does nothing if Heal Block applies (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT) { MaxHP(100); HP(2); Items(ITEM_PECHA_BERRY, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Enigma Berry doesn't trigger if partner was hit (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
