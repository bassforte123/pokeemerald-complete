...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Enigma Berry recovers 25% of HP if hit by super effective move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT) { MaxHP(100); HP(2); Items(ITEM_PECHA_BERRY, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_ENDURE); MOVE(opponent, MOVE_BITE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Enigma Berry does nothing if not hit by super effective move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_MIGHTYENA) { MaxHP(100); HP(2); Items(ITEM_PECHA_BERRY, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_ENDURE); MOVE(opponent, MOVE_BITE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Enigma Berry does nothing if Heal Block applies (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT) { MaxHP(100); HP(2); Items(ITEM_PECHA_BERRY, ITEM_ENIGMA_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Enigma Berry doesn't trigger if partner was hit (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_PECHA_BERRY, ITEM_ENIGMA_BERRY); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        NOT ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, opponentRight);
    } THEN {
        EXPECT(opponentRight->items[1]== ITEM_ENIGMA_BERRY);
    }
}
#endif
