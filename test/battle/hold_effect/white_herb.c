...
DOUBLE_BATTLE_TEST("White Herb restores stats after Attack was lowered by Intimidate in doubles (Traits)")
{
    GIVEN {
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WYNAUT) { Item(ITEM_WHITE_HERB); }
        PLAYER(SPECIES_ARBOK) { Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); }
        PLAYER(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { ; }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("White Herb restores stats when they're lowered (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_LEER) == EFFECT_DEFENSE_DOWN);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_LEER); }
    } SCENE {
...
SINGLE_BATTLE_TEST("White Herb restores stats after Attack was lowered by Intimidate in singles (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
...
DOUBLE_BATTLE_TEST("White Herb restores stats after Attack was lowered by Intimidate in doubles (Items)")
{
    GIVEN {
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        PLAYER(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
        PLAYER(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { ; }
...
SINGLE_BATTLE_TEST("White Herb restores stats after Attack was lowered by Intimidate while switching in (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("White Herb restores stats after all hits of a multi hit move happened (Items)")
{
    u16 species;
    enum Ability ability;
...
SINGLE_BATTLE_TEST("White Herb wont have time to activate if it is knocked off or stolen by Thief (Items)")
{
    enum Move move;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        ASSUME(GetMoveEffect(MOVE_THIEF) == EFFECT_STEAL_ITEM);
        PLAYER(SPECIES_SLUGMA) {  Ability(ABILITY_WEAK_ARMOR); Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
    } SCENE {
...
SINGLE_BATTLE_TEST("White Herb wont have time to activate if Magician steals it (Items)")
{
    GIVEN {
        PLAYER(SPECIES_SLUGMA) {  Ability(ABILITY_WEAK_ARMOR); Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_FENNEKIN) { Ability(ABILITY_MAGICIAN); }
...
SINGLE_BATTLE_TEST("White Herb has correct interactions with Intimidate triggered Defiant and Competitive (Items)")
{
    u16 species;
    enum Ability ability;
    ...
    GIVEN {
        PLAYER(species) { Ability(ability); Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { ; }
    } SCENE {
...
DOUBLE_BATTLE_TEST("White Herb is correctly displayed (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_PECHA_BERRY, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(playerRight, MOVE_SUPERPOWER, target: opponentRight); }
