...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_SHELL_BELL); }
...
SINGLE_BATTLE_TEST("Shell Bell restores 1/8 HP at move end, one strike")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(maxHp); HP(hp); Items(ITEM_SHELL_BELL); }
...
SINGLE_BATTLE_TEST("Shell Bell recovers 1/8 of HP from after the last hit from all hits of a multi hit move (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_BULLET_SEED); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Shell Bell recovers no HP if the move did no damage (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); };
...
SINGLE_BATTLE_TEST("Shell Bell activates if it hits a Substitute (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Shell Bell activates after Absorb (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET);
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_GIBLE) { Ability(ABILITY_ROUGH_SKIN); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Shell Bell does not activate on Future Sight if the original user is on the field (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Shell Bell restores 1/8 HP of damage dealt (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(16); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); HP(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Level(16); };
    } WHEN {
        TURN { MOVE(player, MOVE_SEISMIC_TOSS); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Shell Bell doesn't restore HP for damage dealt by a foreseen move (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_WOBBUFFET) { Level(16); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); HP(10); }
...
SINGLE_BATTLE_TEST("Shell Bell does not activate on Future Sight if the original user is not on the field (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
...
DOUBLE_BATTLE_TEST("Shell Bell heals accumulated damage for spread moves (Multi)")
{
    s16 opponentLeftDamage;
    s16 opponentRightDamage;
    s16 playerRightDamage;
    ...
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_DISCHARGE) == MOVE_TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_ARIADOS) { MaxHP(maxHp); HP(initHp); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        PLAYER(SPECIES_WOBBUFFET) {}
        OPPONENT(SPECIES_GYARADOS) {}
        OPPONENT(SPECIES_CHANSEY) {}
    } WHEN {
...
SINGLE_BATTLE_TEST("Shell Bell restores 1/8 HP at move end, one strike (Multi)")
{
    const u16 maxHp = 200;
    u16 hp, opponentHp;
    u16 hpGainFromDamage, hpGainActual;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_DRAGON_RAGE) == EFFECT_FIXED_HP_DAMAGE);
        ASSUME(GetMoveFixedHPDamage(MOVE_DRAGON_RAGE) == 40);
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(maxHp); HP(hp); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET) { MaxHP(maxHp); HP(opponentHp); }
    } WHEN {
        TURN { MOVE(player, MOVE_DRAGON_RAGE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Shell Bell recovers only 1 damage if the move only did 1 damage (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); };
