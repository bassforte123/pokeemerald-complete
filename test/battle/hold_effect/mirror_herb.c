...
SINGLE_BATTLE_TEST("Mirror Herb copies all of foe's positive stat changes in a turn", s16 damage)
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); Items(item); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Mirror Herb copies all of Stuff Cheeks' stat boosts")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_MIRROR_HERB); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MIRROR_HERB); }
...
SINGLE_BATTLE_TEST("Mirror Herb copies all of Stuff Cheeks' stat boosts (Multi)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIECHI_BERRY].holdEffect == HOLD_EFFECT_ATTACK_UP);
