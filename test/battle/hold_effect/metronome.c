...
SINGLE_BATTLE_TEST("Metronome Item gradually boosts power of consecutively used moves by 20%, up to 100% (Multi)")
{
    ...
    u32 j;
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_METRONOME); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        for (j = 0; j < METRONOME_TURNS; ++j) {
            TURN { MOVE(player, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Metronome Item's boost is reset if the attacker uses a different move (Multi)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item's boost is reset if the move fails (Multi)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item counts called moves instead of the calling move (Multi)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item counts charging turn of moves for its attacking turn (Multi)", s16 damage)
{
    u32 item;
...
SINGLE_BATTLE_TEST("Metronome Item doesn't increase damage per hit of multi-hit moves (Multi)")
{
    s16 damage[3];
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FURY_ATTACK) == EFFECT_MULTI_HIT);
