...
SINGLE_BATTLE_TEST("Starf Berry randomly raises the holder's Attack, Defense, Sp. Atk, Sp. Def, or Speed by two stages (Multi)")
{
    PASSES_RANDOMLY(1, 5, RNG_RANDOM_STAT_UP);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_STARF_BERRY); HP(100); MaxHP(400); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_STARF_BERRY); HP(101); MaxHP(400); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_MUNCHLAX) { Items(ITEM_BLK_APRICORN, ITEM_STARF_BERRY); HP(201); MaxHP(400); Ability(ABILITY_GLUTTONY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_FLAPPLE) { Items(ITEM_BLK_APRICORN, ITEM_STARF_BERRY); HP(101); MaxHP(400); Ability(ABILITY_RIPEN); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
