...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LIFE_ORB); }
...
SINGLE_BATTLE_TEST("Life Orb doesn't cause any HP loss if user is unable to attack")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LIFE_ORB); Status1(STATUS1_PARALYSIS); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_POUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Life Orb activates if it hits a Substitute (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb does not activate if using status move on a Substitute (Multi)")
{
    GIVEN {
        ASSUME(MoveIgnoresSubstitute(MOVE_GROWL));
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
...
SINGLE_BATTLE_TEST("Life Orb does not activate if using a status move (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb doesn't cause any HP loss if user is unable to attack (Multi)")
{
    PASSES_RANDOMLY(25, 100, RNG_PARALYSIS);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); Status1(STATUS1_PARALYSIS); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_POUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Life Orb does not activate if on a confusion hit (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb activates if move connected but no damage was dealt (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); }
...
SINGLE_BATTLE_TEST("Life Orb does not activate on a charge turn (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
