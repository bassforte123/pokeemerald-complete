...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ABILITY_PATCH, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_POUND); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Life Orb doesn't cause any HP loss if user is unable to attack (Multi)")
{
    PASSES_RANDOMLY(25, 100, RNG_PARALYSIS);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ABILITY_PATCH, ITEM_LIFE_ORB); Status1(STATUS1_PARALYSIS); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_POUND); }
    } SCENE {
