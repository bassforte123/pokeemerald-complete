...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_REMEDY, ITEM_AIR_BALLOON); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EARTHQUAKE); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_REMEDY, ITEM_AIR_BALLOON); MaxHP(100); HP(1); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_GRASSY_TERRAIN); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Air Balloon pops before it can be stolen with Thief or Covet (Multi)")
{
    u32 move;
    PARAMETRIZE { move = MOVE_THIEF; }
    PARAMETRIZE { move = MOVE_COVET; }
