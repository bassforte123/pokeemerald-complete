...
SINGLE_BATTLE_TEST("Air Balloon pops when Disguise is broken (Traits)")
{
    u32 species, newSpecies;
    PARAMETRIZE { species = SPECIES_MIMIKYU_DISGUISED;       newSpecies = SPECIES_MIMIKYU_BUSTED; }
    PARAMETRIZE { species = SPECIES_MIMIKYU_TOTEM_DISGUISED; newSpecies = SPECIES_MIMIKYU_BUSTED_TOTEM; }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Air Balloon prevents the holder from taking damage from ground type moves (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Air Balloon only displays entry message when user switches in (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Air Balloon pops when the holder is hit by a move that is not ground type (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_AIR_BALLOON); };
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Air Balloon no longer prevents the holder from taking damage from ground type moves once it has been popped (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_AIR_BALLOON); };
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Air Balloon can not be restored with Recycle after it has been popped (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_AIR_BALLOON); };
...
SINGLE_BATTLE_TEST("Air Balloon prevents the user from being healed by Grassy Terrain (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_GRASSY_TERRAIN) == EFFECT_GRASSY_TERRAIN);
...
SINGLE_BATTLE_TEST("Air Balloon pops before it can be stolen with Magician (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_AIR_BALLOON); };
        OPPONENT(SPECIES_DELPHOX) { Ability(ABILITY_MAGICIAN); };
...
SINGLE_BATTLE_TEST("Air Balloon pops before it can be stolen by Thief (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_THIEF) == EFFECT_STEAL_ITEM);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_AIR_BALLOON); };
...
SINGLE_BATTLE_TEST("Air Balloon pops if a damaging move hits the holder's Substitute (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SUBSTITUTE) == EFFECT_SUBSTITUTE);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_AIR_BALLOON); Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
        TURN {MOVE(player, MOVE_SUBSTITUTE); MOVE(opponent, MOVE_EMBER);}
    } SCENE {
...
SINGLE_BATTLE_TEST("Air Balloon pops when Disguise is broken (Items)")
{
    u32 species, newSpecies;
    PARAMETRIZE { species = SPECIES_MIMIKYU_DISGUISED;       newSpecies = SPECIES_MIMIKYU_BUSTED; }
    PARAMETRIZE { species = SPECIES_MIMIKYU_TOTEM_DISGUISED; newSpecies = SPECIES_MIMIKYU_BUSTED_TOTEM; }
    ...
    GIVEN {
        PLAYER(species) { Ability(ABILITY_DISGUISE); Items(ITEM_NONE, ITEM_AIR_BALLOON); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_AERIAL_ACE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Air Balloon pops when the holder faints from a damaging move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_AIR_BALLOON); HP(1); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
