...
SINGLE_BATTLE_TEST("Red Card doesn't prevent Emergency Exit activation when triggered (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Red Card switches the attacker with a random non-fainted replacement (Items)")
{
    PASSES_RANDOMLY(1, 2, RNG_FORCE_RANDOM_SWITCH);
    GIVEN {
...
DOUBLE_BATTLE_TEST("Red Card switches the target with a random non-battler, non-fainted replacement (Items)")
{
    PASSES_RANDOMLY(1, 2, RNG_FORCE_RANDOM_SWITCH);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
...
SINGLE_BATTLE_TEST("Red Card does not activate if holder faints (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Red Card does not activate if attacker faints from recoil (Items)")
{
    GIVEN {
        ASSUME(GetMoveRecoil(MOVE_FLARE_BLITZ) > 0);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
...
SINGLE_BATTLE_TEST("Red Card does not activate if target is behind a Substitute (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card activates after the last hit of a multi-hit move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if no replacements (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if replacements fainted (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if knocked off (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if stolen by a move (Items)")
{
    enum Item item;
    bool32 activate;
    PARAMETRIZE { item = ITEM_NONE; activate = FALSE; }
...
SINGLE_BATTLE_TEST("Red Card does not activate if stolen by Magician (Items)")
{
    enum Item item;
    bool32 activate;
    PARAMETRIZE { item = ITEM_NONE; activate = FALSE; }
...
DOUBLE_BATTLE_TEST("Red Card activates for only the fastest target (Items)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Red Card activates but fails if the attacker is rooted (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        PLAYER(SPECIES_WYNAUT);
...
DOUBLE_BATTLE_TEST("Red Card activates but fails if the attacker has Suction Cups (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        PLAYER(SPECIES_WYNAUT);
...
DOUBLE_BATTLE_TEST("Red Card activates but fails if the attacker has Guard Dog (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        PLAYER(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Red Card does not activate if switched by Dragon Tail (Items)")
{
    bool32 hasWynaut, activate;
    PARAMETRIZE { hasWynaut = TRUE; activate = FALSE; }
    PARAMETRIZE { hasWynaut = FALSE; activate = TRUE; }
...
SINGLE_BATTLE_TEST("Red Card activates and overrides U-turn (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if attacker's Sheer Force applied (Items)")
{
    enum Move move;
    bool32 activate;
    PARAMETRIZE { move = MOVE_SCRATCH; activate = TRUE; }
...
SINGLE_BATTLE_TEST("Red Card is consumed after dragged out replacement has its Speed lowered by Sticky Web (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STICKY_WEB) == EFFECT_STICKY_WEB);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not cause the dragged out mon to lose hp due to it's held Life Orb (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card does not activate if holder is switched in mid-turn (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Red Card doesn't prevent Emergency Exit activation when triggered (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Red Card activates and is consumed but fails if the attacker is Dynamaxed (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Red Card activates before Eject Pack (Items)")
{
    GIVEN {
        ASSUME_MOVE_EFFECT_STAT_CHANGE(MOVE_OVERHEAT, self: TRUE, spAtk: -2);
...
DOUBLE_BATTLE_TEST("Red Card will activate before Eject Button if holder is faster (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(20); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
        ...
        OPPONENT(SPECIES_WYNAUT) { Speed(35); Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_HYPER_VOICE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_HYPER_VOICE, playerLeft);
