...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
SINGLE_BATTLE_TEST("Red Card does not activate if stolen by a move (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, item); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(opponent, MOVE_THIEF); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Red Card does not activate if stolen by Magician (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        OPPONENT(SPECIES_FENNEKIN) { Ability(ABILITY_MAGICIAN); Items(ITEM_BLK_APRICORN, item); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        PLAYER(SPECIES_WYNAUT) { Speed(2); Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); }
        OPPONENT(SPECIES_WYNAUT) { Speed(4); }
        OPPONENT(SPECIES_UNOWN) { Speed(1); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Red Card is consumed after dragged out replacement has its Speed lowered by Sticky Web (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Moves(MOVE_SCRATCH); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_STICKY_WEB); }
        TURN { MOVE(player, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Red Card does not cause the dragged out mon to lose hp due to it's held Life Orb (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_BLK_APRICORN, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_BLK_APRICORN, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
SINGLE_BATTLE_TEST("Red Card prevents Emergency Exit activation when triggered (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_GOLISOPOD) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); Ability(ABILITY_EMERGENCY_EXIT); MaxHP(263); HP(262); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SUPER_FANG); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Red Card activates before Eject Pack (Multi)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffectSelf(MOVE_OVERHEAT, MOVE_EFFECT_SP_ATK_MINUS_2) == TRUE);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
