...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Blunder Policy raises the users speed by 2 stages if the user misses (Items)")
{
    PASSES_RANDOMLY(3, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_FOCUS_BLAST) == 70);
...
SINGLE_BATTLE_TEST("Blunder Policy will never trigger if the move fails due to an immunity (Items)")
{
    PASSES_RANDOMLY(10, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_FOCUS_BLAST) == 70);
...
SINGLE_BATTLE_TEST("Blunder Policy will never trigger if the move fails due to Protect (Items)")
{
    PASSES_RANDOMLY(10, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_FOCUS_BLAST) == 70);
...
SINGLE_BATTLE_TEST("Blunder Policy will never trigger if an OHKO move fails (Items)")
{
    PASSES_RANDOMLY(7, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_FISSURE) == 30);
        ASSUME(GetMoveEffect(MOVE_FISSURE) == EFFECT_OHKO);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_BLUNDER_POLICY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FISSURE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Blunder Policy will never trigger if the move misses due to semi-invulnerability (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FLY) == EFFECT_SEMI_INVULNERABLE);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_BLUNDER_POLICY); Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_FLY); MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Blunder Policy state is cleared between actions if it could not activate (Items)")
{
    PASSES_RANDOMLY(3, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_FOCUS_BLAST) == 70);
...
SINGLE_BATTLE_TEST("Blunder Policy activates if the first strike of Triple Kick misses (Items)")
{
    PASSES_RANDOMLY(1, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_TRIPLE_KICK) == 90);
...
SINGLE_BATTLE_TEST("Blunder Policy does not activate if Triple Kick misses after the first strike (Items)")
{
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_TRIPLE_KICK) == 90);
        ASSUME(GetMoveEffect(MOVE_TRIPLE_KICK) == EFFECT_TRIPLE_KICK);
        ASSUME(MoveMakesContact(MOVE_TRIPLE_KICK));
        PLAYER(SPECIES_MACHAMP) { Ability(ABILITY_NO_GUARD); Items(ITEM_GREAT_BALL, ITEM_BLUNDER_POLICY); }
        OPPONENT(SPECIES_OINKOLOGNE) { Ability(ABILITY_LINGERING_AROMA); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRIPLE_KICK, WITH_RNG(RNG_ACCURACY, FALSE)); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Blunder Policy activates if the first strike of Population Bomb misses (Items)")
{
    PASSES_RANDOMLY(1, 10, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_POPULATION_BOMB) == 90);
...
SINGLE_BATTLE_TEST("Blunder Policy does not activate if Population Bomb misses after the first strike (Items)")
{
    GIVEN {
        ASSUME(GetMoveAccuracy(MOVE_POPULATION_BOMB) == 90);
        ASSUME(GetMoveEffect(MOVE_POPULATION_BOMB) == EFFECT_POPULATION_BOMB);
...
DOUBLE_BATTLE_TEST("Blunder Policy activates for Dragon Darts if one target misses for accuracy but the other target is hit twice (Items)")
{
    struct BattlePokemon *chosenTarget = NULL;
    struct BattlePokemon *finalTarget = NULL;
    enum Item itemLeft, itemRight;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_BLUNDER_POLICY); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, itemLeft); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, itemRight); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_DRAGON_DARTS, target: chosenTarget, hit: FALSE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_DRAGON_DARTS, playerLeft);
