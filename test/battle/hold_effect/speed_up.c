...
SINGLE_BATTLE_TEST("Salac Berry raises the holder's Speed by one stage when HP drops to 1/4 or below")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(160); HP(80); Items(ITEM_SALAC_BERRY); }
...
{
    GIVEN {
        PLAYER(SPECIES_BELLSPROUT) { MaxHP(80); HP(80); Ability(ABILITY_GLUTTONY); Items(ITEM_SALAC_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_APPLIN) { MaxHP(160); HP(80); Ability(ABILITY_RIPEN); Items(ITEM_SALAC_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_DRAGON_RAGE); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Salac Berry does not miss timing miss timing")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { MaxHP(100); HP(26); Items(ITEM_SALAC_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Salac Berry raises Speed by one stage when HP drops to 1/4 or below if holder has Ripen (Multi)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Salac Berry does not miss timing miss timing (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
