...
DOUBLE_BATTLE_TEST("Restore HP Item effects do not miss timing (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { MaxHP(100); HP(51); Items(ITEM_BLK_APRICORN, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_FIRE_PLEDGE, target: opponentRight); MOVE(playerRight, MOVE_GRASS_PLEDGE, target: opponentRight); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Restore HP Item effects do not miss timing after a recoil move (Multi)")
{
    u16 item;
...
#if B_HP_BERRIES <= GEN_3
SINGLE_BATTLE_TEST("Restore HP Berry triggers only during the end turn (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { MaxHP(100); HP(51); Items(ITEM_BLK_APRICORN, ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
        TURN {}
    } SCENE {
