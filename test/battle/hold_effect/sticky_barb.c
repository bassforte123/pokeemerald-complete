...
SINGLE_BATTLE_TEST("Sticky Barb hurts its holder at the end of the turn (Multi)")
{
    s16 damage;
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_STICKY_BARB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Sticky Barb gets transferred if its holder is hit by a contact move (Multi)")
{
    u32 move;
    PARAMETRIZE { move = MOVE_SCRATCH; }
    PARAMETRIZE { move = MOVE_GROWL; }
