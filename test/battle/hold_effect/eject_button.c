...
SINGLE_BATTLE_TEST("Eject Button activates after Wandering Spirit (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Eject Button activates and the attacker takes Life Orb recoil before replacement comes out (Traits)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_LIFE_ORB) == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_SHED_SKIN); Innates(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); SEND_OUT(opponent, 1); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, player);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Eject Button is not triggered when there is nothing to switch in (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(0); }
    } WHEN {
        TURN {
            MOVE(player, MOVE_QUICK_ATTACK);
...
SINGLE_BATTLE_TEST("Eject Button is not activated by a Sheer Force boosted move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_NIDOKING) { Ability(ABILITY_SHEER_FORCE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
...
SINGLE_BATTLE_TEST("Eject Button will not activate under Substitute (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_RAICHU) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
               MOVE(opponent, MOVE_SUBSTITUTE);
...
SINGLE_BATTLE_TEST("Eject Button is not blocked by trapping abilities or moves (Items)")
{
    GIVEN {
        PLAYER(SPECIES_DUGTRIO) { Ability(ABILITY_ARENA_TRAP); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
...
SINGLE_BATTLE_TEST("Eject Button is not triggered after the mon loses Eject Button (Items)")
{
    GIVEN {
        PLAYER(SPECIES_RAICHU);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
...
SINGLE_BATTLE_TEST("Eject Button is not triggered after given to player by Picketpocket (Items)")
{
    GIVEN {
        PLAYER(SPECIES_REGIELEKI) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SNEASEL) { Ability(ABILITY_PICKPOCKET); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
/*SINGLE_BATTLE_TEST("Eject Button has no chance to activate after Dragon Tail (Items)")
{
    GIVEN {
        PLAYER(SPECIES_KOMMO_O);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
...
SINGLE_BATTLE_TEST("Eject Button prevents Volt Switch / U-Turn from activating (Items)")
{
    GIVEN {
        PLAYER(SPECIES_MANECTRIC);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Eject Button is activated before Emergency Exit (Items)")
{
    GIVEN {
        PLAYER(SPECIES_LATIAS);
        OPPONENT(SPECIES_GOLISOPOD) { Ability(ABILITY_EMERGENCY_EXIT); Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(player, MOVE_THUNDERBOLT);
...
SINGLE_BATTLE_TEST("Eject Button is not triggered after High Jump Kick crash damage (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
...
DOUBLE_BATTLE_TEST("Eject Button activation will not trigger an attack from the incoming mon (Items)")
{
    GIVEN {
        PLAYER(SPECIES_TATSUGIRI) { Speed(10); Ability(ABILITY_COMMANDER); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(100); Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_DONDOZO) { Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(50); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
        OPPONENT(SPECIES_WYNAUT) { Speed(1); }
    } WHEN {
        TURN { MOVE(opponentRight, MOVE_MAKE_IT_RAIN); SEND_OUT(playerRight, 2); SEND_OUT(opponentRight, 2); }
...
SINGLE_BATTLE_TEST("Eject Button activates after Wandring Spirit (Items)")
{
    GIVEN {
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_YAMASK_GALAR) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); Ability(ABILITY_WANDERING_SPIRIT); }
    } WHEN {
        TURN {
            SWITCH(opponent, 1);
            MOVE(player, MOVE_DRAGON_CLAW);
...
DOUBLE_BATTLE_TEST("Eject Button will activate before Red Card if holder is faster (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(20); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(5); };
        OPPONENT(SPECIES_WOBBUFFET) { Speed(30); Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WYNAUT) { Speed(25); Items(ITEM_PECHA_BERRY, ITEM_RED_CARD); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_HYPER_VOICE);
...
SINGLE_BATTLE_TEST("Eject Button activates and the attacker takes Life Orb recoil before replacement comes out (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_LIFE_ORB) == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_EKANS) { Items(ITEM_GREAT_BALL, ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); SEND_OUT(opponent, 1); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, player);
