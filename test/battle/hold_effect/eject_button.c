...
SINGLE_BATTLE_TEST("Eject Button is not triggered when there is nothing to switch in (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(0); }
    } WHEN {
        TURN {
            MOVE(player, MOVE_QUICK_ATTACK);
...
SINGLE_BATTLE_TEST("Eject Button will not activate under Substitute (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_RAICHU) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
               MOVE(opponent, MOVE_SUBSTITUTE);
...
{
    GIVEN {
        PLAYER(SPECIES_REGIELEKI) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SNEASEL) { Ability(ABILITY_PICKPOCKET); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Eject Button is activated before Emergency Exit")
{
    GIVEN {
        PLAYER(SPECIES_LATIAS);
        OPPONENT(SPECIES_GOLISOPOD) { Ability(ABILITY_EMERGENCY_EXIT); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(player, MOVE_THUNDERBOLT);
...
DOUBLE_BATTLE_TEST("Eject Button activation will not trigger an attack from the incoming mon (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_TATSUGIRI) { Speed(10); Ability(ABILITY_COMMANDER); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(100); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_DONDOZO) { Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(50); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
        OPPONENT(SPECIES_WYNAUT) { Speed(1); }
    } WHEN {
        TURN { MOVE(opponentRight, MOVE_MAKE_IT_RAIN); SEND_OUT(playerRight, 2); SEND_OUT(opponentRight, 2); }
