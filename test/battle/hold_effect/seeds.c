...
SINGLE_BATTLE_TEST("Electric Seed raises the holder's Defense on Electric Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_ELECTRIC_SURGE; item = ITEM_ELECTRIC_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_ELECTRIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ability); Items(ITEM_BLK_APRICORN, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_ELECTRIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Grassy Seed raises the holder's Defense on Grassy Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_GRASSY_SURGE; item = ITEM_GRASSY_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_GRASSY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_GRASSY_SEED); }
        OPPONENT(SPECIES_TAPU_BULU) { Ability(ability); Items(ITEM_BLK_APRICORN, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_GRASSY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Misty Seed raises the holder's Sp. Defense on Misty Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_MISTY_SURGE; item = ITEM_MISTY_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_MISTY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_MISTY_SEED); }
        OPPONENT(SPECIES_TAPU_FINI) { Ability(ability); Items(ITEM_BLK_APRICORN, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_MISTY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Psychic Seed raises the holder's Sp. Defense on Psychic Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_PSYCHIC_SURGE; item = ITEM_PSYCHIC_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_PSYCHIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_PSYCHIC_SEED); }
        OPPONENT(SPECIES_TAPU_LELE) { Ability(ability); Items(ITEM_BLK_APRICORN, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_PSYCHIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Seeds get consumed in Terrain even if holder is not affected by Terrain (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_PIDGEY) { Items(ITEM_BLK_APRICORN, item); }
        OPPONENT(species) { Ability(ability); }
    } WHEN {
        TURN { }
    } SCENE {
        ...
        EXPECT_EQ(player->items[0], ITEM_BLK_APRICORN);
...
{
    GIVEN {
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_BLK_APRICORN, ITEM_ELECTRIC_SEED); Speed(5); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); Items(ITEM_BLK_APRICORN, ITEM_ELECTRIC_SEED); Speed(10); }
    } WHEN {
        TURN { }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_ELECTRIC_SURGE);
...
SINGLE_BATTLE_TEST("Electric Seed doesn't activate on existing Electric Terrain before user's ability changes the terrain (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_BLK_APRICORN, ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); }
    } WHEN {
        TURN { SWITCH(player, 1); }
    } SCENE {
