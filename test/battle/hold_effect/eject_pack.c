...
SINGLE_BATTLE_TEST("Eject Pack does not cause the new Pokémon to lose HP due to it's held Life Orb (Multi)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_TM_ICE_BEAM, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_OVERHEAT); SEND_OUT(player, 1); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Eject Pack is triggered by self-inflicting stat decreases (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET)  { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(player, MOVE_OVERHEAT); SEND_OUT(player, 1); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Eject Pack will not trigger if the conditions are not met (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_BELDUM) { Ability(ABILITY_CLEAR_BODY); };
        PLAYER(SPECIES_RALTS) { Ability(ABILITY_TRACE); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WYNAUT) { Speed(10); }
    } WHEN {
        TURN { MOVE(player, MOVE_OVERHEAT); SEND_OUT(opponent, 1); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after an ability stat drop (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(speed); Items(ITEM_TM_ICE_BEAM, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WYNAUT)  { Speed(4); }
        OPPONENT(SPECIES_WOBBUFFET)  { Speed(5); }
        OPPONENT(species) { Speed(6); Ability(ability); }
