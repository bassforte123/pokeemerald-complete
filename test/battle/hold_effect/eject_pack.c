...
SINGLE_BATTLE_TEST("Eject Pack does not cause the new Pokémon to lose HP due to it's held Life Orb")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_LIFE_ORB); }
...
SINGLE_BATTLE_TEST("Eject Pack is triggered by self-inflicting stat decreases")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET)  { Items(ITEM_EJECT_PACK); }
...
DOUBLE_BATTLE_TEST("Eject Pack will not trigger if the conditions are not met")
{
    GIVEN {
        ...
        PLAYER(SPECIES_RALTS) { Ability(ABILITY_TRACE); Items(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); Items(ITEM_EJECT_BUTTON); }
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after an ability stat drop")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(speed); Items(ITEM_EJECT_PACK); }
...
DOUBLE_BATTLE_TEST("Eject Pack will trigger on the fastest mon at the end of the turn")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(1); Items(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); Items(ITEM_EJECT_PACK); }
...
SINGLE_BATTLE_TEST("Eject Pack is triggered by self-inflicting stat decreases (Multi)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
...
SINGLE_BATTLE_TEST("Eject Pack will not activate if Parting Shot user can switch out (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Eject Pack will miss timing to switch out user if Eject Button was activated on target (Multi)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after a move stat drop (Multi)")
{
    u32 speed;
...
DOUBLE_BATTLE_TEST("Eject Pack will trigger on the fastest mon at the end of the turn (Multi)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
