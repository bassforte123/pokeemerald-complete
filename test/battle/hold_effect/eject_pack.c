...
DOUBLE_BATTLE_TEST("Eject Pack will not trigger if the conditions are not met (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_BELDUM) { Ability(ABILITY_CLEAR_BODY); };
        PLAYER(SPECIES_RALTS) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CLEAR_BODY); Item(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after intimidate (switch in after fainting) (Traits)")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WYNAUT)  { HP(1); Speed(4); }
        OPPONENT(SPECIES_WOBBUFFET)  { Speed(5); }
        OPPONENT(SPECIES_EKANS) { Speed(6); Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_POUND, target: opponentLeft);
            SEND_OUT(opponentLeft, 2);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Eject Pack does not cause the new Pokémon to lose HP due to it's held Life Orb (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
...
SINGLE_BATTLE_TEST("Eject Pack does not activate if there are no Pokémon left to battle (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET) { HP(0); }
...
SINGLE_BATTLE_TEST("Eject Pack is triggered by self-inflicting stat decreases (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET)  { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(player, MOVE_OVERHEAT); SEND_OUT(player, 1); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Eject Pack switches the user out even if rooted by Ingrain (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_INGRAIN) == EFFECT_INGRAIN);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
...
SINGLE_BATTLE_TEST("Eject Pack will miss timing to switch out user if Emergency Exit was activated on target (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Eject Pack activates once intimidate mon switches in (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Eject Pack will not activate if Parting Shot user can switch out (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Eject Pack will not trigger if the conditions are not met (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_BELDUM) { Ability(ABILITY_CLEAR_BODY); };
        PLAYER(SPECIES_RALTS) { Ability(ABILITY_TRACE); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); }
...
SINGLE_BATTLE_TEST("Eject Pack will miss timing to switch out user if Eject Button was activated on target (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); Items(ITEM_PECHA_BERRY, ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_WYNAUT) { Speed(10); }
    } WHEN {
        TURN { MOVE(player, MOVE_OVERHEAT); SEND_OUT(opponent, 1); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after an ability stat drop (Items)")
{
    u32 speed;
    u32 species, ability;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(speed); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WYNAUT)  { Speed(4); }
        OPPONENT(SPECIES_WOBBUFFET)  { Speed(5); }
        OPPONENT(species) { Speed(6); Ability(ability); }
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after a move stat drop (Items)")
{
    u32 speed;
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after intimidate (switch in after fainting) (Items)")
{
    u32 speed;
...
DOUBLE_BATTLE_TEST("Eject Pack: Only the fastest Eject Pack will activate after intimidate (switch in after 2 mons fainted) (Items)")
{
    u32 speed;
...
SINGLE_BATTLE_TEST("Eject Pack does not activate if mon is switched in due to Eject Button (Items)")
{
    GIVEN {
        PLAYER(SPECIES_DUGTRIO) { Ability(ABILITY_ARENA_TRAP); }
...
DOUBLE_BATTLE_TEST("Eject Pack will trigger on the fastest mon at the end of the turn (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_SYRUP_BOMB, MOVE_EFFECT_SYRUP_BOMB) == TRUE);
        PLAYER(SPECIES_WOBBUFFET) { Speed(1); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); Items(ITEM_PECHA_BERRY, ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); }
        OPPONENT(SPECIES_WYNAUT) { Speed(4); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); }
    } WHEN {
