...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Zoom Lens boosts accuracy when the target has already moved this turn (Items)")
{
    PASSES_RANDOMLY(55 * (100 + gItemsInfo[ITEM_ZOOM_LENS].holdEffectParam) / 100, 100, RNG_ACCURACY);
    GIVEN {
        ASSUME(GetMovePriority(MOVE_QUICK_ATTACK) > GetMovePriority(MOVE_SING));
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_ZOOM_LENS); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_QUICK_ATTACK); MOVE(player, MOVE_SING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Zoom Lens does not boost accuracy against a target that switched in this turn (Items)")
{
    PASSES_RANDOMLY(55, 100, RNG_ACCURACY);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_ZOOM_LENS); }
