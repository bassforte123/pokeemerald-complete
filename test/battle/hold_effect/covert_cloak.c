...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Covert Cloak blocks secondary effects (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_NUZZLE; }
    PARAMETRIZE { move = MOVE_INFERNO; }
...
SINGLE_BATTLE_TEST("Covert Cloak does not block primary effects (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_INFESTATION; }
    PARAMETRIZE { move = MOVE_THOUSAND_ARROWS; }
...
SINGLE_BATTLE_TEST("Covert Cloak does not block self-targeting effects, primary or secondary (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_POWER_UP_PUNCH; }
    PARAMETRIZE { move = MOVE_FLAME_CHARGE; }
...
DOUBLE_BATTLE_TEST("Covert Cloak does or does not block Sparkling Aria depending on number of targets hit (Items)")
{
    enum Move moveToUse;
    PARAMETRIZE { moveToUse = MOVE_FINAL_GAMBIT; }
    PARAMETRIZE { moveToUse = MOVE_SCRATCH; }
...
DOUBLE_BATTLE_TEST("Covert Cloak does block Sparkling Aria when only one mon is hit (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_PROTECT; }
    PARAMETRIZE { move = MOVE_FLY; }
...
SINGLE_BATTLE_TEST("Covert Cloak blocks Sparkling Aria in singles (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_COVERT_CLOAK); Status1(STATUS1_BURN); }
...
SINGLE_BATTLE_TEST("Covert Cloak does not prevent ability stat changes (Items)")
{
    GIVEN {
