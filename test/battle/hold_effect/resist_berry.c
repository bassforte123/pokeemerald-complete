...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Weakness berries decrease the base power of moves by half (Items)", s16 damage)
{
    enum Move move = MOVE_NONE;
    enum Item item = ITEM_NONE;
    u32 defender = 0;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(defender) { Items(ITEM_PECHA_BERRY, item); }
    } WHEN {
        TURN { MOVE(player, move); }
    } SCENE {
        if (1 == i % 2) {
...
SINGLE_BATTLE_TEST("Weakness berries do not activate unless a move is super effective (Items)", s16 damage)
{
    enum Move move = MOVE_NONE;
    enum Item item = ITEM_NONE;
    u32 defender = 0;
...
SINGLE_BATTLE_TEST("Weakness berries do not decrease the power of Struggle (Items)", s16 damage)
{
    enum Item item = ITEM_NONE;
...
SINGLE_BATTLE_TEST("Weakness berries do not activate if Disguise blocks the damage (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_BABIRI_BERRY) == HOLD_EFFECT_RESIST_BERRY);
        ASSUME(GetItemHoldEffectParam(ITEM_BABIRI_BERRY) == TYPE_STEEL);
        ASSUME(GetMoveType(MOVE_METAL_CLAW) == TYPE_STEEL);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_MIMIKYU) { Items(ITEM_PECHA_BERRY, ITEM_BABIRI_BERRY); Ability(ABILITY_DISGUISE); }
    } WHEN {
        TURN { MOVE(player, MOVE_METAL_CLAW); }
    } SCENE {
        NONE_OF {
