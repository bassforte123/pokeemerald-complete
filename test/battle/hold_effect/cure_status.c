...
SINGLE_BATTLE_TEST("Pecha and Lum Berries cure bad poison (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Rawst and Lum Berries cure burn (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Aspear and Lum Berries cure freeze or frostbite (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Chesto and Lum Berries cure sleep (Multi)")
{
    u16 item;
...
TO_DO_BATTLE_TEST("Chesto and Lum Berries don't trigger if the holder has Comatose (Multi)")

SINGLE_BATTLE_TEST("Cheri and Lum Berries cure paralysis (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Perism and Lum Berries cure confusion (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Berry hold effect cures status if a Pokémon enters a battle (Multi)")
{
    u16 status;
    u16 item;
...
SINGLE_BATTLE_TEST("Opponent Pokemon can be further poisoned with Toxic spikes after a status healing hold effect was previously used (Multi)")
{
    u16 item;
...
// Basically same as above, but with the sides reversed.
SINGLE_BATTLE_TEST("Player Pokemon can be further poisoned with Toxic spikes after a status healing hold effect was previously used (Multi)")
{
    u16 item;
...
DOUBLE_BATTLE_TEST("Lum Berry correctly cures all battlers if multiple fainted the previous turn (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CATERPIE) { HP(1); }
...
SINGLE_BATTLE_TEST("Lum Berry properly cures a battler affected by a non-volatiles status and confusion (Multi)")
{
    u32 status;
    PARAMETRIZE { status = STATUS1_BURN;}
    PARAMETRIZE { status = STATUS1_FREEZE;}
