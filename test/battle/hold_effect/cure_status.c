...
SINGLE_BATTLE_TEST("Lum Berry properly cures a battler affected by only confusion")
{
    GIVEN {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Pecha and Lum Berries cure poison (Items)")
{
    enum Item item;
    ...
    GIVEN {
        ASSUME(gItemsInfo[ITEM_PECHA_BERRY].holdEffect == HOLD_EFFECT_CURE_PSN);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY, item); }
    } WHEN {
        TURN { MOVE(player, MOVE_POISON_POWDER); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_POISON_POWDER, player);
...
SINGLE_BATTLE_TEST("Pecha and Lum Berries cure bad poison (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Rawst and Lum Berries cure burn (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Aspear and Lum Berries cure freeze or frostbite (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Chesto and Lum Berries cure sleep (Items)")
{
    enum Item item;
...
TO_DO_BATTLE_TEST("Chesto and Lum Berries don't trigger if the holder has Comatose (Items)")

SINGLE_BATTLE_TEST("Cheri and Lum Berries cure paralysis (Items)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Perism and Lum Berries cure confusion (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Berry hold effect cures status if a Pokémon enters a battle (Items)")
{
    u16 status;
    enum Item item;
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_PECHA_BERRY].holdEffect == HOLD_EFFECT_CURE_PSN);
        ASSUME(gItemsInfo[ITEM_CHESTO_BERRY].holdEffect == HOLD_EFFECT_CURE_SLP);
        PLAYER(SPECIES_WOBBUFFET) { Status1(status); Items(ITEM_ORAN_BERRY, ITEM_LUM_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Status1(status); Items(ITEM_ORAN_BERRY, item); }
    } WHEN {
        TURN { }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_BERRY, player);
...
SINGLE_BATTLE_TEST("Opponent Pokemon can be further poisoned with Toxic spikes after a status healing hold effect was previously used (Items)")
{
    enum Item item;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_ORAN_BERRY, item); }
    } WHEN {
        TURN { MOVE(player, MOVE_TOXIC_SPIKES); }
        TURN { SWITCH(opponent, 1); }
        TURN { SWITCH(opponent, 0); }
...
// Basically same as above, but with the sides reversed.
SINGLE_BATTLE_TEST("Player Pokemon can be further poisoned with Toxic spikes after a status healing hold effect was previously used (Items)")
{
    enum Item item;
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_LUM_BERRY].holdEffect == HOLD_EFFECT_CURE_STATUS);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_ORAN_BERRY, item); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_TOXIC_SPIKES); }
...
DOUBLE_BATTLE_TEST("Lum Berry correctly cures all battlers if multiple fainted the previous turn (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CATERPIE) { HP(1); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY, ITEM_LUM_BERRY); Status1(STATUS1_BURN); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY, ITEM_LUM_BERRY); Status1(STATUS1_POISON); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY, ITEM_LUM_BERRY); Status1(STATUS1_PARALYSIS); }
        OPPONENT(SPECIES_CATERPIE);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_EXPLOSION);
               SEND_OUT(opponentRight, 3);
...
SINGLE_BATTLE_TEST("Lum Berry properly cures a battler affected by only confusion (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_CONFUSE_RAY) == EFFECT_CONFUSE);
        PLAYER(SPECIES_WOBBUFFET) { Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY, ITEM_LUM_BERRY); Speed(2); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_CONFUSE_RAY); MOVE(player, MOVE_CELEBRATE, WITH_RNG(RNG_FROZEN, 0)); }
        TURN { MOVE(opponent, MOVE_SWITCHEROO); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Lum Berry properly cures a battler affected by a non-volatiles status and confusion (Items)")
{
    u32 status;
    PARAMETRIZE { status = STATUS1_BURN;}
    PARAMETRIZE { status = STATUS1_FREEZE;}
