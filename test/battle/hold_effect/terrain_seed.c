...
SINGLE_BATTLE_TEST("Electric Seed raises the holder's Defense on Electric Terrain")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ELECTRIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ability); Items(item); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Grassy Seed raises the holder's Defense on Grassy Terrain")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GRASSY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GRASSY_SEED); }
        OPPONENT(SPECIES_TAPU_BULU) { Ability(ability); Items(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
...
SINGLE_BATTLE_TEST("Misty Seed raises the holder's Sp. Defense on Misty Terrain")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MISTY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_MISTY_SEED); }
        OPPONENT(SPECIES_TAPU_FINI) { Ability(ability); Items(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
...
SINGLE_BATTLE_TEST("Psychic Seed raises the holder's Sp. Defense on Psychic Terrain")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PSYCHIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PSYCHIC_SEED); }
        OPPONENT(SPECIES_TAPU_LELE) { Ability(ability); Items(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
...
SINGLE_BATTLE_TEST("Seeds get consumed in Terrain even if holder is not affected by Terrain")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_PIDGEY) { Items(item); }
...
{
    GIVEN {
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_ELECTRIC_SEED); Speed(5); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); Items(ITEM_ELECTRIC_SEED); Speed(10); }
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Electric Seed doesn't activate on existing Electric Terrain before user's ability changes the terrain")
{
    GIVEN {
        ...
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_ELECTRIC_SEED); }
...
SINGLE_BATTLE_TEST("Electric Seed raises the holder's Defense on Electric Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_ELECTRIC_SURGE; item = ITEM_ELECTRIC_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ELECTRIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ability); Items(ITEM_PECHA_BERRY, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_ELECTRIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Grassy Seed raises the holder's Defense on Grassy Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_GRASSY_SURGE; item = ITEM_GRASSY_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_GRASSY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_GRASSY_SEED); }
        OPPONENT(SPECIES_TAPU_BULU) { Ability(ability); Items(ITEM_PECHA_BERRY, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_GRASSY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Misty Seed raises the holder's Sp. Defense on Misty Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_MISTY_SURGE; item = ITEM_MISTY_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_MISTY_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_MISTY_SEED); }
        OPPONENT(SPECIES_TAPU_FINI) { Ability(ability); Items(ITEM_PECHA_BERRY, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_MISTY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Psychic Seed raises the holder's Sp. Defense on Psychic Terrain (Multi)")
{
    ...
    PARAMETRIZE { ability = ABILITY_PSYCHIC_SURGE; item = ITEM_PSYCHIC_SEED; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_PSYCHIC_SEED); }
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_PSYCHIC_SEED); }
        OPPONENT(SPECIES_TAPU_LELE) { Ability(ability); Items(ITEM_PECHA_BERRY, item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_PSYCHIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Seeds get consumed in Terrain even if holder is not affected by Terrain (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_PIDGEY, 0) == TYPE_FLYING || GetSpeciesType(SPECIES_PIDGEY, 1) == TYPE_FLYING);
        PLAYER(SPECIES_PIDGEY) { Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(species) { Ability(ability); }
    } WHEN {
        TURN { }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_PECHA_BERRY, ITEM_ELECTRIC_SEED); Speed(5); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); Items(ITEM_PECHA_BERRY, ITEM_ELECTRIC_SEED); Speed(10); }
    } WHEN {
        TURN { }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_ELECTRIC_SURGE);
...
SINGLE_BATTLE_TEST("Electric Seed doesn't activate on existing Electric Terrain before user's ability changes the terrain (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_GRASSY_SURGE); Items(ITEM_PECHA_BERRY, ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); }
    } WHEN {
        TURN { SWITCH(player, 1); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Electric Seed raises the holder's Defense on Electric Terrain (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_ELECTRIC_SEED); }
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_ELECTRIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Grassy Seed raises the holder's Defense on Grassy Terrain (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_GRASSY_SEED); }
        OPPONENT(SPECIES_TAPU_BULU) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_GRASSY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Misty Seed raises the holder's Sp. Defense on Misty Terrain (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_MISTY_SEED); }
        OPPONENT(SPECIES_TAPU_FINI) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_MISTY_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Psychic Seed raises the holder's Sp. Defense on Psychic Terrain (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_PSYCHIC_SEED); }
        OPPONENT(SPECIES_TAPU_LELE) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(item); }
    } WHEN {
        if (ability == ABILITY_TELEPATHY)
            TURN { MOVE(player, MOVE_PSYCHIC_TERRAIN); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Electric Seed is consumed on Electric Terrain before other abilities change the terrain (Multi)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_TAPU_KOKO) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_ELECTRIC_SURGE); Item(ITEM_ELECTRIC_SEED); Speed(10); }
    } WHEN {
        TURN { }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_ELECTRIC_SURGE);
...
SINGLE_BATTLE_TEST("Electric Seed doesn't activate on existing Electric Terrain before user's ability changes the terrain (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_TAPU_BULU) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_GRASSY_SURGE); Item(ITEM_ELECTRIC_SEED); }
