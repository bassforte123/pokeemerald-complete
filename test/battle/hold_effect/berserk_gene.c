...
SINGLE_BATTLE_TEST("Berserk Gene does not confuse a Pokemon with Own Tempo but still raises attack sharply in a single battle (Traits)", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_SLOWBRO) { Ability(ABILITY_REGENERATOR); Innates(ABILITY_OWN_TEMPO); Item(item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(player, MOVE_SCRATCH);
...
SINGLE_BATTLE_TEST("Berserk Gene does not cause an infinite loop (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_BESTOW) == EFFECT_BESTOW);
        PLAYER(SPECIES_TOXEL) { Item(ITEM_BERSERK_GENE); Ability(ABILITY_RATTLED); Innates(ABILITY_KLUTZ); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Berserk Gene sharply raises attack at the start of a single battle (Items)", s16 damage)
{
    enum Item item;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_BERSERK_GENE; }
...
DOUBLE_BATTLE_TEST("Berserk Gene sharply raises attack at the start of a double battle (Items)", s16 damage)
{
    enum Item item;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_BERSERK_GENE; }
...
SINGLE_BATTLE_TEST("Berserk Gene activates on switch in (Items)", s16 damage)
{
    enum Item item;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_BERSERK_GENE; }
...
SINGLE_BATTLE_TEST("Berserk Gene does not confuse a Pokemon with Own Tempo but still raises attack sharply in a single battle (Items)", s16 damage)
{
    enum Item item;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_BERSERK_GENE; }
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_SLOWBRO) { Ability(ABILITY_OWN_TEMPO); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(player, MOVE_SCRATCH);
...
DOUBLE_BATTLE_TEST("Berserk Gene does not confuse a Pokemon with Own Tempo but still raises attack sharply in a double battle (Items)", s16 damage)
{
    enum Item item;
    bool8 positionLeft = FALSE;
...
SINGLE_BATTLE_TEST("Berserk Gene does not confuse on Misty Terrain but still raises attack sharply (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_TAPU_FINI) { Ability(ABILITY_MISTY_SURGE); Items(ITEM_PECHA_BERRY, ITEM_BERSERK_GENE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {
            MOVE(player, MOVE_SCRATCH);
...
SINGLE_BATTLE_TEST("Berserk Gene does not confuse when Safeguard is active (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_BERSERK_GENE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SAFEGUARD); }
        TURN { SWITCH(player, 1); }
...
SINGLE_BATTLE_TEST("Berserk Gene causes confusion for more than 5 turns (Items)") // how else would be check for infinite?
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_BERSERK_GENE); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Berserk Gene causes infinite confusion (Items)") // check if bit is set
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_BERSERK_GENE); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Berserk Gene causes confusion timer to not tick down (Items)", u32 confusionTurns)
{
    u32 turns;
    PARAMETRIZE { turns = 1; }
    PARAMETRIZE { turns = 2; }
...
SINGLE_BATTLE_TEST("Berserk Gene does not cause an infinite loop (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_BESTOW) == EFFECT_BESTOW);
        PLAYER(SPECIES_TOXEL) { Items(ITEM_PECHA_BERRY, ITEM_BERSERK_GENE); Ability(ABILITY_KLUTZ); }
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Berserker Gene confusion can be healed with bag items (Items)")
{
    enum Item item;
    PARAMETRIZE { item = ITEM_FULL_HEAL; }
    PARAMETRIZE { item = ITEM_HEAL_POWDER; }
...
SINGLE_BATTLE_TEST("Berserker Gene confusion can be healed with used held items (Items)")
{
    enum Item item;
    PARAMETRIZE { item = ITEM_PERSIM_BERRY; }
    PARAMETRIZE { item = ITEM_LUM_BERRY; }
