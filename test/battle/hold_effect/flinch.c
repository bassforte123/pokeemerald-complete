...
#if MAX_MON_TRAITS > 1
DOUBLE_BATTLE_TEST("Kings Rock flinch chance boosted by Serene Grace does not stack with rainbow (Traits)")
{
    PASSES_RANDOMLY(20, 100, RNG_HOLD_EFFECT_FLINCH);
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WATER_PLEDGE) == EFFECT_PLEDGE);
        ASSUME(GetMoveEffect(MOVE_FIRE_PLEDGE) == EFFECT_PLEDGE);
        PLAYER(SPECIES_TOGEPI) { Speed(4); Ability(ABILITY_SUPER_LUCK); Innates(ABILITY_SERENE_GRACE); Item(ITEM_KINGS_ROCK); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Kings Rock holder will flinch the target 10% of the time (Items)")
{
    PASSES_RANDOMLY(10, 100, RNG_HOLD_EFFECT_FLINCH);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_KINGS_ROCK); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Kings Rock does not increase flinch chance of a move that has the flinch effect (Items)")
{
    PASSES_RANDOMLY(30, 100, RNG_SECONDARY_EFFECT);
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_HEADBUTT, MOVE_EFFECT_FLINCH));
...
DOUBLE_BATTLE_TEST("Kings Rock flinch chance boosted by Serene Grace does not stack with rainbow (Items)")
{
    PASSES_RANDOMLY(20, 100, RNG_HOLD_EFFECT_FLINCH);
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WATER_PLEDGE) == EFFECT_PLEDGE);
        ASSUME(GetMoveEffect(MOVE_FIRE_PLEDGE) == EFFECT_PLEDGE);
        PLAYER(SPECIES_TOGEPI) { Speed(4); Ability(ABILITY_SERENE_GRACE); Items(ITEM_GREAT_BALL, ITEM_KINGS_ROCK); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
