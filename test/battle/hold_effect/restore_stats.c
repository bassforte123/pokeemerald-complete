...
SINGLE_BATTLE_TEST("White Herb restores stats when they're lowered (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_LEER) == EFFECT_DEFENSE_DOWN);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_LEER); }
    } SCENE {
...
{
    GIVEN {
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        PLAYER(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
        PLAYER(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { ; }
...
SINGLE_BATTLE_TEST("White Herb wont have time to activate if it is knocked off or stolen by Thief (Multi)")
{
    ...
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_THIEF, MOVE_EFFECT_STEAL_ITEM) == TRUE);
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        PLAYER(SPECIES_SLUGMA) {  Ability(ABILITY_WEAK_ARMOR); Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
    } SCENE {
...
SINGLE_BATTLE_TEST("White Herb has correct interactions with Intimidate triggered Defiant and Competitive (Multi)")
{
    ...
    GIVEN {
        PLAYER(species) { Ability(ability); Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { ; }
    } SCENE {
...
DOUBLE_BATTLE_TEST("White Herb is correctly displayed (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_BLK_APRICORN, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(playerRight, MOVE_SUPERPOWER, target: opponentRight); }
