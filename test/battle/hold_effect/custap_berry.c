...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(1); MaxHP(160); HP(40); Items(ITEM_TM_ICE_BEAM, ITEM_CUSTAP_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_BELLSPROUT) { Speed(1); MaxHP(160); HP(80); Ability(ABILITY_GLUTTONY); Items(ITEM_TM_ICE_BEAM, ITEM_CUSTAP_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(2); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_REGIROCK) { HP(1); Items(ITEM_TM_ICE_BEAM, ITEM_CUSTAP_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { SWITCH(opponent, 1); }
