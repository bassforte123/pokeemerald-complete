...
SINGLE_BATTLE_TEST("Room Serive decreases the holder's seep by one stage")
{
    GIVEN {
        ...
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_ROOM_SERVICE); }
...
SINGLE_BATTLE_TEST("Room Serive decreases the holder's seep by one stage (Multi)")
{
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_PECHA_BERRY, ITEM_ROOM_SERVICE); }
        OPPONENT(SPECIES_WYNAUT) { HP(1); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK_ROOM); }
...
SINGLE_BATTLE_TEST("Room Serive decreases the holder's seep by one stage (Multi)")
{
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_UNNERVE); Innates(ABILITY_INTIMIDATE); Item(ITEM_ROOM_SERVICE); }
        OPPONENT(SPECIES_WYNAUT) { HP(1); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK_ROOM); }
