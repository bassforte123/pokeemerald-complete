...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Lansat Berry raises the holder's critical-hit-ratio by two stages when HP drops to 1/4 or below (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Lansat Berry raises the holder's critical-hit-ratio by two stages when HP drops to 1/2 or below (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Lansat Berry raises the holder's critical-hit-ratio by 2 stages (Items)")
{
    u32 genConfig = 0, chance;
    for (u32 j = GEN_1; j <= GEN_5; j++)
        PARAMETRIZE { genConfig = j; chance = 4; } // 25%
