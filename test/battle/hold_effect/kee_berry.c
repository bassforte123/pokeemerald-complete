...
SINGLE_BATTLE_TEST("Kee Berry raises the holder's Defense by two stages with Ripen when hit by a physical move (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Kee Berry doesn't trigger if the item hold user used a physical move (Multi)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Kee Berry doesn't trigger if partner was hit (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Kee Berry doesn't trigger if the move was boosted by Sheer Force (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_KEE_BERRY); }
        OPPONENT(SPECIES_NIDOKING) { Ability(ABILITY_SHEER_FORCE); }
