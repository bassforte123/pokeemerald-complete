...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Binding Band increases the damage taken from binding effects (Items)", s16 damage)
{
    enum Item item;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_GREAT_BALL, item); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_WRAP); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_WRAP, opponent);
...
SINGLE_BATTLE_TEST("Binding Band increases the damage taken from binding effects even if holder's item gets nullified after (Items)", s16 damage)
{
    enum Item item;
