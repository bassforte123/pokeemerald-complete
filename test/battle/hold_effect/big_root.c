...
SINGLE_BATTLE_TEST("Big Root increases healing from absorbing moves", s16 damage, s16 heal)
{
    ...
    GIVEN {
        PLAYER(SPECIES_XURKITREE) { HP(200); Items(item); }
...
SINGLE_BATTLE_TEST("Big Root increases healing from absorbing moves (Multi)", s16 damage, s16 heal)
{
    ...
    GIVEN {
        PLAYER(SPECIES_XURKITREE) { HP(200); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_ABSORB); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Big Root increases the damage restored from Leech Seed, Ingrain and Aqua Ring (Multi)", s16 heal, s16 damage)
{
    u32 item;
    u32 move;
