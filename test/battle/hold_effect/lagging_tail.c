...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(100); Items(ITEM_PECHA_BERRY, ITEM_LAGGING_TAIL); }
        PLAYER(SPECIES_WYNAUT) { Speed(10); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(30); Items(ITEM_PECHA_BERRY, ITEM_BERRY_JUICE); }
        OPPONENT(SPECIES_WYNAUT) { Speed(20); }
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_TRICK, target: playerLeft); }
        TURN {}
