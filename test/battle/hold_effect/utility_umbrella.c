...
<<<<<<< HEAD
        OPPONENT(SPECIES_WOBBUFFET) { Items(heldItem); };
=======
...
SINGLE_BATTLE_TEST("Utility Umbrella blocks Sun damage modifiers (Multi)", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, heldItem); };
    } WHEN {
        TURN { MOVE(opponent, setupMove); }
        TURN { MOVE(player, attackingMove); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Utility Umbrella blocks Rain damage modifiers (Multi)", s16 damage)
{
    u16 setupMove, attackingMove, heldItem;
    PARAMETRIZE { setupMove = MOVE_RAIN_DANCE; attackingMove = MOVE_EMBER;     heldItem = ITEM_UTILITY_UMBRELLA; }
    PARAMETRIZE { setupMove = MOVE_RAIN_DANCE; attackingMove = MOVE_EMBER;     heldItem = ITEM_NONE; }
