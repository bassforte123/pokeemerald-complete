...
SINGLE_BATTLE_TEST("Ability Shield prevents Neutralizing Gas (Multi)")
{
    u32 item;
    ...
    GIVEN {
        PLAYER(SPECIES_TORKOAL) { Ability(ABILITY_DROUGHT); Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); }
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mold Breaker (Multi)")
{
    u32 item;
    ...
    GIVEN {
        PLAYER(SPECIES_SHEDINJA) { Ability(ABILITY_WONDER_GUARD); Items(ITEM_NUGGET, item); }
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mycelium Might (Multi)")
{
    u32 item;
    ...
    GIVEN {
        PLAYER(SPECIES_VIGOROTH) { Ability(ABILITY_VITAL_SPIRIT); Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_TOEDSCOOL) { Ability(ABILITY_MYCELIUM_MIGHT); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Sunsteel Strike (Multi)")
{
    u32 item;
...
SINGLE_BATTLE_TEST("Ability Shield protects the user's ability from being suppressed by Gastro Acid (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_GASTRO_ACID) == EFFECT_GASTRO_ACID);
        PLAYER(SPECIES_BLAZIKEN) { Ability(ABILITY_SPEED_BOOST); Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_GASTRO_ACID); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_GYARADOS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap even if user has Klutz (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_LOPUNNY) { Ability(ABILITY_KLUTZ); Items(ITEM_NUGGET, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
    } SCENE {
