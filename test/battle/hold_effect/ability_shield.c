...
SINGLE_BATTLE_TEST("Ability Shield protects against Neutralizing Gas (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_TORKOAL) { Ability(ABILITY_DROUGHT); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); }
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mold Breaker (no message) (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_EARTHQUAKE) == TYPE_GROUND);
        PLAYER(SPECIES_FLYGON) { Ability(ABILITY_LEVITATE); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_EXCADRILL) { Ability(ABILITY_MOLD_BREAKER); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_EARTHQUAKE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mycelium Might (no message) (Multi)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SPORE) == EFFECT_NON_VOLATILE_STATUS);
        ASSUME(GetMoveNonVolatileStatus(MOVE_SPORE) == MOVE_EFFECT_SLEEP);
        PLAYER(SPECIES_VIGOROTH) { Ability(ABILITY_VITAL_SPIRIT); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_TOEDSCOOL) { Ability(ABILITY_MYCELIUM_MIGHT); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SPORE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Sunsteel Strike (no message) (Multi)")
{
    u32 item;
    ...
    GIVEN {
        ASSUME(MoveIgnoresTargetAbility(MOVE_SUNSTEEL_STRIKE));
        PLAYER(SPECIES_SHEDINJA) { Ability(ABILITY_WONDER_GUARD); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SUNSTEEL_STRIKE); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects the user's ability from being suppressed by Gastro Acid (Multi)")
{
    u32 item;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_GASTRO_ACID) == EFFECT_GASTRO_ACID);
        PLAYER(SPECIES_BLAZIKEN) { Ability(ABILITY_SPEED_BOOST); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_GASTRO_ACID); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap (Multi)")
{
    u32 item;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_GYARADOS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap even if user has Klutz (Multi)")
{
    u32 item;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SKILL_SWAP) == EFFECT_SKILL_SWAP);
        PLAYER(SPECIES_LOPUNNY) { Ability(ABILITY_KLUTZ); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SKILL_SWAP); }
    } SCENE {
...
// These currently do not activate, but probably should do held item animation + message
TO_DO_BATTLE_TEST("Ability Shield prevents the user's Trace from changing its ability (Multi)");
