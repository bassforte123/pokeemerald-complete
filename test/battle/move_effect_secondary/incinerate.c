...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Incinerate destroys Berries and Gems even if the holder has Klutz (Traits)")
{
    enum Item item;
    ...
    GIVEN {
        ...
        ASSUME(GetItemHoldEffect(ITEM_GHOST_GEM) == HOLD_EFFECT_GEMS);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CAPSAKID) { Ability(ABILITY_INSOMNIA); Innates(ABILITY_KLUTZ); Item(item); }
    } WHEN {
        TURN { MOVE(player, MOVE_INCINERATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_INCINERATE, player);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Incinerate destroys Berries and Gems even if the holder has Klutz (Items)")
{
    enum Item item;
    ...
    GIVEN {
        ...
        ASSUME(GetItemHoldEffect(ITEM_GHOST_GEM) == HOLD_EFFECT_GEMS);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CAPSAKID) { Ability(ABILITY_KLUTZ); Items(ITEM_GREAT_BALL, item); }
    } WHEN {
        TURN { MOVE(player, MOVE_INCINERATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_INCINERATE, player);
...
SINGLE_BATTLE_TEST("Incinerate activates Occa Berry before destroying it (Items)")
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_INCINERATE) == TYPE_FIRE);
        ASSUME(IsSpeciesOfType(SPECIES_CAPSAKID, TYPE_GRASS));
        ...
        ASSUME(GetItemHoldEffectParam(ITEM_OCCA_BERRY) == TYPE_FIRE);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CAPSAKID) { Items(ITEM_GREAT_BALL, ITEM_OCCA_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_INCINERATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_BERRY, opponent);
...
SINGLE_BATTLE_TEST("Incinerate destroys Oran Berry before it can restore HP (Items)")
{
    s16 damage;
    ...
    GIVEN {
        ...
        ASSUME(GetItemHoldEffectParam(ITEM_ORAN_BERRY) == 10);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_ORAN_BERRY); HP(201); MaxHP(400); }
    } WHEN {
        TURN { MOVE(player, MOVE_INCINERATE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_INCINERATE, player);
...
SINGLE_BATTLE_TEST("Incinerate destroys Rowap Berry before it can damage the attacker (Items)")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_INCINERATE) == DAMAGE_CATEGORY_SPECIAL);
        ASSUME(GetItemHoldEffect(ITEM_ROWAP_BERRY) == HOLD_EFFECT_ROWAP_BERRY);
