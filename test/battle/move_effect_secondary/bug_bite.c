...
SINGLE_BATTLE_TEST("Bug Bite eats the target's berry and immediately gains its effect (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(399); MaxHP(400); Status1(status1); Moves(MOVE_SLEEP_TALK, MOVE_BUG_BITE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_POTION, item); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Tanga Berry activates before Bug Bite (Multi)")
{
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_TANGA_BERRY].holdEffectParam == TYPE_BUG);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) {Items(ITEM_POTION, ITEM_TANGA_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_BUG_BITE); }
    } SCENE {
        MESSAGE("Wobbuffet used Bug Bite!");
...
SINGLE_BATTLE_TEST("Bug Bite ignores Unnerve (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_TYRANITAR) { Ability(ABILITY_UNNERVE); Items(ITEM_POTION, ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_BUG_BITE); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_BUG_BITE, player);
