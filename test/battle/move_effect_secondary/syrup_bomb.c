...
SINGLE_BATTLE_TEST("Sticky Syrup is not reflected by Mirror Armor (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Sticky Syrup stat drop is prevented by Flower Veil (Traits)")
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_CHIKORITA, 0) == TYPE_GRASS || GetSpeciesType(SPECIES_CHIKORITA, 1) == TYPE_GRASS);
        PLAYER(SPECIES_WOBBUFFET);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Sticky Syrup speed reduction is prevented by Clear Amulet (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
