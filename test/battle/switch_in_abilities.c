...
MULTI_BATTLE_TEST("Switch-in abilities trigger in Speed Order after post-KO switch - multibattle (Traits)")
{
    ...
    GIVEN {
        ...
        PARTNER(SPECIES_GYARADOS) { Speed(spdPlayer2); Ability(ABILITY_INTIMIDATE); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(1); Speed(1); }
        OPPONENT_A(SPECIES_WEEZING_GALAR) { Speed(spdOpponent1); Ability(ABILITY_LEVITATE); Innates(ABILITY_MISTY_SURGE); }
        OPPONENT_B(SPECIES_WOBBUFFET) { HP(1); Speed(1); }
        OPPONENT_B(SPECIES_VULPIX_ALOLA) { Speed(spdOpponent2); Ability(ABILITY_SNOW_WARNING); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_EXPLOSION); SEND_OUT(playerLeft, 1); SEND_OUT(opponentLeft, 1); SEND_OUT(playerRight, 1); SEND_OUT(opponentRight, 1); }
...
TWO_VS_ONE_BATTLE_TEST("Switch-in abilities trigger in Speed Order after post-KO switch - 2v1 (Traits)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_TYRANITAR) { Speed(spdPlayer1); Ability(ABILITY_UNNERVE); Innates(ABILITY_SAND_STREAM); }
        PARTNER(SPECIES_WYNAUT) { HP(1); Speed(1); }
        PARTNER(SPECIES_GYARADOS) { Speed(spdPlayer2); Ability(ABILITY_STENCH); Innates(ABILITY_INTIMIDATE); }
        OPPONENT_A(SPECIES_WOBBUFFET) { HP(1); Speed(1); }
        OPPONENT_A(SPECIES_WYNAUT) { HP(1); Speed(1); }
        OPPONENT_A(SPECIES_WEEZING_GALAR) { Speed(spdOpponent1); Ability(ABILITY_LEVITATE); Innates(ABILITY_MISTY_SURGE); }
        OPPONENT_A(SPECIES_VULPIX_ALOLA) { Speed(spdOpponent2); Ability(ABILITY_SNOW_CLOAK); Innates(ABILITY_SNOW_WARNING); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_EXPLOSION); SEND_OUT(playerLeft, 1); SEND_OUT(opponentLeft, 2); SEND_OUT(playerRight, 1); SEND_OUT(opponentRight, 3); }
    } SCENE {
        MESSAGE("Wobbuffet used Explosion!");
