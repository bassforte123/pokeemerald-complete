...
SINGLE_BATTLE_TEST("Ivy Cudgel changes the move type depending on the form of Ogerpon (Multi)")
{
    ...
    GIVEN {
        PLAYER(ogerpon) { Items( ITEM_GREEN_APRICORN, item); }
        OPPONENT(species);
    } WHEN {
        TURN { MOVE(player, MOVE_IVY_CUDGEL); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ivy Cudgel does not change the move type if used by Pokémon other than Ogerpon (Multi)")
{
    u16 item;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items( ITEM_GREEN_APRICORN, item); }
        OPPONENT(SPECIES_BLASTOISE);
    } WHEN {
        TURN { MOVE(player, MOVE_IVY_CUDGEL); }
    } SCENE {
