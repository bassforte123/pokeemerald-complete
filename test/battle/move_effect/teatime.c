...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_NONE); }
    } WHEN {
        TURN { MOVE(player, MOVE_TEATIME); }
    } SCENE {
        MESSAGE("Wobbuffet used Teatime!");
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_TEATIME); }
    } SCENE {
        MESSAGE("The opposing Wobbuffet used Teatime!");
...
SINGLE_BATTLE_TEST("Teatime triggers Volt Absorb if it has been affected by Electrify or Plasma Fists, even when not holding a Berry (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_JOLTEON) { Ability(ABILITY_VOLT_ABSORB); Items( ITEM_LUXURY_BALL, item); HP(55); MaxHP(100); }
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
...
SINGLE_BATTLE_TEST("Teatime triggers Lightning Rod if it has been affected by Electrify or Plasma Fists, even when not holding a Berry (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_PIKACHU) { Ability(ABILITY_LIGHTNING_ROD); Items( ITEM_LUXURY_BALL, item); }
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
...
SINGLE_BATTLE_TEST("Teatime triggers Motor Drive if it has been affected by Electrify or Plasma Fists, even when not holding a Berry (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_ELECTIVIRE) { Ability(ABILITY_MOTOR_DRIVE); Items( ITEM_LUXURY_BALL, item); }
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_LUXURY_BALL, ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
