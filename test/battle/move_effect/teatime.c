...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_LIECHI_BERRY); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Teatime triggers Volt Absorb if it has been affected by Electrify or Plasma Fists, even when not holding a Berry")
{
    ...
    GIVEN {
        PLAYER(SPECIES_JOLTEON) { Ability(ABILITY_VOLT_ABSORB); Items(item); HP(55); MaxHP(100); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
...
SINGLE_BATTLE_TEST("Teatime triggers Lightning Rod if it has been affected by Electrify or Plasma Fists, even when not holding a Berry")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_PIKACHU) { Ability(ABILITY_LIGHTNING_ROD); Items(item); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
...
SINGLE_BATTLE_TEST("Teatime triggers Motor Drive if it has been affected by Electrify or Plasma Fists, even when not holding a Berry")
{
    ...
    GIVEN {
        PLAYER(SPECIES_ELECTIVIRE) { Ability(ABILITY_MOTOR_DRIVE); Items(item); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN {
            MOVE(player, move);
...
SINGLE_BATTLE_TEST("Teatime causes the user to consume its Berry, even under the effects of Magic Room (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Teatime causes the user to consume its Berry, ignoring HP requirements, when not used by the Player (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Teatime causes other Pokemon to consume their Berry even if the user doesn't have a Berry as its held item (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LIECHI_BERRY); }
...
SINGLE_BATTLE_TEST("Teatime causes other Pokemon to consume their Berry even if the user doesn't have a Berry as its held item, when not used by the Player (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_NONE); }
...
DOUBLE_BATTLE_TEST("Teatime causes all Pokémon to consume their berry (Multi)")
{
    struct BattlePokemon *user = NULL;
    PARAMETRIZE { user = playerLeft; }
    PARAMETRIZE { user = playerRight; }
...
SINGLE_BATTLE_TEST("Teatime fails if no Pokémon is holding a Berry (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_NONE); }
...
SINGLE_BATTLE_TEST("Teatime does not affect Pokémon in the semi-invulnerable turn of a move (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_NONE); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LIECHI_BERRY); }
