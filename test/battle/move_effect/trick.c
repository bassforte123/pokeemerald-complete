...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Trick swaps held items (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Trick succeeds if only the user has an item (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_SITRUS_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Trick succeeds if only the target has an item (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LUM_BERRY); }
...
SINGLE_BATTLE_TEST("Trick fails if either item is Mail (Items)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, atkItem); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, defItem); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
    } SCENE {
        MESSAGE("But it failed!");
...
SINGLE_BATTLE_TEST("Trick fails if either battler holds a Mega Stone (Items)")
{
    ...
    GIVEN {
        ...
        OPPONENT(defSpecies) { Items(ITEM_NONE, defItem); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
    } SCENE {
        MESSAGE("But it failed!");
...
SINGLE_BATTLE_TEST("Trick fails if an item changes the holder's form (Items)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_GIRATINA_ORIGIN) { Items(ITEM_NONE, atkItem); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, defItem); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Trick doesn't fail if the user has Sticky Hold (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Trick fails against Sticky Hold (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_SITRUS_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Ability(ABILITY_STICKY_HOLD); Items(ITEM_NONE, ITEM_LUM_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
    } SCENE {
        MESSAGE("The opposing Wobbuffet's item cannot be removed!");
...
SINGLE_BATTLE_TEST("Trick fails if the target is behind a Substitute (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_SITRUS_BERRY); Speed(50); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_LUM_BERRY); Speed(100); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SUBSTITUTE); MOVE(player, MOVE_TRICK); }
    } SCENE {
        MESSAGE("But it failed!");
...
SINGLE_BATTLE_TEST("Trick can be used against targets with an active form change that doesn't require items (Items)")
{
    GIVEN {
        PLAYER(SPECIES_XERNEAS);
...
SINGLE_BATTLE_TEST("Trick does not remove the user's choice lock if both the target and use are holding choice items before Gen 5 (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MODERN_TRICK_CHOICE_LOCK, GEN_4);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NUGGET, ITEM_CHOICE_SCARF); MovesWithPP({MOVE_TRICK, 1}, {MOVE_CELEBRATE, 10}); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NUGGET, ITEM_CHOICE_SCARF); }
    }
    WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
        TURN { FORCED_MOVE(player); }
...
SINGLE_BATTLE_TEST("Trick removes the user's choice lock if both the target and use are holding choice items from Gen 5 onwards (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MODERN_TRICK_CHOICE_LOCK, GEN_5);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NUGGET, ITEM_CHOICE_SCARF); MovesWithPP({MOVE_TRICK, 1}, {MOVE_CELEBRATE, 10}); }
