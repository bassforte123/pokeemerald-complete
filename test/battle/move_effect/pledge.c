...
DOUBLE_BATTLE_TEST("Sea Of Fire does not damage Fire-types or Magic Guard Pokemon (Traits)")
{
    enum Species species;
    enum Ability ability;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(4); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(species) { Speed(2); Ability(ABILITY_LIGHT_METAL); Innates(ability); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_FIRE_PLEDGE, target: opponentRight);
               MOVE(playerRight, MOVE_GRASS_PLEDGE, target: opponentRight);
