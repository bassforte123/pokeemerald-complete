...
SINGLE_BATTLE_TEST("Sky Drop: If target was locked into a move that would confuse, the target will be freed and confusion occurs immediately (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_TORKOAL) { Ability(ABILITY_DROUGHT); }
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Sky Drop: If target was locked into a move that would confuse, the target will be freed and confusion occurs immediately (attacker faints due to target ability) (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_SHARPEDO) { Ability(ABILITY_ROUGH_SKIN); }
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Sky Drop user and target can't activate Eject items while the move is being used (Items)")
{
    u32 item;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_VULLABY) { Item(item); } // Flying type used here so Sky Drop doesn't damage
        OPPONENT(SPECIES_MACHAMP) { Ability(ABILITY_GUTS); Innates(ABILITY_NO_GUARD); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SKY_DROP, target: opponentLeft);
               MOVE(opponentRight, MOVE_BREAKING_SWIPE);
...
DOUBLE_BATTLE_TEST("Sky Drop user and target can't activate Eject items while the move is being used (Items)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_VULLABY, 0) == TYPE_FLYING || GetSpeciesType(SPECIES_VULLABY, 1) == TYPE_FLYING);
        ASSUME(MoveHasAdditionalEffectWithChance(MOVE_BREAKING_SWIPE, MOVE_EFFECT_STAT_MINUS, 100));
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, item); }
        PLAYER(SPECIES_MACHAMP) { Ability(ABILITY_NO_GUARD); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_VULLABY) { Items(ITEM_GREAT_BALL, item); } // Flying type used here so Sky Drop doesn't damage
        OPPONENT(SPECIES_MACHAMP) { Ability(ABILITY_NO_GUARD); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SKY_DROP, target: opponentLeft);
