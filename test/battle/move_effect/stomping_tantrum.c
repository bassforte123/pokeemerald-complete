...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Stomping Tantrum will deal double damage if user failed to attack due to paralysis (Items)")
{
    s16 damage[3];
    PASSES_RANDOMLY(25, 100, RNG_PARALYSIS);
    GIVEN {
...
SINGLE_BATTLE_TEST("Stomping Tantrum will not deal double if it missed (Items)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Stomping Tantrum will deal double damage if hit into an immunity ability (Items)")
{
    s16 damage[2];
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ELECTRIFY) == EFFECT_ELECTRIFY);
