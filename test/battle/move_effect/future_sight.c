...
SINGLE_BATTLE_TEST("Future Sight uses Sp. Atk stat of the original user without modifiers (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_PIKACHU) { Items( ITEM_IRON_BALL, item); }
        PLAYER(SPECIES_RAICHU) { Items( ITEM_IRON_BALL, item); }
        OPPONENT(SPECIES_REGICE);
    } WHEN {
...
SINGLE_BATTLE_TEST("Future Sight is not boosted by Life Orb is original user if not on the field (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_PIKACHU);
        PLAYER(SPECIES_RAICHU) { Items( ITEM_IRON_BALL, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_REGICE);
    } WHEN {
        TURN { MOVE(player, MOVE_SEED_FLARE, WITH_RNG(RNG_SECONDARY_EFFECT, FALSE)); }
        TURN { MOVE(player, MOVE_FUTURE_SIGHT); }
...
SINGLE_BATTLE_TEST("Future Sight breaks Focus Sash and doesn't make the holder endure another move (Multi)")
{
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_FOCUS_SASH].holdEffect == HOLD_EFFECT_FOCUS_SASH);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_PIDGEY) { Level(1); Items( ITEM_IRON_BALL, ITEM_FOCUS_SASH); }
    } WHEN {
        TURN { MOVE(player, MOVE_FUTURE_SIGHT); }
        TURN { }
        TURN { }
