...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Future Sight doesn't ignore Wonder Guard (Gen 5+) (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Future Sight does not trigger Protean (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_KECLEON) { Ability(ABILITY_PROTEAN); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Future Sight does not trigger Cursed Body (Traits)")
{
    PASSES_RANDOMLY(10, 10, RNG_CURSED_BODY);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_FRILLISH) { Ability(ABILITY_WATER_ABSORB); Innates(ABILITY_CURSED_BODY); }
    } WHEN {
        TURN { MOVE(player, MOVE_FUTURE_SIGHT); }
        TURN {}
        TURN {}
...
SINGLE_BATTLE_TEST("Future Sight is affected by Beads of Ruin on the original slot (Traits)")
{
    s16 damage[2];
...
SINGLE_BATTLE_TEST("Toxic Chain can inflict bad poison from Future Sight if the user is still on the field (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
...
SINGLE_BATTLE_TEST("Toxic Chain does not trigger from Future Sight if the user is not on the field (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_OKIDOGI) { Ability(ABILITY_GUARD_DOG); Innates(ABILITY_TOXIC_CHAIN); }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Future Sight uses Sp. Atk stat of the original user without modifiers (Items)")
{
    enum Item item;
    s16 seedFlareDmg;
    s16 futureSightDmg;
...
SINGLE_BATTLE_TEST("Future Sight is not boosted by Life Orb is original user if not on the field (Items)")
{
    s16 seedFlareDmg;
    s16 futureSightDmg;
...
SINGLE_BATTLE_TEST("Future Sight breaks Focus Sash and doesn't make the holder endure another move (Items)")
{
    GIVEN {
        ASSUME(GetMovePower(MOVE_PSYCHIC) > 0);
        ASSUME(gItemsInfo[ITEM_FOCUS_SASH].holdEffect == HOLD_EFFECT_FOCUS_SASH);
...
SINGLE_BATTLE_TEST("Future Sight does not trigger Red Card (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_RED_CARD].holdEffect == HOLD_EFFECT_RED_CARD);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Future Sight does not trigger Eject Button (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Future Sight does not trigger Rowap Berry (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ROWAP_BERRY].holdEffect == HOLD_EFFECT_ROWAP_BERRY);
        ASSUME(GetMoveCategory(MOVE_FUTURE_SIGHT) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_ROWAP_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_FUTURE_SIGHT); }
        TURN {}
        TURN {}
