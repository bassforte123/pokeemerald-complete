...
SINGLE_BATTLE_TEST("Aura Wheel can be used by Pokémon transformed into Morpeko (Traits)")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_DITTO) { Moves(MOVE_AURA_WHEEL, MOVE_CELEBRATE); Ability(ABILITY_LIMBER); Innates(ABILITY_IMPOSTER); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_AURA_WHEEL); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_AURA_WHEEL, opponent);
...
SINGLE_BATTLE_TEST("Aura Wheel can be turned into a Normal-type move after Morpeko gains Normalize (Traits)")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_ENTRAINMENT) == EFFECT_ENTRAINMENT);
        ASSUME(gSpeciesInfo[SPECIES_DUSKULL].types[0] == TYPE_GHOST || gSpeciesInfo[SPECIES_DUSKULL].types[1] == TYPE_GHOST);
        PLAYER(SPECIES_MORPEKO) { Ability(ABILITY_HUNGER_SWITCH); Innates(ABILITY_NORMALIZE); }
        OPPONENT(SPECIES_DELCATTY) { Ability(ABILITY_CUTE_CHARM); }
        OPPONENT(SPECIES_DUSKULL);
    } WHEN {
        if (hangryMode)
            TURN { }
