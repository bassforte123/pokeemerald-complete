...
SINGLE_BATTLE_TEST("Grudge depletes all PP from a Max Move's base move (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Grudge's effect disappears if the user takes a new turn - Flinching (Multi)")
{
    PASSES_RANDOMLY(10, 100, RNG_HOLD_EFFECT_FLINCH);
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FALSE_SWIPE) == EFFECT_FALSE_SWIPE);
