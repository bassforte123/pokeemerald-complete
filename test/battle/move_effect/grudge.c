...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Grudge does not deplete PP of a Z-Move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_NORMALIUM_Z); Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_POUND, MOVE_SURF); };
    } WHEN {
        TURN {
            MOVE(player, MOVE_GRUDGE);
            MOVE(opponent, MOVE_SCRATCH, gimmick: GIMMICK_Z_MOVE);
...
SINGLE_BATTLE_TEST("Grudge depletes all PP from a Max Move's base move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) {  Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_POUND, MOVE_SURF); Items(ITEM_PECHA_BERRY, ITEM_LAGGING_TAIL); }
    } WHEN {
        TURN { MOVE(player, MOVE_GRUDGE); MOVE(opponent, MOVE_SCRATCH, gimmick: GIMMICK_DYNAMAX); SEND_OUT(player, 1); }
    } SCENE {
        MESSAGE("Wobbuffet used Grudge!");
...
SINGLE_BATTLE_TEST("Grudge's effect disappears if the user takes a new turn - Flinching (Items)")
{
    PASSES_RANDOMLY(10, 100, RNG_HOLD_EFFECT_FLINCH);
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FALSE_SWIPE) == EFFECT_FALSE_SWIPE);
        ...
        PLAYER (SPECIES_WOBBUFFET);
        OPPONENT (SPECIES_WOBBUFFET);
        OPPONENT (SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_FALSE_SWIPE, MOVE_SURF); Items(ITEM_PECHA_BERRY, ITEM_KINGS_ROCK); }
    }
    WHEN {
        TURN { SWITCH(opponent, 1); MOVE(player, MOVE_GRUDGE); }
        TURN { MOVE(opponent, MOVE_FALSE_SWIPE); MOVE(player, MOVE_CELEBRATE); }
