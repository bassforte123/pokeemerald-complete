...
DOUBLE_BATTLE_TEST("Protect is not transferred to a mon that is switched in due to Eject Button")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_EJECT_BUTTON); }
...
SINGLE_BATTLE_TEST("Protect: Protective Pads protects from secondary effects (Multi)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Wide Guard is still activate even if user is switched out due to Eject Button (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
