...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Sticky Web raises Speed by 1 for a Pokemon with Contrary (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - the battler which set up Sticky Web has its Speed lowered instead (Traits)")
{
    u8 playerSetUpper, opponentSetUpper; // 0 left, 1 right
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper switched (Traits)")
{
    u16 speedPlayer, speedOpponent;
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper fainted (Traits)")
{
    bool8 hasReplacement;
...
DOUBLE_BATTLE_TEST("Sticky Web setter has their speed lowered with Mirror Armor even after Ally Switch (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE);
        PLAYER(SPECIES_CHARMANDER);
...
#define BATTLER_OPPONENT (opponentSetUpper == 0 ? opponentLeft : opponentRight)
#define BATTLER_PLAYER (playerSetUpper == 0 ? playerLeft : playerRight)
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - the battler which set up Sticky Web has its Speed lowered instead (Items)")
{
    u8 playerSetUpper, opponentSetUpper; // 0 left, 1 right
    ...
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE);
        PLAYER(SPECIES_CHARMANDER);
        PLAYER(SPECIES_CORVIKNIGHT) { Ability(ABILITY_MIRROR_ARMOR); Items(ITEM_PECHA_BERRY, ITEM_IRON_BALL); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE);
        OPPONENT(SPECIES_WEEDLE);
    } WHEN {
        TURN { MOVE(BATTLER_OPPONENT, MOVE_STICKY_WEB); }
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper switched (Items)")
{
    u16 speedPlayer, speedOpponent;
    ...
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE) { Speed(speedPlayer); }
        PLAYER(SPECIES_CHARMANDER) { Speed(speedPlayer); }
        PLAYER(SPECIES_CORVIKNIGHT) { Ability(ABILITY_MIRROR_ARMOR); Items(ITEM_PECHA_BERRY, ITEM_IRON_BALL); Speed(speedOpponent); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE) { Speed(speedOpponent); }
        OPPONENT(SPECIES_WEEDLE) { Speed(speedOpponent); }
        OPPONENT(SPECIES_PIDGEY) { Speed(speedOpponent); } // Flying type,so not affected by Sticky Web.
    } WHEN {
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper fainted (Items)")
{
    bool8 hasReplacement;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_SQUIRTLE) {Speed(5); }
        PLAYER(SPECIES_CHARMANDER) {Speed(5); }
        PLAYER(SPECIES_CORVIKNIGHT) {Ability(ABILITY_MIRROR_ARMOR); Items(ITEM_PECHA_BERRY, ITEM_IRON_BALL); Speed(5); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE) {Speed(7); }
        OPPONENT(SPECIES_WEEDLE) {Speed(7); }
        if (hasReplacement) {
            OPPONENT(SPECIES_PIDGEY) {Speed(7); }
...
DOUBLE_BATTLE_TEST("Sticky Web setter has their speed lowered with Mirror Armor even after Ally Switch (Items)")
{
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE);
        PLAYER(SPECIES_CHARMANDER);
