...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - the battler which set up Sticky Web has its Speed lowered instead (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE);
        PLAYER(SPECIES_CHARMANDER);
        PLAYER(SPECIES_CORVIKNIGHT) { Ability(ABILITY_MIRROR_ARMOR); Items( ITEM_GREEN_APRICORN, ITEM_IRON_BALL); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE);
        OPPONENT(SPECIES_WEEDLE);
    } WHEN {
        TURN { MOVE(BATTLER_OPPONENT, MOVE_STICKY_WEB); }
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper switched (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_SQUIRTLE) { Speed(speedPlayer); }
        PLAYER(SPECIES_CHARMANDER) { Speed(speedPlayer); }
        PLAYER(SPECIES_CORVIKNIGHT) { Ability(ABILITY_MIRROR_ARMOR); Items( ITEM_GREEN_APRICORN, ITEM_IRON_BALL); Speed(speedOpponent); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE) { Speed(speedOpponent); }
        OPPONENT(SPECIES_WEEDLE) { Speed(speedOpponent); }
        OPPONENT(SPECIES_PIDGEY) { Speed(speedOpponent); } // Flying type,so not affected by Sticky Web.
    } WHEN {
...
DOUBLE_BATTLE_TEST("Sticky Web has correct interactions with Mirror Armor - no one has their Speed lowered if the set upper fainted (Multi)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_SQUIRTLE) {Speed(5); }
        PLAYER(SPECIES_CHARMANDER) {Speed(5); }
        PLAYER(SPECIES_CORVIKNIGHT) {Ability(ABILITY_MIRROR_ARMOR); Items( ITEM_GREEN_APRICORN, ITEM_IRON_BALL); Speed(5); } // Iron Ball, so that flying type Corviknight is affected by Sticky Web.
        OPPONENT(SPECIES_CATERPIE) {Speed(7); }
        OPPONENT(SPECIES_WEEDLE) {Speed(7); }
        if (hasReplacement) {
            OPPONENT(SPECIES_PIDGEY) {Speed(7); }
