...
SINGLE_BATTLE_TEST("Fling fails if pokemon holds no item (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Fling fails if pokemon is under the effects of Embargo or Magic Room (Multi)")
{
    u16 move;
...
SINGLE_BATTLE_TEST("Fling fails for pokemon with Klutz ability (Multi)")
{
    u16 ability;
...
SINGLE_BATTLE_TEST("Fling doesn't consume the item if pokemon is asleep/frozen/paralyzed (Multi)")
{
    u32 status;
    u16 item;
...
SINGLE_BATTLE_TEST("Fling throws one item then the other (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items( ITEM_RAZOR_CLAW, ITEM_POTION); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING);}
        TURN { MOVE(player, MOVE_FLING);}
