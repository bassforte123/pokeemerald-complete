...
SINGLE_BATTLE_TEST("Fling - Item does not get blocked by Unnerve if it isn't a berry (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
        PLAYER(SPECIES_CALYREX) { Item(ITEM_MENTAL_HERB); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_UNNERVE); }
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_TAUNT); MOVE(opponent, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_FLING); MOVE(opponent, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Fling's secondary effects are blocked by Shield Dust (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Item(item); }
        OPPONENT(SPECIES_WOBBUFFET) { Ability(ABILITY_TELEPATHY); Innates(ABILITY_SHIELD_DUST); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
        MESSAGE("Wobbuffet used Fling!");
...
SINGLE_BATTLE_TEST("Fling doesn't fail when holding a Booster Energy and the target is a Paradox mon (Traits)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_BOOSTER_ENERGY) == HOLD_EFFECT_BOOSTER_ENERGY);
        ASSUME(gSpeciesInfo[SPECIES_RAGING_BOLT].isParadox == TRUE);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_RAGING_BOLT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_PROTOSYNTHESIS); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_FLING, player);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Fling fails if Pokémon holds no item (Items)")
{
    enum Item item;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING);}
    } SCENE {
...
SINGLE_BATTLE_TEST("Fling fails if Pokémon is under the effects of Embargo or Magic Room (Items)")
{
    enum Move move;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EMBARGO) == EFFECT_EMBARGO);
        ASSUME(GetMoveEffect(MOVE_MAGIC_ROOM) == EFFECT_MAGIC_ROOM);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); }
        TURN { MOVE(player, MOVE_FLING); }
...
SINGLE_BATTLE_TEST("Fling fails for Pokémon with Klutz ability (Gen5+) (Items)")
{
    enum Ability ability;
    u32 config;
    ...
    GIVEN {
        WITH_CONFIG(B_KLUTZ_FLING_INTERACTION, config);
        PLAYER(SPECIES_BUNEARY) { Items(ITEM_NONE, ITEM_RAZOR_CLAW); Ability(ability); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Fling fails if the item changes the Pokémon's form (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Fling's thrown item can be regained with Recycle (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
...
SINGLE_BATTLE_TEST("Fling - Item is lost even when there is no target (Items)")
{
    GIVEN {
        ASSUME(IsExplosionMove(MOVE_SELF_DESTRUCT));
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, ITEM_RAZOR_CLAW); Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) {Speed(5); }
        OPPONENT(SPECIES_WOBBUFFET) {Speed(5); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SELF_DESTRUCT); MOVE(player, MOVE_FLING); SEND_OUT(opponent, 1); }
...
SINGLE_BATTLE_TEST("Fling - Item is lost when target protects itself (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_PROTECT) == EFFECT_PROTECT);
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
...
SINGLE_BATTLE_TEST("Fling - Item does not get blocked by Unnerve if it isn't a berry (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
        PLAYER(SPECIES_CALYREX) { Items(ITEM_NONE, ITEM_MENTAL_HERB); Ability(ABILITY_UNNERVE); }
...
SINGLE_BATTLE_TEST("Fling doesn't consume the item if the user is asleep/frozen/paralyzed (Items)")
{
    u32 status;
    enum Item item;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, item); Status1(status); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        if (status == STATUS1_FREEZE) {
            TURN { MOVE(player, MOVE_FLING, WITH_RNG(RNG_FROZEN, FALSE)); }
...
SINGLE_BATTLE_TEST("Fling applies special effects when throwing specific Items (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Fling's secondary effects are blocked by Shield Dust (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Fling's berry effects are blocked by Shield Dust (Items)")
{
    enum Item item;
    u32 status1 = STATUS1_NONE;
...
SINGLE_BATTLE_TEST("Fling's berry effects are blocked by Covert Cloak (Items)")
{
    enum Item item;
    u32 status1 = STATUS1_NONE;
    ...
    GIVEN {
        ...
        ASSUME(GetItemHoldEffect(ITEM_LIECHI_BERRY) == HOLD_EFFECT_ATTACK_UP);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, item); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_COVERT_CLOAK); Status1(status1); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_FLING, player);
...
SINGLE_BATTLE_TEST("Fling - thrown berry's effect activates for the target even if the trigger conditions are not met (Items)")
{
    enum Item item;
    enum HoldEffect effect;
    enum Stat statId = STAT_HP;
    ...
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_FLING) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, item); Attack(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Status1(status1); HP(399); MaxHP(400); MovesWithPP({MOVE_CELEBRATE, 35}); }
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Fling deals damage based on items fling power (Items)")
{
    s16 damage[2];
...
SINGLE_BATTLE_TEST("Fling deals damage based on a TM's move power (Items)")
{
    s16 damage[2];
    ...
    GIVEN {
        ASSUME(GetMovePower(MOVE_EARTHQUAKE) == GetMovePower(MOVE_EGG_BOMB));
        ASSUME(!IsSpeciesOfType(SPECIES_WOBBUFFET, TYPE_DARK));
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_TM_EARTHQUAKE); }
        OPPONENT(SPECIES_HIPPOWDON);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
        TURN { MOVE(player, MOVE_EGG_BOMB); }
...
SINGLE_BATTLE_TEST("Fling doesn't fail when holding a Booster Energy and the target is a Paradox mon (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_BOOSTER_ENERGY) == HOLD_EFFECT_BOOSTER_ENERGY);
        ASSUME(gSpeciesInfo[SPECIES_RAGING_BOLT].isParadox == TRUE);
...
SINGLE_BATTLE_TEST("Fling reveals the user's item before dealing damage (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffectSelf(MOVE_FLING, MOVE_EFFECT_ITEM_MESSAGE));
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Fling doesn't reveal the user's item if it failed to use the move (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffectSelf(MOVE_FLING, MOVE_EFFECT_ITEM_MESSAGE));
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_POTION); Status1(STATUS1_SLEEP); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_FLING); }
    } SCENE {
        NONE_OF {
...
SINGLE_BATTLE_TEST("Fling doesn't reveal the user's item if it missed (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffectSelf(MOVE_FLING, MOVE_EFFECT_ITEM_MESSAGE));
        ASSUME(GetItemHoldEffect(ITEM_BRIGHT_POWDER) == HOLD_EFFECT_EVASION_UP);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_BRIGHT_POWDER); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_POTION); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_FLING, hit: FALSE); }
    } SCENE {
        NONE_OF {
            MESSAGE("The opposing Wobbuffet flung its Potion! (Items)");
            ANIMATION(ANIM_TYPE_MOVE, MOVE_FLING, opponent);
            HP_BAR(player);
        };
    }
...
SINGLE_BATTLE_TEST("Fling - Mental Herb effect should not remove the target's held item (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
        ASSUME(GetItemHoldEffect(ITEM_MENTAL_HERB) == HOLD_EFFECT_MENTAL_HERB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_MENTAL_HERB); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
    } WHEN {
        TURN { MOVE(player, MOVE_TAUNT); MOVE(opponent, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_FLING); MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Fling - White Herb effect should not remove the target's held item (Items)")
{
    GIVEN {
        ASSUME_STAT_CHANGE(MOVE_GROWL, attack: -1);
        ASSUME(GetItemHoldEffect(ITEM_WHITE_HERB) == HOLD_EFFECT_WHITE_HERB);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_WHITE_HERB); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
    } WHEN {
        TURN { MOVE(player, MOVE_GROWL); }
        TURN { MOVE(player, MOVE_FLING); }
