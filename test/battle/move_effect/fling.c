...
SINGLE_BATTLE_TEST("Fling fails for Pokémon with Klutz ability (Gen5+)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_BUNEARY) { Items(ITEM_RAZOR_CLAW); Ability(ability); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_GIRATINA_ORIGIN) { Items(ITEM_GRISEOUS_CORE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_VENUSAUR) { Items(ITEM_BLASTOISINITE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_FLING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Fling - Item does not get blocked by Unnerve if it isn't a berry")
{
    GIVEN {
        ...
        PLAYER(SPECIES_CALYREX) { Items(ITEM_MENTAL_HERB); Ability(ABILITY_UNNERVE); }
...
SINGLE_BATTLE_TEST("Fling - thrown berry's effect activates for the target even if the trigger conditions are not met")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(item); Attack(1); }
...
SINGLE_BATTLE_TEST("Fling deals damage based on items fling power")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_VENUSAURITE); }
...
SINGLE_BATTLE_TEST("Fling deals damage based on a TM's move power")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_TM_EARTHQUAKE); }
...
SINGLE_BATTLE_TEST("Fling fails if Pokémon is under the effects of Embargo or Magic Room (Multi)")
{
    u16 move;
...
SINGLE_BATTLE_TEST("Fling fails for Pokémon with Klutz ability (Gen5+) (Multi)")
{
    enum Ability ability;
    u32 config;
...
SINGLE_BATTLE_TEST("Fling fails if the item changes the Pokémon's form (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Fling works if the item changes a Pokémon's form but not the one holding it (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Fling's thrown item can be regained with Recycle (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
...
SINGLE_BATTLE_TEST("Fling - Item is lost even when there is no target (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SELF_DESTRUCT) == EFFECT_EXPLOSION);
...
SINGLE_BATTLE_TEST("Fling - Item is lost when target protects itself (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_PROTECT) == EFFECT_PROTECT);
        PLAYER(SPECIES_WOBBUFFET) {Items(ITEM_NONE, ITEM_RAZOR_CLAW); }
...
SINGLE_BATTLE_TEST("Fling doesn't consume the item if Pokémon is asleep/frozen/paralyzed (Multi)")
{
    u32 status;
    u16 item;
...
SINGLE_BATTLE_TEST("Fling applies special effects when throwing specific Items (Multi)")
{
    u16 item;
...
SINGLE_BATTLE_TEST("Fling - thrown berry's effect activates for the target even if the trigger conditions are not met (Multi)")
{
    u16 item, effect;
    u8 statId = 0;
    u32 status1 = STATUS1_NONE;
...
SINGLE_BATTLE_TEST("Fling deals damage based on items fling power (Multi)")
{
    s16 damage[2];
...
SINGLE_BATTLE_TEST("Fling deals damage based on a TM's move power (Multi)")
{
    s16 damage[2];
