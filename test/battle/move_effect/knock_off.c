...
SINGLE_BATTLE_TEST("Knock Off doesn't knock off items from Pokemon behind substitutes (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Knock Off knocks one item then the next (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items( ITEM_POTION, ITEM_SUPER_POTION);}
    } WHEN {
        TURN { MOVE(player, MOVE_KNOCK_OFF); }
        TURN { MOVE(player, MOVE_KNOCK_OFF); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_KNOCK_OFF, player);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_ITEM_KNOCKOFF);
        MESSAGE("Wobbuffet knocked off the opposing Wobbuffet's Super Potion!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_KNOCK_OFF, player);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_ITEM_KNOCKOFF);
        MESSAGE("Wobbuffet knocked off the opposing Wobbuffet's Potion!");
    } THEN {
        EXPECT(opponent->items[0] == ITEM_NONE);
        EXPECT(opponent->items[1] == ITEM_NONE);
    }
