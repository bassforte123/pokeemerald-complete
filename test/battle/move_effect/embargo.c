...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect experience gain", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(20); Items(item); }
...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect effort values")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_POWER_WEIGHT); }
...
SINGLE_BATTLE_TEST("Embargo negates a held item's Speed reduction")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_MACHO_BRACE); Speed(20); }
...
//         PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_SOOTHE_BELL); };
...
SINGLE_BATTLE_TEST("Embargo doesn't block a held item's form-changing effect, but it does block its other effects", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARCEUS_GRASS) { Items(heldItem); };
...
SINGLE_BATTLE_TEST("Embargo makes Fling and Natural Gift fail")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(heldItem); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_LIGHT_BALL); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_EMBARGO); }
...
SINGLE_BATTLE_TEST("Baton Pass passes Embargo's effect")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_LIGHT_BALL); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); }
...
SINGLE_BATTLE_TEST("Embargo doesn't block the effects of berries obtained through Bug Bite or Pluck")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_ORAN_BERRY); };
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); }
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Plate items on the move Judgment", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_ARCEUS) { Items(heldItem); };
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Drive items on the move Techno Blast", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_GENESECT) { Items(heldItem); };
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Memory items on the move Multi Attack", s16 damage)
{
    ...
    GIVEN {
        PLAYER(SPECIES_SILVALLY) { Items(heldItem); };
...
SINGLE_BATTLE_TEST("Embargo can be reflected by Magic Coat")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_LIGHT_BALL); };
    } WHEN {
...
SINGLE_BATTLE_TEST("Embargo doesn't prevent Mega Evolution")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_CHARIZARD) { Items(ITEM_CHARIZARDITE_Y); };
    } WHEN {
        TURN { MOVE(player, MOVE_EMBARGO); }
...
SINGLE_BATTLE_TEST("Embargo blocks an affected Pokémon's trainer from using items (Multi)")
{
    // As of writing, the battle tests system doesn't perform all the operations involved
    // in the action of an NPC using an item in battle.
    KNOWN_FAILING;
...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect experience gain (Multi)", s32 exp)
{
    u32 item;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Level(20); Items(ITEM_PECHA_BERRY, item); }
        OPPONENT(SPECIES_CATERPIE) { Level(10); HP(1); }
        ASSUME(gItemsInfo[ITEM_LUCKY_EGG].holdEffect == HOLD_EFFECT_LUCKY_EGG);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); MOVE(player, MOVE_SCRATCH); }
...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect effort values (Multi)")
{
    u32 finalHPEVAmount;

    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_POWER_WEIGHT); }
        OPPONENT(SPECIES_CATERPIE) { HP(1); }
        ASSUME(gItemsInfo[ITEM_POWER_WEIGHT].holdEffect == HOLD_EFFECT_POWER_ITEM);
        ASSUME(gItemsInfo[ITEM_POWER_WEIGHT].holdEffectParam != 0);
        ASSUME(gItemsInfo[ITEM_POWER_WEIGHT].secondaryId == STAT_HP);
...
SINGLE_BATTLE_TEST("Embargo negates a held item's Speed reduction (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(19); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_MACHO_BRACE); Speed(20); }
        ASSUME(gItemsInfo[ITEM_MACHO_BRACE].holdEffect == HOLD_EFFECT_MACHO_BRACE);
    } WHEN {
        TURN { MOVE(player, MOVE_EMBARGO); }
        TURN { MOVE(player, MOVE_SCRATCH); MOVE(opponent, MOVE_SCRATCH); }
...
// Friendship increase is 1, the held item effect actually does not affect the Friendship gained.
//
// WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect friendship (Multi)")
// {
//     u32 initialFriendship;
//     u32 finalFriendship;
...
//     GIVEN {
//         ASSUME(gItemsInfo[ITEM_X_ACCURACY].battleUsage == EFFECT_ITEM_INCREASE_STAT);
//         PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_SOOTHE_BELL); };
//         OPPONENT(SPECIES_WOBBUFFET);
//     } WHEN {
//         TURN { USE_ITEM(player, ITEM_X_ACCURACY); }
//         TURN { MOVE(player, MOVE_SING); }
...
SINGLE_BATTLE_TEST("Embargo doesn't block a held item's form-changing effect, but it does block its other effects (Multi)", s16 damage)
{
    u32 heldItem;
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARCEUS_GRASS) { Items(ITEM_PECHA_BERRY, heldItem); };
        ASSUME(gItemsInfo[ITEM_MEADOW_PLATE].holdEffect == HOLD_EFFECT_PLATE);
        ASSUME(gItemsInfo[ITEM_MEADOW_PLATE].holdEffectParam == 20);
        ASSUME(gItemsInfo[ITEM_MEADOW_PLATE].secondaryId == TYPE_GRASS);
    } WHEN {
...
SINGLE_BATTLE_TEST("Embargo makes Fling and Natural Gift fail (Multi)")
{
    u32 heldItem, moveId;
    ...
    PARAMETRIZE { heldItem = ITEM_CHERI_BERRY; moveId = MOVE_NATURAL_GIFT; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, heldItem); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); }
        TURN { MOVE(player, moveId); }
...
SINGLE_BATTLE_TEST("Embargo doesn't stop an item flung at an affected target from activating (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Baton Pass passes Embargo's effect (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_PECHA_BERRY, ITEM_LIGHT_BALL); };
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); }
        TURN { MOVE(player, MOVE_BATON_PASS); SEND_OUT(player, 1); }
...
SINGLE_BATTLE_TEST("Embargo doesn't block the effects of berries obtained through Bug Bite or Pluck (Multi)")
{
    u32 hp = 10;

    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); };
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ORAN_BERRY); };
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); }
        TURN { MOVE(player, MOVE_PLUCK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Plate items on the move Judgment (Multi)", s16 damage)
{
    u32 heldItem;
    ...
    PARAMETRIZE { heldItem = ITEM_PIXIE_PLATE; }
    GIVEN {
        PLAYER(SPECIES_ARCEUS) { Items(ITEM_PECHA_BERRY, heldItem); };
        OPPONENT(SPECIES_DRAGONITE);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); MOVE(player, MOVE_JUDGMENT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Drive items on the move Techno Blast (Multi)", s16 damage)
{
    u32 heldItem;
    ...
    PARAMETRIZE { heldItem = ITEM_SHOCK_DRIVE; }
    GIVEN {
        PLAYER(SPECIES_GENESECT) { Items(ITEM_PECHA_BERRY, heldItem); };
        OPPONENT(SPECIES_GYARADOS);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); MOVE(player, MOVE_TECHNO_BLAST); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Memory items on the move Multi Attack (Multi)", s16 damage)
{
    u32 heldItem;
    ...
    PARAMETRIZE { heldItem = ITEM_FIRE_MEMORY; }
    GIVEN {
        PLAYER(SPECIES_SILVALLY) { Items(ITEM_PECHA_BERRY, heldItem); };
        OPPONENT(SPECIES_VENUSAUR);
    } WHEN {
        TURN { MOVE(opponent, MOVE_EMBARGO); MOVE(player, MOVE_MULTI_ATTACK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Embargo can be reflected by Magic Coat (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIGHT_BALL); };
    } WHEN {
        TURN { MOVE(player, MOVE_MAGIC_COAT); MOVE(opponent, MOVE_EMBARGO); }
        TURN { MOVE(opponent, MOVE_FLING); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Embargo doesn't prevent Mega Evolution (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Embargo doesn't prevent Primal Reversion (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
TO_DO_BATTLE_TEST("Embargo doesn't prevent the usage of Z-Moves (Multi)")
TO_DO_BATTLE_TEST("Embargo doesn't block held item effects that affect prize money (Multi)")
#endif
