...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Embargo blocks the effect of an affected Pokémon's held item (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_FOCUS_SASH].holdEffect == HOLD_EFFECT_FOCUS_SASH);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Embargo blocks an affected Pokémon's trainer from using items (Items)")
{
    // As of writing, the battle tests system doesn't perform all the operations involved
    // in the action of an NPC using an item in battle.
    KNOWN_FAILING;
...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect experience gain (Items)", s32 exp)
{
    enum Item item;
...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect effort values (Items)")
{
    u32 finalHPEVAmount;
...
SINGLE_BATTLE_TEST("Embargo negates a held item's Speed reduction (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(19); }
...
// Friendship increase is 1, the held item effect actually does not affect the Friendship gained.
//
// WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect friendship (Items)")
// {
//     u32 initialFriendship;
//     u32 finalFriendship;
...
SINGLE_BATTLE_TEST("Embargo doesn't block a held item's form-changing effect, but it does block its other effects (Items)", s16 damage)
{
    u32 heldItem;
...
SINGLE_BATTLE_TEST("Embargo makes Fling and Natural Gift fail (Items)")
{
    u32 heldItem, moveId;
...
SINGLE_BATTLE_TEST("Baton Pass passes Embargo's effect (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Embargo doesn't block the effects of berries obtained through Bug Bite or Pluck (Items)")
{
    u32 hp = 10;
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Plate items on the move Judgment (Items)", s16 damage)
{
    u32 heldItem;
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Drive items on the move Techno Blast (Items)", s16 damage)
{
    u32 heldItem;
...
SINGLE_BATTLE_TEST("Embargo disables the effect of the Memory items on the move Multi Attack (Items)", s16 damage)
{
    u32 heldItem;
...
SINGLE_BATTLE_TEST("Embargo can be reflected by Magic Coat (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Embargo doesn't prevent Mega Evolution (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Embargo doesn't prevent Primal Reversion (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
TO_DO_BATTLE_TEST("Embargo doesn't prevent the usage of Z-Moves (Items)")
TO_DO_BATTLE_TEST("Embargo doesn't block held item effects that affect prize money (Items)")
#endif
