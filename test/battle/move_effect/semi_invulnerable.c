...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Semi-invulnerable moves don't need to charge with Power Herb (Items)")
{
    enum Move move;
...
SINGLE_BATTLE_TEST("Power Herb semi-invulnerable moves do not keep the user untargetable that turn (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_POWER_HERB) == HOLD_EFFECT_POWER_HERB);
        PLAYER(SPECIES_BASCULEGION) { Items(ITEM_GREAT_BALL, ITEM_POWER_HERB); Speed(20); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
    } WHEN {
        TURN { MOVE(player, MOVE_PHANTOM_FORCE); MOVE(opponent, MOVE_WATER_GUN); }
    } SCENE {
