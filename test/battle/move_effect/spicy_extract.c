...
SINGLE_BATTLE_TEST("Spicy Extract is prevented by target's ability if it's Attack stat is maxed out (Traits)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_BELDUM) { Ability(ABILITY_LIGHT_METAL); Innates(ability); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SWORDS_DANCE); }
        TURN { MOVE(opponent, MOVE_SWORDS_DANCE); }
        TURN { MOVE(opponent, MOVE_SWORDS_DANCE); MOVE(player, MOVE_SPICY_EXTRACT); }
...
AI_DOUBLE_BATTLE_TEST("Spicy Extract user will not choose the move if it does not benefit partner (Traits)")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
        OPPONENT(species) { Speed(20); Ability(ABILITY_LIGHT_METAL); Innates(ability); Moves(MOVE_SCRATCH); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(40); Moves(MOVE_SCRATCH, MOVE_SPICY_EXTRACT); }
    } WHEN {
        TURN {
            EXPECT_MOVE(opponentRight, MOVE_SCRATCH);
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Spicy Extract bypasses accuracy checks (Items)")
{
    PASSES_RANDOMLY(100, 100, RNG_ACCURACY);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Spicy Extract will fail if target is in a semi-invulnerability state (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_BRIGHTPOWDER); }
...
SINGLE_BATTLE_TEST("Spicy Extract against Clear Amulet and Contrary raises Defense only (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
AI_DOUBLE_BATTLE_TEST("Spicy Extract user will use it if partner holds Clear Amulet and a physical move (Items)")
{
    enum Move move;
