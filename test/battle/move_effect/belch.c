...
AI_SINGLE_BATTLE_TEST("AI: Belch has nonzero score after eating a berry (Multi)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_BAYLEEF) { Level(18); Moves(MOVE_MUD_SHOT, MOVE_SCRATCH); }
        OPPONENT(SPECIES_PIKACHU) { Level(15); Items(ITEM_GREAT_BALL, ITEM_SHUCA_BERRY); Moves(MOVE_BELCH, MOVE_SCRATCH); }
    } WHEN {
        TURN { MOVE(player, MOVE_MUD_SHOT); EXPECT_MOVE(opponent, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_SCRATCH); EXPECT_MOVE(opponent, MOVE_BELCH);}
    } SCENE {
...
SINGLE_BATTLE_TEST("Belch cannot be used if the user has not eaten a berry (Multi)")
{
    ...
    PARAMETRIZE { item = ITEM_ORAN_BERRY; }
    GIVEN {
        PLAYER(SPECIES_SKWOVET) { Items(ITEM_GREAT_BALL, item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        if (item == ITEM_NONE)
            TURN { MOVE(player, MOVE_BELCH, allowed: FALSE); MOVE(player, MOVE_CELEBRATE); }
