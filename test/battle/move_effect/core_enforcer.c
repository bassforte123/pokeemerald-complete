...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Core Enforcer does not suppress the ability of a target that switched in this turn (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WEEZING) { Ability(ABILITY_STENCH); Innates(ABILITY_LEVITATE); }
    } WHEN {
        TURN { SWITCH(opponent, 1); MOVE(player, MOVE_CORE_ENFORCER); }
        TURN { MOVE(player, MOVE_EARTHQUAKE); }
    } SCENE {
