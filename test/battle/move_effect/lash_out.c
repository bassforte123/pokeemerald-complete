...
SINGLE_BATTLE_TEST("Lash Out damage is only boosted on the turn that Intimidate switches in (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_GRIMMSNARL) { Moves(move, MOVE_CELEBRATE, MOVE_LASH_OUT); }
        OPPONENT(SPECIES_INCINEROAR) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_BLAZE); Moves(MOVE_CELEBRATE); }
        OPPONENT(SPECIES_INCINEROAR) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_INTIMIDATE); Moves(MOVE_CELEBRATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_LASH_OUT); MOVE(opponent, MOVE_CELEBRATE); }
        TURN { MOVE(player, move); SWITCH(opponent, 1); }
        TURN { MOVE(player, MOVE_LASH_OUT); MOVE(opponent, MOVE_CELEBRATE); }
...
DOUBLE_BATTLE_TEST("Lash Out damage is boosted by Cotton Down activation in doubles (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_GRIMMSNARL)  { Speed(1); Moves(MOVE_LASH_OUT); }
        PLAYER(SPECIES_RATTATA)     { Speed(2); Moves(MOVE_TACKLE, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_ELDEGOSS)  { Speed(3); Ability(ABILITY_LIGHT_METAL); Innates(ability); }
        OPPONENT(SPECIES_GOLEM)     { Speed(4); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_LASH_OUT, target:opponentRight); MOVE(playerRight, MOVE_TACKLE, target:opponentLeft); }
        TURN { MOVE(playerLeft, MOVE_LASH_OUT, target:opponentRight); }
