...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Psycho Shift burn is blocked by Substitute unless the user has Infiltrator (Traits)")
{
    enum Ability ability;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_PSYCHO_SHIFT) == EFFECT_PSYCHO_SHIFT);
        ASSUME(GetMoveEffect(MOVE_SUBSTITUTE) == EFFECT_SUBSTITUTE);
        PLAYER(SPECIES_DRAGAPULT) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Status1(STATUS1_BURN); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_SUBSTITUTE); MOVE(player, MOVE_PSYCHO_SHIFT); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Psycho Shift burn is blocked by Safeguard unless the user has Infiltrator (Traits)")
{
    enum Ability ability;
...
SINGLE_BATTLE_TEST("Psycho Shift triggers Synchronize before curing the user (Gen5+) (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_PSYCHO_SHIFT) == EFFECT_PSYCHO_SHIFT);
        PLAYER(SPECIES_WOBBUFFET) { Status1(STATUS1_BURN); }
