...
SINGLE_BATTLE_TEST("Bestow fails if the user is holding Mail")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_ORANGE_MAIL); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_BESTOW); }
    } SCENE {
...
{
    GIVEN {
        PLAYER(SPECIES_BLAZIKEN) { Items(ITEM_BLAZIKENITE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_BESTOW); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Bestow fails if the user's held item is a Z-Crystal")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_FIGHTINIUM_Z); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_BESTOW); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Bestow fails if the user has Sticky Hold (Multi)")
{
    GIVEN {
