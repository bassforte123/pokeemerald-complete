...
SINGLE_BATTLE_TEST("Octolock triggers Defiant for both stat reductions (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_BISHARP) { Ability(ABILITY_PRESSURE); Innates(ABILITY_DEFIANT); }
    } WHEN {
        TURN { MOVE(player, MOVE_OCTOLOCK); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_OCTOLOCK, player);
...
SINGLE_BATTLE_TEST("Octolock stat drops are not reflected by Mirror Armor (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Octolock stat drop is prevented by Flower Veil (Traits)")
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_CHIKORITA, 0) == TYPE_GRASS || GetSpeciesType(SPECIES_CHIKORITA, 1) == TYPE_GRASS);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_COMFEY) { Ability(ABILITY_TRIAGE); Innates(ABILITY_FLOWER_VEIL); }
        OPPONENT(SPECIES_CHIKORITA);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_OCTOLOCK, target: opponentRight); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Octolock reduction is prevented by Clear Amulet (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
