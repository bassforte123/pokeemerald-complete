...
SINGLE_BATTLE_TEST("Stuff Cheeks raises Defense by 2 stages after consuming the berry and gaining its effect (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Stuff Cheeks can be used even if Magic Room is active (Multi)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Stuff Cheeks fails if the user's berry is removed before they use the move (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_KNOCK_OFF) == EFFECT_KNOCK_OFF);
        PLAYER(SPECIES_SKWOVET) { Items(ITEM_LIECHI_BERRY, ITEM_PECHA_BERRY); }
...
AI_SINGLE_BATTLE_TEST("AI uses Stuff Cheeks (Multi)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STUFF_CHEEKS) == EFFECT_STUFF_CHEEKS);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_OMNISCIENT);
