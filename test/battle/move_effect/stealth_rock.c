...
SINGLE_BATTLE_TEST("Stealth Rock damages the correct pokemon when Eject Button is triggered (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_METAPOD) { Items( ITEM_GREEN_APRICORN, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_METAPOD);
        OPPONENT(SPECIES_JOLTEON);
    } WHEN {
        TURN { MOVE(opponent, MOVE_STEALTH_ROCK); MOVE(player, MOVE_HARDEN); }
...
DOUBLE_BATTLE_TEST("Stealth Rock damages the correct pokemon when Eject Button is triggered in double battle (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_METAPOD) { Items( ITEM_GREEN_APRICORN, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_METAPOD) { Items( ITEM_GREEN_APRICORN, ITEM_EJECT_BUTTON); }
