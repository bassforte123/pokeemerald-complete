...
DOUBLE_BATTLE_TEST("Magic Room prevents item hold effects (Multi)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(100); HP(60); Items(ITEM_PECHA_BERRY, ITEM_BERRY_JUICE); }
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(100); HP(60); Items(ITEM_PECHA_BERRY, ITEM_BERRY_JUICE); }
        OPPONENT(SPECIES_WOBBUFFET) { MaxHP(100); HP(60); Items(ITEM_PECHA_BERRY, ITEM_BERRY_JUICE); }
        OPPONENT(SPECIES_WOBBUFFET) { MaxHP(100); HP(60); Items(ITEM_PECHA_BERRY, ITEM_BERRY_JUICE); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_MAGIC_ROOM); }
        TURN {
...
SINGLE_BATTLE_TEST("Magic Room: An item that can activate will activate once Magic Room is over (Multi)")
{
    GIVEN {
