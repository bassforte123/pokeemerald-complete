...
SINGLE_BATTLE_TEST("Pursuit doesn't trigger a switching mon's Emergency Exit (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_GOLISOPOD) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_EMERGENCY_EXIT); HP(251); MaxHP(500); }
        PLAYER(SPECIES_ZIGZAGOON);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { SWITCH(player, 1); MOVE(opponent, MOVE_PURSUIT); }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Pursuit attacks a switching foe and takes Life Orb damage (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Pursuit user mega evolves before attacking a switching foe and hits twice if user has Parental Bond (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_ZIGZAGOON);
...
// Checked so that Pursuit has only 1 PP and it forces the player to use Struggle.
SINGLE_BATTLE_TEST("Pursuit becomes a locked move after being used on switch-out while holding a Choice Item (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_CHOICE_BAND].holdEffect == HOLD_EFFECT_CHOICE_BAND);
...
SINGLE_BATTLE_TEST("Pursuit user gets forced out by Red Card and target still switches out (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_RED_CARD].holdEffect == HOLD_EFFECT_RED_CARD);
...
SINGLE_BATTLE_TEST("Pursuit user faints to Life Orb and target still switches out (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_LIFE_ORB].holdEffect == HOLD_EFFECT_LIFE_ORB);
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Pursuit user switches out due to Red Card and partner's switch is cancelled if switching to same Pokémon (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_RED_CARD) == HOLD_EFFECT_RED_CARD);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Pursuit doesn't trigger a switching mon's Eject Button (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_EJECT_BUTTON) == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_GREAT_BALL, ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_ZIGZAGOON);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { SWITCH(player, 1); MOVE(opponent, MOVE_PURSUIT); }
...
// Extrapolated from the previous test's mechanic
SINGLE_BATTLE_TEST("Pursuit doesn't trigger a switching mon's Eject Pack (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_EJECT_PACK) == HOLD_EFFECT_EJECT_PACK);
        PLAYER(SPECIES_GOOMY) { Items(ITEM_GREAT_BALL, ITEM_EJECT_PACK); Ability(ABILITY_GOOEY); }
        PLAYER(SPECIES_ZIGZAGOON);
        OPPONENT(SPECIES_CORVIKNIGHT) { Ability(ABILITY_MIRROR_ARMOR); }
    } WHEN {
        TURN { SWITCH(player, 1); MOVE(opponent, MOVE_PURSUIT); }
