...
#if MAX_MON_ITEMS > 1
WILD_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing a Bisharp that holds Leader's Crest increases tracker (Items)")
{
    GIVEN {
        PLAYER(SPECIES_BISHARP);
        OPPONENT(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
        HP_BAR(opponent, hp: 0);
...
// To be replaced with WILD_DOUBLE_BATTLE_TEST when that is made possible (also see TryUpdateEvolutionTracker)
DOUBLE_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing multiple Bisharps holding Leader's Crest increases tracker multiple times (Items)")
{
    GIVEN {
        ASSUME(GetMoveTarget(MOVE_LAVA_PLUME) == TARGET_FOES_AND_ALLY);
        PLAYER(SPECIES_BISHARP);
        PLAYER(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
        OPPONENT(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_LAVA_PLUME); }
...
WILD_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing a Bisharp that doesn't hold Leader's Crest doesn't increase tracker (Items)")
{
    GIVEN {
        PLAYER(SPECIES_BISHARP);
        OPPONENT(SPECIES_BISHARP) { HP(1); }
...
WILD_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing a non-Bisharp that holds Leader's Crest doesn't increase tracker (Items)")
{
    u32 species;
    ...
    GIVEN {
        PLAYER(SPECIES_BISHARP);
        OPPONENT(species) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
        HP_BAR(opponent, hp: 0);
...
WILD_BATTLE_TEST("Evolution Tracker: Pawniard KO-ing a Bisharp that holds Leader's Crest doesn't increase tracker (Items)")
{
    GIVEN {
        PLAYER(SPECIES_PAWNIARD);
        OPPONENT(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
...
WILD_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing eligible battler from contact effects doesn't increase tracker (Items)")
{
    GIVEN {
        ASSUME(GetItemHoldEffect(ITEM_ROCKY_HELMET) == HOLD_EFFECT_ROCKY_HELMET);
        ASSUME(MoveMakesContact(MOVE_SCRATCH));
        PLAYER(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY,ITEM_ROCKY_HELMET); }
        OPPONENT(SPECIES_BISHARP) { Items(ITEM_PECHA_BERRY, ITEM_LEADERS_CREST); HP(1); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
WILD_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing eligible battler with passive damage doesn't increase tracker (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_LEECH_SEED) == EFFECT_LEECH_SEED);
        PLAYER(SPECIES_BISHARP);
...
// To be replaced with WILD_DOUBLE_BATTLE_TEST when that is made possible (also see TryUpdateEvolutionTracker)
DOUBLE_BATTLE_TEST("Evolution Tracker: Bisharp KO-ing eligible battler with bursting flames doesn't increase tracker (Items)")
{
    GIVEN {
        ASSUME(MoveHasAdditionalEffect(MOVE_FLAME_BURST, MOVE_EFFECT_FLAME_BURST));
        PLAYER(SPECIES_BISHARP);
